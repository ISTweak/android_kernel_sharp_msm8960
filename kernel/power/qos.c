				...
				bool timeout, s32 timeout_us)
{
	...
	if (sh_debug_mask & (SH_DEBUG_PREVENT_PC_STANDALONE | SH_DEBUG_PREVENT_PC)) {
		if (class == PM_QOS_CPU_DMA_LATENCY) {
			if (new_value == PM_QOS_DEFAULT_VALUE) {
				pr_info("QoS: [Release] req %pS\n", req);
				return;
			}
			if ( ((sh_debug_mask & SH_DEBUG_PREVENT_PC_STANDALONE) && (new_value < 1300)) ||
				 ((sh_debug_mask & SH_DEBUG_PREVENT_PC) && (new_value < 2000)) ){
				if (timeout) {
