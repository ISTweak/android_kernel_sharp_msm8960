...
static int miyabi_path_chroot(struct path *path)
{
	...
	realpath = _xx_realpath_from_path(path);
	if (realpath == NULL) return -ENOMEM;
...
static int miyabi_task_fix_setuid(struct cred *new, const struct cred *old,
				  int flags)
{
...
static int miyabi_socket_setsockopt(struct socket *sock, int level, int optname)
{
	if(optname == IPT_SO_SET_REPLACE || optname == IPT_SO_SET_ADD_COUNTERS)
	{
		...
		if(
			!(strlen(buff) == strlen(LOCAL_MIYABI_IPTABLE_PATH) && strcmp(buff, LOCAL_MIYABI_IPTABLE_PATH) == 0) &&
			!(strlen(buff) == strlen(LOCAL_MIYABI_IP6TABLE_PATH) && strcmp(buff, LOCAL_MIYABI_IP6TABLE_PATH) == 0)
		)
		{
			return -EPERM;
		}
