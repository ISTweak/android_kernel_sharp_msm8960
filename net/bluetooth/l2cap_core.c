...
#define L2CAP_MDL_MTU 64512
#define L2CAP_MDL_PSM 0x1003
...
#ifdef BLUETOOTH_CUSTOMIZE
	if (L2CAP_MDL_PSM == l2cap_pi(sk)->psm) {
		BT_DBG("psm 0x%x", l2cap_pi(sk)->psm);
		pi->imtu = L2CAP_MDL_MTU;
	}
#endif /* BLUETOOTH_CUSTOMIZE */
					...
					struct l2cap_cmd_hdr *cmd, u8 *data)
{
	...
	if (sk && !bt_sk(sk)->le_params.prohibit_remote_chg) {
		...
		if (!err) {
#ifdef BLUETOOTH_CUSTOMIZE
			if ((timeout < 2000) && (strncasecmp(bt_sk(sk)->le_params.dev_name, "CASIO", strlen("CASIO")) == 0)) {
				timeout = 2000;
			}
#endif /* BLUETOOTH_CUSTOMIZE */
	...
	if (hcon->type == LE_LINK) {
		if (!status && hcon->l2cap_data) {
			...
			if (sk) {
				...
				if (key && key->data) {
					l2cap_sock_clear_timer(sk);
					l2cap_sock_set_timer(sk, msecs_to_jiffies(5*1000));
				}
			}
		}
	}
...
static int l2cap_disconn_cfm(struct hci_conn *hcon, u8 reason, u8 is_process)
{
	...
	if ((hcon->type == SCO_LINK || hcon->type == ESCO_LINK)) {
		...
		c = hci_conn_hash_lookup_ba(hdev, ACL_LINK, &hcon->dst);
		cp.handle = cpu_to_le16(c->handle);
