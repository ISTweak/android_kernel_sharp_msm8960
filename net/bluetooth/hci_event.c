...
static void hci_cs_le_start_enc(struct hci_dev *hdev, u8 status)
{
	...
	if (status) {
		...
		clear_bit(HCI_CONN_LE_START_ENC_PEND, &conn->pend);
		hci_encrypt_cfm(conn, status, 0);

		if (test_bit(HCI_MGMT, &hdev->flags))
			mgmt_encrypt_change(hdev->id, &conn->dst, status);
unlock:
		hci_dev_unlock(hdev);
	}
...
#ifdef BLUETOOTH_CUSTOMIZE
		if ((ev->status == 0x05) && hci_find_link_key(hdev, &conn->dst)) {
				struct hci_cp_auth_requested cp;
				hci_remove_link_key(hdev, &conn->dst);
				cp.handle = cpu_to_le16(conn->handle);
				hci_send_cmd(conn->hdev, HCI_OP_AUTH_REQUESTED,
...
#ifdef BLUETOOTH_CUSTOMIZE
static inline void hci_key_refresh_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
							...
							struct sk_buff *skb)
{
	...
	if (data) {
		...
		hci_send_cmd(hdev, HCI_OP_REMOTE_OOB_DATA_REPLY, sizeof(cp),
									&cp);
#ifdef BLUETOOTH_CUSTOMIZE
		hci_remove_remote_oob_data(hdev, &ev->bdaddr);
#endif /* BLUETOOTH_CUSTOMIZE */
	} else {
...
#ifdef BLUETOOTH_CUSTOMIZE
	clear_bit(HCI_CONN_LE_CONN_UPDATE_PEND, &conn->pend);
	if (test_and_clear_bit(HCI_CONN_LE_START_ENC_PEND, &conn->pend)) {
		BT_DBG("HCI_CONN_LE_START_ENC_PEND, send HCI LE Start Encryption");
		hci_send_cmd(hdev, HCI_OP_LE_START_ENC, sizeof(conn->cp), &conn->cp);
		memset(&conn->cp, 0, sizeof(conn->cp));
	}
#endif /* BLUETOOTH_CUSTOMIZE */
