...
/* SH_AUDIO_DRIVER -> */ /*02-178*/
#include <linux/irq.h>
#include <linux/wakelock.h>
#include <linux/suspend.h>
/* SH_AUDIO_DRIVER <- */ /*02-178*/
#include "wcd9310.h"
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-168*/
#include <linux/wakelock.h>
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-168*/
...
#if (CONFIG_SH_AUDIO_DRIVER_MODEL_NUMBER == 304)/*02-154*/
#define TABLA_FAKE_REMOVAL_MIN_PERIOD_MS 250
#else
#define TABLA_FAKE_REMOVAL_MIN_PERIOD_MS 50
#endif /* CONFIG_SH_AUDIO_DRIVER *//*02-154*/
#define TABLA_MBHC_BUTTON_MIN 0x8000

#define TABLA_MBHC_FAKE_INSERT_LOW 10
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-113*/
#define TABLA_MBHC_FAKE_INSERT_HIGH 150
#else
#define TABLA_MBHC_FAKE_INSERT_HIGH 80
#endif /* CONFIG_SH_AUDIO_DRIVER *//*02-113*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-168*/
static struct wake_lock wakelock_jack;
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-168*/

#ifdef CONFIG_SH_AUDIO_DRIVER /*02-133*/
struct tabla_priv *tabla_switch;
struct tabla_bandgap_timer {
 struct delayed_work work;
 struct wake_lock    wakelock;
};

static struct tabla_bandgap_timer tabla_bandgap_timer;

static void tabla_bandgap_reg_off(struct work_struct *work)
{
 struct snd_soc_codec *codec = tabla_switch->codec;
 snd_soc_write(codec, TABLA_A_BIAS_CENTRAL_BG_CTL, 0x00);
}
EXPORT_SYMBOL_GPL(tabla_bandgap_reg_off);

static void tabla_bandgap_timer_init(void)
{
 INIT_DELAYED_WORK(&tabla_bandgap_timer.work, tabla_bandgap_reg_off);
 wake_lock_init(&tabla_bandgap_timer.wakelock, WAKE_LOCK_SUSPEND, "tabla_bandgap_timer");
}
EXPORT_SYMBOL_GPL(tabla_bandgap_timer_init);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-133*/
	...
	enum tabla_bandgap_type choice)
{
	...
	if ((tabla->bandgap_type == TABLA_BANDGAP_OFF) &&
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-133*/
                if (delayed_work_pending(&tabla_bandgap_timer.work)) {
                        cancel_delayed_work_sync(&tabla_bandgap_timer.work);
                        wake_unlock(&tabla_bandgap_timer.wakelock);
                }
#endif /*CONFIG_SH_AUDIO_DRIVER*/ /*02-133*/
		tabla_codec_enable_audio_mode_bandgap(codec);
	} else if (choice == TABLA_BANDGAP_MBHC_MODE) {
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-198*/
		if (delayed_work_pending(&tabla_bandgap_timer.work)) {
			cancel_delayed_work_sync(&tabla_bandgap_timer.work);
			wake_unlock(&tabla_bandgap_timer.wakelock);
		}
#endif /* CONFIG_SH_AUDIO_DRIVER *//*02-198*/
		...
		tabla_codec_enable_audio_mode_bandgap(codec);
               ...
               cancel_delayed_work_sync(&tabla_bandgap_timer.work);
               wake_unlock(&tabla_bandgap_timer.wakelock);
               wake_lock_timeout(&tabla_bandgap_timer.wakelock, 1 * HZ);
               schedule_delayed_work(&tabla_bandgap_timer.work, msecs_to_jiffies(1000));
#else
		snd_soc_write(codec, TABLA_A_BIAS_CENTRAL_BG_CTL, 0x50);
#endif /*CONFIG_SH_AUDIO_DRIVER*/ /*02-133*/
	} else {
...
#ifdef CONFIG_SH_AUDIO_DRIVER
		/* register setting */
		if (bias_mode_for_testmode) {
			if ( w->reg == TABLA_A_MICB_1_CTL)
			{
				pr_debug("%s: TABLA_A_MICB_1_CTL set register\n", __func__);
				snd_soc_update_bits(codec, w->reg, 0xFF, 0x86);
				snd_soc_update_bits(codec, TABLA_A_MICB_CFILT_1_CTL, 0xFF, 0xC0);
			}
			else if ( w->reg == TABLA_A_MICB_3_CTL)
			{
				pr_debug("%s: TABLA_A_MICB_3_CTL set register\n", __func__);
				snd_soc_update_bits(codec, w->reg, 0xFF, 0xC6);
				snd_soc_update_bits(codec, TABLA_A_MICB_CFILT_3_CTL, 0xFF, 0xC0);
			}
		} else {
			snd_soc_update_bits(codec, w->reg, 0x0E, 0x0A);
		}
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-137*/
		if (w->reg == TABLA_A_MICB_1_CTL && tabla->current_plug == PLUG_TYPE_HEADSET) {
			pr_debug("%s micbias 2 enable\n", __func__);
			snd_soc_update_bits(codec, tabla->mbhc_bias_regs.ctl_reg, 0x80, 0x80);
		}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-137*/
...
/*SH_AUDIO_DRIVER->*//*02-097*/
		...
		if (bias_mode_for_testmode) {
			if ( w->reg == TABLA_A_MICB_1_CTL)
			{
				pr_debug("%s: set register\n", __func__);
				snd_soc_update_bits(codec, TABLA_A_MICB_1_CTL, 0xFF, 0x16);
				snd_soc_update_bits(codec, TABLA_A_MICB_CFILT_1_CTL, 0xFF, 0x40);
			}
			else if ( w->reg == TABLA_A_MICB_3_CTL)
			{
				pr_debug("%s: set register\n", __func__);
				snd_soc_update_bits(codec, TABLA_A_MICB_3_CTL, 0xFF, 0x16);
				snd_soc_update_bits(codec, TABLA_A_MICB_CFILT_3_CTL, 0xFF, 0x40);
			}
		}

#ifdef CONFIG_SH_AUDIO_DRIVER /*02-137*/
		if (w->reg == TABLA_A_MICB_1_CTL) {
			u8 mbhc_micb_ctl_val = snd_soc_read(codec, tabla->mbhc_bias_regs.ctl_reg);
			if (mbhc_micb_ctl_val & 0x80) {
				pr_debug("%s micbias 2 disable\n", __func__);
				snd_soc_update_bits(codec, tabla->mbhc_bias_regs.ctl_reg, 0x80, 0x00);
			}
		}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-137*/
#endif /* CONFIG_SH_AUDIO_DRIVER */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-168*/
    wake_lock_timeout(&wakelock_jack, 1 * HZ);
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-168*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 02-191 */
	u8 main_micb_ctl_val;
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 02-191 */
	pr_debug("%s: event = %d\n", __func__, event);
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 02-191 */
		main_micb_ctl_val = snd_soc_read(codec, TABLA_A_MICB_1_CTL);
		if ( !(mbhc_micb_ctl_val & 0x80) ||
		((main_micb_ctl_val & 0x80) && tabla->current_plug == PLUG_TYPE_HEADSET)
		) {
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 02-191 */
		if (!(mbhc_micb_ctl_val & 0x80)) {
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 02-191 */
			TABLA_ACQUIRE_LOCK(tabla->codec_resource_lock);
			tabla_codec_switch_micbias(codec, 1);
				    ...
				    enum snd_jack_types jack_type)
{
	...
	if (jack_type == SND_JACK_HEADSET) {
		u8 mbhc_micb_ctl_val = snd_soc_read(codec, TABLA_A_MICB_1_CTL);
		if (mbhc_micb_ctl_val & 0x80) {
			if (!insertion) {
				u8 mbhc_micb_ctl_val = snd_soc_read(codec, tabla->mbhc_bias_regs.ctl_reg);
				if (mbhc_micb_ctl_val & 0x80) {
				     ...
				     s32 mic_volt, bool highhph, bool *highv)
{
	...
	*highv = mic_volt > v_hs_max;
	if (!highhph && *highv)
		invalid = true;
#ifndef CONFIG_SH_AUDIO_DRIVER /*02-113*/
	...
	for (i = 0; (plug_type[0] != PLUG_TYPE_GND_MIC_SWAP && !inval) &&
		...
		if (mic_mv[i] < plug_type_ptr->v_no_mic) {
			...
			pr_debug("%s: Detect attempt %d, detected High "
				 "Headphone\n", __func__, i);
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-113*/
		} else if (mic_mv[i] > tabla->mbhc_data.v_inval_ins_low && mic_mv[i] < tabla->mbhc_data.v_inval_ins_high) {
			plug_type[i] = PLUG_TYPE_HEADPHONE;
			pr_debug("%s: Detect attempt %d, detected Headphone\n",
				 __func__, i);
		} else {
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-096*/
/* setting register */
...
#endif /* CONFIG_SH_AUDIO_DRIVER *//*02-096*/
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-096*/
	TABLA_REG_VAL(TABLA_1_A_MICB_4_INT_RBIAS, 0x00),
#else /* CONFIG_SH_AUDIO_DRIVER */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-096*/
	TABLA_REG_VAL(TABLA_2_A_MICB_4_INT_RBIAS, 0x00),
#else /* CONFIG_SH_AUDIO_DRIVER */
...
static const struct tabla_reg_mask_val tabla_1_x_codec_reg_init_val[] = {
	/* Initialize mic biases to differential mode */
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-096*/
	{TABLA_1_A_MICB_4_INT_RBIAS, 0xFF, 0x00},
#else /* CONFIG_SH_AUDIO_DRIVER */
...
static const struct tabla_reg_mask_val tabla_2_higher_codec_reg_init_val[] = {
	/* Initialize mic biases to differential mode */
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-096*/
	{TABLA_2_A_MICB_4_INT_RBIAS, 0xFF, 0x00},
#else /* CONFIG_SH_AUDIO_DRIVER */
...
#endif
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-168*/
    wake_lock_init(&wakelock_jack, WAKE_LOCK_SUSPEND, "tabla_jack_report");
#endif /* CONFIG_SH_AUDIO_DRIVER */ /*02-168*/
	codec->ignore_pmdown_time = 1;
#ifdef CONFIG_SH_AUDIO_DRIVER /*02-133*/
    tabla_switch = tabla;
 tabla_bandgap_timer_init();
#endif /*CONFIG_SH_AUDIO_DRIVER*/ /*02-133*/
	return ret;
