...
#define TSIF_NUM_OF_EACH_TSBUFFER	128	/* 192*21 < 1page(4096) < 192*22 (byte) 6 pages needed to avoid TSIF IRQ: OVERFLOW */
#define TSIF_NUM_OF_EACH_TSBUFFER_1SEG  16 /* equal to original TSIF_PKTS_IN_CHUNK_DEFAULT */
#define TSIF_MM_TSBUFFER_WIDTH		8	/* 128*8 = 1024 (packet) */ 
#define TSIF_1SEG_TSBUFFER_WIDTH	16	/* double size of original TSIF_CHUNKS_IN_BUF_DEFAULT */
#define TS_PKT_IN_2ND_BUF   (TSIF_NUM_OF_EACH_TSBUFFER * TSIF_MM_TSBUFFER_WIDTH)
/* The largest to-be-required buffer size in MMB and 1SEG */
#define TSIF_2ND_BUF_SIZE	(TSIF_NUM_OF_EACH_TSBUFFER * TSIF_MM_TSBUFFER_WIDTH * TSIF_PKT_SIZE)  	

typedef struct {
	unsigned char  data_buffer_ptr[TSIF_2ND_BUF_SIZE];	
	int  cnt_data_buffer_ptr;	
    int  wi;           
    int  ri;           
    unsigned rptr;
    unsigned dev_rptr;
	unsigned num_of_each_tsbuffer;
} tsif_2nd_buffer_t;

extern tsif_2nd_buffer_t tsif_2nd_buffer;

extern unsigned char * tsif_pcktloc_from_num(tsif_2nd_buffer_t *p2nd_buffer, int num);
