...
struct usb_gadget_ops {
	...
	int	(*notify_chg_info) (struct usb_gadget *, unsigned mA);
#endif /* CONFIG_USB_ANDROID_SH_CUST */
#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
	int	(*notify_enumeration_started)(struct usb_gadget *);
...
static inline int usb_gadget_notify_chg_info(struct usb_gadget *gadget, unsigned mA)
{
	if (!gadget->ops->notify_chg_info)
		return -EOPNOTSUPP;
	return gadget->ops->notify_chg_info(gadget, mA);
}
#endif /* CONFIG_USB_ANDROID_SH_CUST */

#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
static inline int usb_gadget_notify_enumeration_started(struct usb_gadget *gadget)
{
	if (!gadget->ops->notify_enumeration_started)
		return -EOPNOTSUPP;
	return gadget->ops->notify_enumeration_started(gadget);
}
#endif /* CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE */
