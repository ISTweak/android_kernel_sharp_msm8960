...
#define CFG_GET_RUNTIME_INFO          47
/* QUALCOMM NADESHIKO MERGE <- */
/* SHLOCAL_CAMERA_IMAGE_QUALITY<- */
#define CFG_MAX			48


#define CFG_SHDIAG_GET_I2C_DATA	101
#define CFG_SHDIAG_SET_I2C_DATA	102
#define CFG_SHDIAG_GET_OTP_DATA	103
#define CFG_SHDIAG_ACTUATOR_SET_TABLE	104
#define CFG_SHDIAG_SET_I2C_TABLE_DATA	105
#define CFG_SHDIAG_SET_SMEM	106
#define CFG_SHDIAG_ACTUATOR_SET_SMEM 107
#define CFG_SHDIAG_SET_OIS	108
#define CFG_SH_SET_EXP_GAIN	109
#define CFG_SHDIAG_AE_LOCK	110
#define CFG_SH_ACTUATOR_FW_DL	111
#define CFG_SH_ACTUATOR_INIT_LENSPOS  112
#define CFG_SH_GET_AF_SEARCH_STATUS 113
#define CFG_SH_SET_FOCUS_MODE 114
#define CFG_SH_GET_SNAPSHOT_INFO 115
...
#define CAMERA_EFFECT_BLEACHBYPASS		12
...
struct focus_info_t {
	...
	uint16_t af_stage;
	int16_t af_position;
	uint16_t af_1st_status;
	uint8_t af_2nd_status;
	uint32_t af_search_status;
	uint32_t af_integrated_value;
};

struct sensor_snapshot_info_t {
	uint32_t exp_value;
	uint16_t a_gain;
	uint16_t d_gain;
};
...
struct af_info_t {
	uint16_t lens_start;
	uint16_t lens_end;
	uint16_t search_point;
	uint16_t *p_step_position_table;
};
...
/* SHLOCAL_CAMERA_IMAGE_QUALITY-> */
/* QUALCOMM NADESHIKO MERGE -> */
		struct runtime_info run_info;
/* QUALCOMM NADESHIKO MERGE <- */
/* SHLOCAL_CAMERA_IMAGE_QUALITY<- */
		struct focus_info_t focus_info;
		struct sensor_snapshot_info_t sensor_snapshot_info;
	} cfg;
};
...
enum ois_mode {
	SHCAM_OIS_MODE_OFF,
	SHCAM_OIS_MODE_STILL,
	SHCAM_OIS_MODE_VIDEO
};
...
/* SHLOCAL_CAMERA_IMAGE_QUALITY-> */
/* QUALCOMM NADESHIKO MERGE -> */
	struct pixel_t snapshot_coord[287];
	struct pixel_t preview_coord[287];
	struct pixel_t video_coord[287];
	struct pixel_t video60_coord[287];
	struct pixel_t video120_coord[287];
/* QUALCOMM NADESHIKO MERGE <- */
/* SHLOCAL_CAMERA_IMAGE_QUALITY<- */
};
