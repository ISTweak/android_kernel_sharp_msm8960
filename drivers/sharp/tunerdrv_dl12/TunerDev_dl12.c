...
#endif
#if VALID_GPIO_DTVCRDDET
	{GPIO_DTVCRDDET_PORTID	, GPIO_DTVCRDDET_PORTNO	, DirctionIn	, 0				, 0				, "gpio_dtv_crddet"	},
#endif
};
...
static int tuner_clk_enable(void)
{
	...
	gpio_cfg = GPIO_CFG( CLOCK_CONTROL_PORTNO, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA );
...
//	clk_enable( gp_clk );
	...
	writel(0xA00, MSM_CLK_CTL_BASE + 0x2D24 + 32*( 0 ));
...
#else
    if ( id == GPIO_DTVCRDDET_PORTID ) {
        shswic_get_cradle_status( &device );
    	*val = ( device == SHSWIC_ID_CRADLE ) ? 0 : 1;
	//	printk("gpio_get() No.%d val[%d] \n", id, *val);
    }
    else {
	    *val = gpio_get_value_cansleep(portno);
    }
...
static int tuner_vreg_enable(void)
{
    ...
    if ( ret != 0 ) {
		printk("Unable to set voltage : %s regulator\n", id);
		return -2;
    }
	...
	if (!regulator_is_enabled(reg)) {
		...
		if ( ret != 0 ) {
			printk("regulator_enable : failed %s regulator\n", id);
			return -3;
		}
	}
