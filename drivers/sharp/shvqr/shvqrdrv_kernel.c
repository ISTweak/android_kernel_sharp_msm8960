...
#define SHVQRDEV_CHECK_PGOFF_A	(0x8FB00000 >> PAGE_SHIFT)
#define SHVQRDEV_CHECK_PGOFF_B	(0x8F700000 >> PAGE_SHIFT)
#define SHVQRDEV_CHECK_MAX_SIZE	(3 * 1024 * 1024)
			...
			if(file)
			{
				if(vm_area->vm_flags & VM_EXEC)
				{
					...
					if(vm_area->vm_flags & VM_READ)
					{
						...
						if(strcmp(p, SHVQRDEV_CHECK_PATH) == 0) ret = 0;
					}
					else
					{
						ret = -EPERM;
...
static int shvqrdrv_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if(size < 0 || size > SHVQRDEV_CHECK_MAX_SIZE)
	{
		return -EINVAL;
	}
