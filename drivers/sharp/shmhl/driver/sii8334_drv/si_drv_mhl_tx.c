...
#ifdef CONFIG_SII8334_MHL_TX
#define SHMHL_CI2CA_OFF 0x00
#define SHMHL_CI2CA_ON  0x01

#define SHMHL_ISR_WAIT				30		/* msec */
#define SHMHL_RGND_TIMEOUT			500		/* msec */
#define SHMHL_RGND_TIMEOUT_COUNT	(SHMHL_RGND_TIMEOUT) / (SHMHL_ISR_WAIT)
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
static bool_t shmhl_judge_I2C_accessible(void);
static void shmhl_RegInstanceSet_for_CI2CA(uint8_t ci2ca);
/* #define SHMHL_GET_DEFAULT_REG_VALUE */
#endif /* CONFIG_SII8334_MHL_TX */

#ifdef CONFIG_SII8334_MHL_TX
extern uint8_t shmhl_reg_eye_pattern;
#endif /* CONFIG_SII8334_MHL_TX */
...
{
#ifdef CONFIG_SII8334_MHL_TX
	static bool_t first_flg = true;
	bool_t i2c_ok_flg = true;
#endif /* CONFIG_SII8334_MHL_TX */
	tmdsPoweredUp = false;
	mhlConnected = false;
...
#ifdef CONFIG_SII8334_MHL_TX
	/* 1.2V Power On for Register Access*/
	shmhl_set_port_vcc12_up();
#endif /* CONFIG_SII8334_MHL_TX */
	...
	if(first_flg) {
		/* I2C access OK */
		if(shmhl_judge_I2C_accessible()) {
			TX_DEBUG_PRINT(("Drv: SiiMhlTxChipInitialize I2C access OK\n"));
		}
		/* I2C access NG */
		else {
			/* CI2CA ON */
			shmhl_RegInstanceSet_for_CI2CA(SHMHL_CI2CA_ON);
			...
			/* I2C access NG */
				...
				TX_DEBUG_PRINT(("Drv: SiiMhlTxChipInitialize I2C access ERROR!\n"));
				i2c_ok_flg = false;
			}
		}
		...
		/* Read Device ID */
		if(i2c_ok_flg) {
			shmhl_set_dev_id(SiiRegRead(REG_DEV_IDL), SiiRegRead(REG_DEV_IDH));

#ifdef SHMHL_GET_DEFAULT_REG_VALUE
			printk("Reg_Dump(DEFAULT) Start -------------------------------\n");
			shmhl_SiI8334_Reg_Dump_All_Offset(NULL, 0);
			printk("Reg_Dump(DEFAULT) End -------------------------------\n");
			WriteInitialRegisterValues();
			printk("Reg_Dump(AFTER INIT) Start -------------------------------\n");
			shmhl_SiI8334_Reg_Dump_All_Offset(NULL, 0);
			printk("Reg_Dump(AFTER INIT) End -------------------------------\n");
#endif /* SHMHL_GET_DEFAULT_REG_VALUE */
		}
...
void SiiMhlTxDeviceIsr (void)
{
...
#ifdef CONFIG_SII8334_MHL_TX
	TX_DEBUG_PRINT(("SiiMhlTxDeviceIsr() Start\n"));

	/* Ignore Isr because already 1.2V  Power Off */
	if(POWER_STATE_D3 == fwPowerState) {
		TX_DEBUG_PRINT(("SiiMhlTxDeviceIsr() Ignore.(Already 1.2V Power Off)\n"));
		return;
	}
#endif /* CONFIG_SII8334_MHL_TX */
	...
	do
	{
    	if( POWER_STATE_D0_MHL != fwPowerState )
    	{
    		...
    		{
#ifdef CONFIG_SII8334_MHL_TX
				TX_DEBUG_PRINT(("SiiMhlTxDeviceIsr() End\n"));
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
		if ((shmhl_get_con_state() == SHMHL_CON_STATE_RGND) && (i >= SHMHL_RGND_TIMEOUT_COUNT)) {
			TX_DEBUG_PRINT(("SiiMhlTxDeviceIsr() Timeout\n"));
			break;
		}
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
	HalTimerWait(200);		/* Avoid CTS 3.2.6.1 Fail */
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
	if (shmhl_reg_eye_pattern != 0x00) {
		TX_DEBUG_PRINT(("Drv:set REG_MHLTX_CTL4 :0x%02X \n", shmhl_reg_eye_pattern));
		SiiRegWrite(REG_MHLTX_CTL4, shmhl_reg_eye_pattern);
	}
	else {
#ifdef CONFIG_SII8334_MHL_TX_WITH_SWIC
		/* With SWIC */
		SiiRegWrite(REG_MHLTX_CTL4, 0xCD);
...
#ifdef CONFIG_SII8334_MHL_TX
	SiiRegModify(REG_INT_CTRL, BIT2 | BIT6,0);	/* INT & HPD pin Push/Pull */
#else /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
	shmhl_notify_si_state(SHMHL_SI_STATE_RGND, (void*)&rgndImpedance);
#endif /* CONFIG_SII8334_MHL_TX */
...
static int Int4Isr (void)
{
	...
	if(0xFF != int4Status)
	{
...
#ifdef CONFIG_SII8334_MHL_TX
			shmhl_notify_si_state(SHMHL_SI_STATE_MHL_EST, NULL);
#endif /* CONFIG_SII8334_MHL_TX */
		...
		else if(int4Status & BIT3)
		{
			...
			shmhl_notify_si_state(SHMHL_SI_STATE_DISC_FAIL, NULL);
#endif /* CONFIG_SII8334_MHL_TX */
		...
		{
#ifdef CONFIG_SII8334_MHL_TX
			/* VSENSE is always High. */
			/* So below process is never used. */
			/* We must wait for shmhl_force_disconnect() is called. */
...
#ifdef CONFIG_SII8334_MHL_TX
        if( retrySendCmd && shmhl_retry_send_msc_command() )
        {
            TX_DEBUG_PRINT(("Drv: MscCommand retry\n"));
            mscCmdInProgress = false;
        }
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
/**********************************************************************/
/* This Function judges that we can access Sii8334 with I2C interface */
/**********************************************************************/
static bool_t shmhl_judge_I2C_accessible(void)
{
	bool_t ret_val = false;
	SiiResultCodes_t lastResultCode;

	TX_DEBUG_PRINT(("shmhl_judge_I2C_access() Start\n"));

	g_chipRevId = SiiRegRead(TX_PAGE_L0 | 0x04);

	lastResultCode = SiiCraGetLastResult();

	if(SII_SUCCESS == lastResultCode) {
		ret_val = true;
	}
	TX_DEBUG_PRINT(("shmhl_judge_I2C_access() End RevId[%02X] ret[%d]\n", (int)g_chipRevId, (int)ret_val));
	return(ret_val);
}

/*********************************************************************/
/* This Function changes RegInstance                                 */
/*      SHMHL_CI2CA_OFF :  Instance=0(default)                       */
/*      SHMHL_CI2CA_ON  :  Instance=1                                */
/*********************************************************************/
static void shmhl_RegInstanceSet_for_CI2CA(uint8_t ci2ca)
{
	TX_DEBUG_PRINT(("shmhl_RegInstanceSet_for_CI2CA() Start\n"));

	SiiRegInstanceSet(TX_PAGE_TPI, ci2ca);
	SiiRegInstanceSet(TX_PAGE_L0, ci2ca);
	SiiRegInstanceSet(TX_PAGE_L1, ci2ca);
	SiiRegInstanceSet(TX_PAGE_2, ci2ca);
	SiiRegInstanceSet(TX_PAGE_3, ci2ca);
	SiiRegInstanceSet(TX_PAGE_CBUS, ci2ca);
	SiiRegInstanceSet(TX_PAGE_DDC_EDID, ci2ca);
	SiiRegInstanceSet(TX_PAGE_DDC_SEGM, ci2ca);

	TX_DEBUG_PRINT(("shmhl_RegInstanceSet_for_CI2CA() End\n"));
}

/*********************************************************************/
/* This Function is based on (int4Status & BIT5) case in Int4Isr().  */
/*********************************************************************/
void shmhl_force_disconnect(void)
{
	TX_DEBUG_PRINT(("shmhl_force_disconnect() Start\n"));

	if ( POWER_STATE_D3 == fwPowerState ) {
		TX_DEBUG_PRINT(("shmhl_force_disconnect() Already Disconnected\n"));
	}
	else {
		MhlTxDrvProcessDisconnection();
	...
	shmhl_notify_si_state(SHMHL_SI_STATE_DISCONNECTED, NULL);

	TX_DEBUG_PRINT(("shmhl_force_disconnect() End\n"));
}
#endif /* CONFIG_SII8334_MHL_TX */
