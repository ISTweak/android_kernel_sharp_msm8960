...
#define SHBATT_BOOT_BATTERY_CURRENT_DEF (-350)
#define SHBATT_BATTERY_BASE_RESISTANCE_DEF (195)
...
#define SHBATT_DRV_IOCTL_CMD_SET_ANDROID_SHUTDOWN_TIMER					_IOR(SHBATT_IOC_MAGIC,  60, int)
#define		SHBATT_DRV_IOCTL_CMD_GET_AVE_CURRENT_AND_AVE_VOLTAGE			_IOR(  SHBATT_IOCTL_MAGIC, 61, shbatt_current_voltage_t)
#define		SHBATT_DRV_IOCTL_CMD_READ_ADC_CHANNELS					_IOR(  SHBATT_IOCTL_MAGIC, 62, shbatt_adc_read_request_t)
#define		SHBATT_DRV_IOCTL_CMD_RESET_CC							_IO(  SHBATT_IOCTL_MAGIC, 63)
...
static const int shbatt_def_resistance_ratio_table[][2] = 
{
	{-20,5500},
	{-10,3600},
	{0,2250},
...
static const int shbatt_def_resistance_ratio_table_size = sizeof(shbatt_def_resistance_ratio_table) / sizeof(shbatt_def_resistance_ratio_table[0]);
...
static int shbatt_linear_convert( int base_value, const int linear_table[][2], int table_size );
...
shbatt_result_t shbatt_api_get_battery_health( shbatt_health_t* hea_p )
{
	...
	do
	{
		...
		result = shbatt_seq_get_battery_health(hea_p);
	}
	while(0);
	SHBATT_TRACE("[E] %s \n",__FUNCTION__);
...
shbatt_result_t shbatt_api_get_calibrate_battery_voltage( int* vol_p )
{
	...
	do
	{
		...
		result = shbatt_seq_get_calibrate_battery_voltage(vol_p);
	}
	while(0);
...
static int shbatt_drv_ioctl_cmd_set_log_enable( struct file* fi_p, unsigned long arg )
{
	...
	if(shbatt_api_set_log_enable( val&0x01 ) != SHBATT_RESULT_SUCCESS) {
...
static shbatt_result_t shbatt_seq_get_battery_technology( shbatt_technology_t* tec_p )
{
	...
	*tec_p = SHBATT_TECHNOLOGY_BATT_LION;
...
static shbatt_result_t shbatt_seq_get_calibrate_battery_voltage( int* vol_p )
{
	...
	int battery_voltage;
	int battery_current;
	int battery_resistance_ratio;
	int battery_resistance;
	
	SHBATT_TRACE("[S] %s \n",__FUNCTION__);
	
	result = shbatt_seq_get_battery_temperature( & battery_temp );
	if( result != SHBATT_RESULT_SUCCESS )
	{
		SHBATT_ERROR("[%s] cannot get battery temp, ret=%d\n",__FUNCTION__, (int)result);
		return result;
	}
	
	result = shbatt_seq_get_battery_voltage( & battery_voltage );
	if( result != SHBATT_RESULT_SUCCESS )
	{
		SHBATT_ERROR("[%s] cannot get battery voltage, ret=%d\n",__FUNCTION__, (int)result);
		return result;
	}
	
	result = shbatt_get_battery_current( & battery_current );
	if( result != SHBATT_RESULT_SUCCESS )
	{
		SHBATT_ERROR("[%s] cannot get battery current, ret=%d\n",__FUNCTION__, (int)result);
		return result;
	}
	
	battery_resistance_ratio = shbatt_linear_convert( battery_temp, shbatt_def_resistance_ratio_table, shbatt_def_resistance_ratio_table_size);
	battery_resistance = SHBATT_BATTERY_BASE_RESISTANCE_DEF * battery_resistance_ratio / 1000;
	
	SHBATT_INFO("[%s] battery_temp              = %d\n",__FUNCTION__, battery_temp);
	SHBATT_INFO("[%s] battery_resistance_ratio  = %d\n",__FUNCTION__, battery_resistance_ratio);
	SHBATT_INFO("[%s] battery_resistance        = %d\n",__FUNCTION__, battery_resistance);
	SHBATT_INFO("[%s] battery_current           = %d\n",__FUNCTION__, battery_current);
	SHBATT_INFO("[%s] battery_voltage           = %d\n",__FUNCTION__, battery_voltage);
	
	*vol_p = battery_voltage - battery_current * battery_resistance / 1000;
	
	SHBATT_INFO("[%s] calibrate_battery_voltage = %d\n",__FUNCTION__, *vol_p);
...
static int shbatt_linear_convert( int base_value, const int linear_table[][2], int table_size )
{
	...
	if( (table_size == 1) || (base_value <= linear_table[0][0]) )
	{
		return linear_table[0][1];
	}
	
	if( linear_table[table_size - 1][0] <= base_value )
	{
		return linear_table[table_size - 1][1];
	}
	
	for( i=0; i < table_size - 1 ; i++ )
	{
		if( (linear_table[i][0] <= base_value) && ( base_value <= linear_table[i+1][0]) )
		{
			int x_length = linear_table[i+1][0] - linear_table[i][0];
			int y_length = linear_table[i+1][1] - linear_table[i][1];
			int x_length_sub = base_value - linear_table[i][0];
			int y_length_sub = 0;
			
			if(x_length != 0)
			{
				y_length_sub = x_length_sub * y_length / x_length;
			}
			
			ret_value = linear_table[i][1] + y_length_sub;
			break;
		}
	}
...
static shbatt_result_t shbatt_seq_initialize( void )
{
	...
	shbatt_usse_pkt.hdr.cmd = SHBATT_CMD_INITIALIZE;
	shbatt_usse_pkt.hdr.ret = SHBATT_RESULT_FAIL;
	shbatt_usse_pkt.prm.val = 0;
...
static ssize_t shbatt_drv_show_battery_property( struct device* dev_p, struct device_attribute* attr_p ,char* buf_p )
{
	...
	switch(property)
	{
		...
		{
#ifdef CONFIG_BATTERY_SH
			size = sprintf(buf_p,"Li-ion\n");
#else
			shbatt_technology_t tec;
			if(shbatt_api_get_battery_technology(&tec) == SHBATT_RESULT_SUCCESS)
			{
				SHBATT_TRACE("/battery/technology = %s\n",technology_text[tec]);
				size = sprintf(buf_p,"%s\n",technology_text[tec]);
			}
#endif
		}
...
static int shbatt_drv_ioctl_cmd_set_android_shutdown_timer( struct file* fi_p, unsigned long arg )
{
	...
	if(val == 1)
	{
printk(KERN_ERR "[P] %s android shutdown timer start \n",__FUNCTION__ );
		cancel_delayed_work(&lowbatt_shutdown_struct);
