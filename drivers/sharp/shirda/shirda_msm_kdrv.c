...
#define SHIRDA_KERNEL_DRIVER_VERSION	"1.04.02"
#define SHIRDA_KERNEL_DRIVER_NAME	SHIRDA_DEVFILE_NAME

#define SHIRDA_CORE_CLK_FREQ	(7372800)

#define SHIRDA_GPIO_TXD		(71)
#define SHIRDA_GPIO_RXD		(72)
#define SHIRDA_GPIO_SD		(121)
...
#define GSBIREG_NO	(10)
#define GSBIREG_BASE		(0x1A200000)
...
#define UART_DM_REGISTER	(0x00040000)
...
#define GSBIREG_HCLK_CTRL_REG	GSBIn_HCLK_CTL_REG(GSBIREG_NO)
...
static int shirda_msm_gpios_request(const struct msm_gpio *table, int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (rc) {
			IRDALOG_ERROR("gpio_request(%d) <%s> failed: %d\n",
			       GPIO_PIN(g->gpio_cfg), g->label ?: "?", rc);
...
static int shirda_gpio_init(void)
{
	...
	ret = gpio_direction_output(SHIRDA_GPIO_SD, SHIRDA_SD_SHUTDOWN);
	if (ret != 0) {
		IRDALOG_FATAL("gpio_direction_output(IRSD) fail errno = %d\n)",
								ret);
		return ret;
	}
...
static int shirda_clk_enable(void)
{
	...
	iface_clk  = clk_get_sys("msm_serial_hs.0", "iface_clk");
	if (IS_ERR(iface_clk)) {
...
static void shirda_set_uartdm_irda_enable(void)
{
	unsigned long lock_flag;
...
static int shirda_open(struct inode *inode, struct file *fp)
{
	...
	shirda_set_uartdm_irda_enable();

	if (shirda_state == SHIRDA_STATE_IDLE) {
