...
/* INTU */
#define D_INTU_GPIO_NO			PM8921_GPIO_PM_TO_SYS(13)
...
#define D_SUCC_OPEN_RETRY_NUM	10
#define D_SUCC_PON_WAIT_USEC	6000
...
static int succ_actNop(void);
static int succ_actBusy(void);
static int succ_actAbnormal(void);
static int succ_actOpenNfc(void);
static int succ_actCloseNfc(void);
static int succ_actOpenFelica(void);
static int succ_actOpenFelicaRetry(void);
static int succ_actCloseFelica(void);
...
{
	gpio_set_value_cansleep(D_HSEL_GPIO_NO, value);
	return 0;
}
...
static long succ_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (copy_from_user(&result, (int __user *)arg, sizeof(result))) {
	...
	if ((result == SHSNFC_UCC_RWS_CHECK) && (mfc_rws_sts == D_RWS_RW_DISABLE)) {
		MFC_DRV_DBG_LOG("rws disable");
		result = SUCC_RETVAL_BUSY;
	} else {
		result = succ_handle_event(event);
	}
...
{
	MFC_DRV_DBG_LOG("");
	succ_handle_event(SUCC_EVENT_END_NFC);
	return 0;
}
...
int succ_handle_event(int event)
{
...
static int succ_actOpenFelicaRetry(void)
{
	...
	for (i = 0 ; i < D_SUCC_OPEN_RETRY_NUM; i++) {
		...
		result = succ_handle_event(SUCC_EVENT_START_FELICA);
