 ...
 *
 * Copyright (C) 2010-2012 SHARP CORPORATION
 *
...
/* RFS */
#define D_RFS_DEVS		(1)
#define D_RFS_GPIO_NO	(49)
#define D_RFS_DEV_NAME	("felica_rfs")

/* INT */
#define D_INT_DEVS		(1)
#define D_INT_GPIO_NO	(46)
...
unsigned int mfc_rws_sts = D_RWS_RW_ENABLE;
 ...
 * function_rws
 */
ssize_t rws_read(struct file *file, char __user * buf,
		      size_t len, loff_t * ppos)
{
	char on[2];
...
ssize_t rws_write(struct file *file, const char __user *data,
		       size_t len, loff_t *ppos)
{
	char on;
