...
#ifdef SHTPSIF_COMMAND
#include <sharp/shtps_dev.h>
#endif	/* SHTPSIF_COMMAND */
...
#define CY_USE_HW_RESET 	0
#define CY_USE_SW_RESET 	1
...
#if defined(CY_USE_FORCE_LOAD) || defined(CONFIG_TOUCHSCREEN_DEBUG) || defined(SHTPSIF_COMMAND)
...
#ifdef ENABLE_JITTER_FILTER
#define P_TIMEOUT_DEFAULT 100
#define P_HIST_NUM_MAX 16
#define P_HIST_NUM_DEFAULT 6
#define P_THRESH_DEFAULT 80
#define DRAG_TD_THRESH_DEFAULT 4
#define DRAG_TU_THRESH_DEFAULT 16
#define DRAG_THRESH_RETURN_TIME 250
#define DRAG_DIR_COUNT_MAX 3
#endif	/* ENABLE_JITTER_FILTER */
...
struct cyttsp4 {
	...
	struct cyttsp4_touch prv_touch[CY_NUM_TCH_ID];
#ifdef ENABLE_JITTER_FILTER
	int pause_touch[2][CY_NUM_TCH_ID];
	bool on_pause[2][CY_NUM_TCH_ID];
	unsigned long p_timeout[CY_NUM_TCH_ID];
	int p_timeout_ms;
	int p_hist[CY_NUM_TCH_ID][P_HIST_NUM_MAX];
	int p_hist_next[CY_NUM_TCH_ID];
	int p_hist_num;
	int p_thresh;
	int drag_thresh[CY_NUM_TCH_ID];
	int drag_td_thresh;
	int drag_tu_thresh;
	unsigned long drag_timeout[2][CY_NUM_TCH_ID];
	int prev_drag_dir[2][CY_NUM_TCH_ID];
	int drag_dir_count[2][CY_NUM_TCH_ID];
#define DRAG_DIR_NONE 0
#define DRAG_DIR_PLUS 1
#define DRAG_DIR_MINUS -1
#endif	/* --ENABLE_JITTER_FILTER */
#ifdef CY_USE_TMA400
	bool starting_up;
#endif /* --CY_USE_TMA400 */
	...
	bool is_charging;
	bool charmor_enabled;
	bool charmor_on;
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
	int spike_touch_count;
#endif
#define SPIKE_TOUCH_COUNT_MAX 3
#ifdef DELAY_CHARGER_ARMOR_OFF
	struct delayed_work charmor_delayed_work;
#endif	/* DELAY_CHARGER_ARMOR_OFF */
	int div_resolution;
	unsigned long sh_hw_revision;
	bool is_calibrating;
...
typedef struct {
	shtpsif_point_t p;		/* Upper left */
	shtpsif_point_t q;		/* Upper right */
	shtpsif_point_t r;		/* Lower left */
	shtpsif_point_t s;		/* Lower right */
} shtpsif_area_t;
...
static wait_queue_head_t shtpsif_wq;
static int shtpsif_event;
static struct hrtimer shtpsif_polling_timer;
static struct work_struct shtpsif_polling_work;

/* Coordinates of six criteria points for adjusting */
static const shtpsif_point_t shtpsif_base_point[ADJUST_POINT] = {
...
static const shtpsif_area_t shtpsif_area_rect[AREA_COUNT] = {
...
static shtpsif_area_t shtpsif_area_diff[AREA_COUNT];
static shtpsif_point_t shtpsif_adjust_param[ADJUST_POINT];
#endif	/* SHTPSIF_COMMAND */

#if defined(CY_AUTO_LOAD_FW) || \
	defined(CY_USE_FORCE_LOAD) || \
	defined(CONFIG_TOUCHSCREEN_DEBUG) || \
	defined(SHTPSIF_COMMAND)
static int _cyttsp4_load_app(struct cyttsp4 *ts, const u8 *fw, int fw_size);
#endif /* CY_AUTO_LOAD_FW || CY_USE_FORCE_LOAD || CONFIG_TOUCHSCREEN_DEBUG || SHTPSIF_COMMAND */
...
static int _cyttsp4_recover_charger_armor(struct cyttsp4 *ts);
static int cyttsp4_set_charger_armor(struct cyttsp4 *ts, bool on);
static int cyttsp4_set_wired_charger_armor(struct cyttsp4 *ts, bool on);
static int cyttsp4_set_wireless_charger_armor(struct cyttsp4 *ts, bool on);
...
#endif /* --CY_USE_TMA884 */
#ifdef SHTPSIF_COMMAND
static void _shtpsif_adjust_point(struct cyttsp4 *ts, int *x, int *y);
static int _shtpsif_poll_start(struct cyttsp4 *ts);
static void _shtpsif_poll_stop(struct cyttsp4 *ts);
static void _shtpsif_poll_scan(struct work_struct *work);
static enum hrtimer_restart _shtpsif_poll_timer_handler(struct hrtimer *timer);
static int shtpsif_hw_reset(struct cyttsp4 *ts);
static int shtpsif_sw_reset(struct cyttsp4 *ts);
static int shtpsif_hw_reset_startup(struct cyttsp4 *ts);
static int shtpsif_sw_reset_startup(struct cyttsp4 *ts);
static int _shtpsif_calibration_idac(struct cyttsp4 *ts);
static int _shtpsif_firmware_version(struct cyttsp4 *ts, u32 *version);
#endif	/* SHTPSIF_COMMAND */
...
static int _cyttsp4_set_mode(struct cyttsp4 *ts, u8 new_mode)
{
	...
	_cyttsp4_recover_charger_armor(ts);
...
static int _cyttsp4_get_sysinfo_regs(struct cyttsp4 *ts)
{
	...
	if (ts->si_ofs.num_btns) {
		...
		if (ts->si_ofs.btn_keys_size <= 0) {
			dev_err(ts->dev, "%s: invalid btn_keys_size %d\n", __func__, ts->si_ofs.btn_keys_size);
			retval = -EINVAL;
			goto _cyttsp4_get_sysinfo_regs_exit_no_handshake;
		}
		if (ts->btn != NULL)
	...
	if (ts->si_ofs.btn_rec_siz * ts->si_ofs.num_btns < 0) {
		dev_err(ts->dev, "%s: invalid btn_rec_siz * num_btns %d\n", __func__, ts->si_ofs.btn_rec_siz * ts->si_ofs.num_btns);
		retval = -EINVAL;
		goto _cyttsp4_get_sysinfo_regs_exit_no_handshake;
	}
...
#ifdef ENABLE_JITTER_FILTER
static int get_diff(int a, int b)
{
	int diff = a - b;
	return (diff >= 0) ? diff : -diff;
}

static int get_direction(int a, int b)
{
	int d = a - b;
	return d > 0 ? DRAG_DIR_PLUS :
		d < 0 ? DRAG_DIR_MINUS :
		DRAG_DIR_NONE;
}

static bool chk_is_dragging(struct cyttsp4 *ts, int xy, int t, int diff, int dir)
{
	if (diff >= ts->drag_thresh[t])
		return true;
	
	if (dir != DRAG_DIR_NONE) {
		if (dir == ts->prev_drag_dir[xy][t]) {
			if (ts->drag_dir_count[xy][t] < DRAG_DIR_COUNT_MAX)
				ts->drag_dir_count[xy][t]++;
		} else {
			ts->drag_dir_count[xy][t] = 1;
		}
		
		ts->prev_drag_dir[xy][t] = dir;
...
static void rec_notify_time(struct cyttsp4 *ts, int xy, int t)
{
	ts->drag_timeout[xy][t] = jiffies + msecs_to_jiffies(DRAG_THRESH_RETURN_TIME);
}

static int chk_notify_time(struct cyttsp4 *ts, int xy, int t)
{
	return time_after(jiffies, ts->drag_timeout[xy][t]);
}

static void rec_p_notify_time(struct cyttsp4 *ts, int t)
{
	ts->p_timeout[t] = jiffies + msecs_to_jiffies(ts->p_timeout_ms);
}

static int chk_p_notify_time(struct cyttsp4 *ts, int t)
{
	return time_after(jiffies, ts->p_timeout[t]);
}

static void set_touch_pause(struct cyttsp4 *ts, struct cyttsp4_touch *touch, int xy, int t)
{
	ts->pause_touch[xy][t] = touch->abs[(xy == 0) ? CY_TCH_X : CY_TCH_Y];
	ts->on_pause[xy][t] = true;
	ts->prev_drag_dir[xy][t] = DRAG_DIR_NONE;
	ts->drag_dir_count[xy][t] = 0;
}

static void set_touch_unpause(struct cyttsp4 *ts, int xy, int t)
{
	ts->on_pause[xy][t] = false;
	ts->prev_drag_dir[xy][t] = DRAG_DIR_NONE;
	ts->drag_dir_count[xy][t] = 0;
}

static void set_drag_thresh_narrow(struct cyttsp4 *ts, int t)
{
	ts->drag_thresh[t] = ts->drag_td_thresh;
}

static void set_drag_thresh_wide(struct cyttsp4 *ts, int t)
{
	ts->drag_thresh[t] = ts->drag_tu_thresh;
}

static bool pass_jitter_filter(struct cyttsp4 *ts, struct cyttsp4_touch *touch, int t)
{
	int xy;
	int abs;
	int diff;
	...
	for (xy = 0; xy <= 1; xy++) {
		abs = xy == 0 ? CY_TCH_X : CY_TCH_Y;
		if (ts->on_pause[xy][t] == true) {
			diff = get_diff(touch->abs[abs], ts->pause_touch[xy][t]);
			if (diff < ts->drag_thresh[t]) {
				touch->abs[abs] = ts->pause_touch[xy][t];
				ret |= false;
			} else {
				rec_notify_time(ts, xy, t);
				ts->on_pause[xy][t] = false;
				ts->prev_drag_dir[xy][t] =
					get_direction(touch->abs[abs], ts->pause_touch[xy][t]);
				ts->drag_dir_count[xy][t] = 1;
				ret |= true;
			}
		} else {
			if (touch->abs[CY_TCH_E] == CY_EV_TOUCHDOWN) {
				set_touch_pause(ts, touch, xy, t);
			} else {
				diff = get_diff(touch->abs[abs], ts->prv_touch[t].abs[abs]);
				dir = get_direction(touch->abs[abs], ts->prv_touch[t].abs[abs]);
				if (chk_is_dragging(ts, xy, t, diff, dir)) {
					rec_notify_time(ts, xy, t);
				} else if (chk_notify_time(ts, xy, t)) {
					set_touch_pause(ts, touch, xy, t);
				}
			}
			ret |= true;
...
static void clear_pressure_history(struct cyttsp4 *ts, int t)
{
	memset(ts->p_hist[t], 0, sizeof(ts->p_hist[t]));
	ts->p_hist_next[t] = 0;
}

static void store_pressure_history(struct cyttsp4 *ts, struct cyttsp4_touch *touch, int t)
{
	ts->p_hist[t][ts->p_hist_next[t]] = touch->abs[CY_TCH_P];
	ts->p_hist_next[t] = (ts->p_hist_next[t] + 1) % ts->p_hist_num;
}

static int get_max_pressure(struct cyttsp4 *ts, int t)
{
	int i;
	int max = 0;

	for (i = 0; i < ts->p_hist_num; i++) {
		if (max < ts->p_hist[t][i])
			max = ts->p_hist[t][i];
	}
...
#ifdef ENABLE_JITTER_FILTER
			touch.abs[CY_TCH_X] /= ts->div_resolution;
			touch.abs[CY_TCH_Y] /= ts->div_resolution;
#endif	/* ENABLE_JITTER_FILTER */			
			...
			if (signal != CY_IGNORE_VALUE) {
				...
				if (touch.abs[CY_TCH_E] == CY_EV_LIFTOFF) {
					...
					dev_dbg(ts->dev,
						...
						touch.abs[CY_TCH_E]);
#ifdef ENABLE_JITTER_FILTER
					clear_pressure_history(ts, t);
					set_drag_thresh_narrow(ts, t);
					set_touch_unpause(ts, 0, t); // X
					set_touch_unpause(ts, 1, t); // Y
#endif	/* ENABLE_JITTER_FILTER */
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
					/* detect spike touch */
					if (ts->prv_touch[t].abs[CY_TCH_E] == CY_EV_TOUCHDOWN) {
						if (ts->spike_touch_count < SPIKE_TOUCH_COUNT_MAX) {
							ts->spike_touch_count++;
						}
					}
#endif
					/* report touch up */
...
#ifdef ENABLE_JITTER_FILTER
					if (touch.abs[CY_TCH_E] == CY_EV_TOUCHDOWN) {
						rec_p_notify_time(ts, t);
					} else {
						if (chk_p_notify_time(ts, t)) {
							set_drag_thresh_narrow(ts, t);
						} else {
							p_max = get_max_pressure(ts, t);
							p_percentage = (p_max == 0) ? 100 :
								100 * touch.abs[CY_TCH_P] / p_max;
							if (p_percentage < ts->p_thresh) {
								set_drag_thresh_wide(ts, t);
								set_touch_pause(ts, &ts->prv_touch[t], 0, t); // X
								set_touch_pause(ts, &ts->prv_touch[t], 1, t); // Y
							}
							store_pressure_history(ts, &touch, t);
						}
					}
					
					if (!pass_jitter_filter(ts, &touch, t))
						goto _cyttsp4_get_mt_touches_pr_tch;
#endif	/* ENABLE_JITTER_FILTER */
...
_cyttsp4_get_mt_touches_pr_tch:
...
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
	if (ts->charmor_enabled == false && ts->spike_touch_count >= SPIKE_TOUCH_COUNT_MAX) {
		ts->charmor_enabled = true;
		_cyttsp4_recover_charger_armor(ts);
	}
#endif
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		t = touch.abs[CY_TCH_T] -
			...
			[(CY_ABS_ID_OST * CY_NUM_ABS_SET) + CY_MIN_OST];
		if (t < CY_NUM_TCH_ID)
			ts->prv_touch[t] = touch;
	}

	return;
}

static int _cyttsp4_force_touchup(struct cyttsp4 *ts)
{
	int i;
	int t;
	...
	if (ts->num_prv_tch > 0) {
		for (i = 0; i < CY_NUM_TCH_ID; i++) {
			if (ts->prv_touch[i].abs[CY_TCH_E] == CY_EV_LIFTOFF)
				continue;
			ts->prv_touch[i].abs[CY_TCH_E] = CY_EV_LIFTOFF;
#ifdef ENABLE_JITTER_FILTER
			clear_pressure_history(ts, i);
			set_drag_thresh_narrow(ts, i);
			set_touch_unpause(ts, 0, i); // X
			set_touch_unpause(ts, 1, i); // Y
#endif	/* --ENABLE_JITTER_FILTER */
			t = ts->prv_touch[i].abs[CY_TCH_T] -
				ts->platform_data->frmwrk->abs
				[(CY_ABS_ID_OST * CY_NUM_ABS_SET) + CY_MIN_OST];
			input_mt_slot(ts->input, t);
			input_mt_report_slot_state(ts->input, MT_TOOL_FINGER, false);
...
static int _cyttsp4_reset_selectable(struct cyttsp4 *ts, int reset_type)
{
	enum cyttsp4_driver_state tmp_state = ts->driver_state;
	int retval = 0;

	if (reset_type == CY_USE_HW_RESET && ts->platform_data->hw_reset) {
...
{
	return _cyttsp4_reset_selectable(ts, CY_USE_HW_RESET);
}
...
int cyttsp4_suspend(void *handle)
{
	...
	if (ts->starting_up) {
		retval = -EBUSY;
		dev_err(ts->dev,
			"%s: Suspend Blocked while starting up\n", __func__);
		return retval;
	}
#endif

#ifdef CALIBRATION_ON_ADJUSTMENT
	if (ts->is_calibrating) {
		retval = -EBUSY;
		dev_err(ts->dev,
			"%s: Suspend Blocked while calibration\n", __func__);
		return retval;
	}
#endif	/* CALIBRATION_ON_ADJUSTMENT */
...
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
			ts->charmor_enabled = false;
			ts->spike_touch_count = 0;
#endif
...
int cyttsp4_resume(void *handle)
{
	...
	case CY_SLEEP_STATE:
		...
		if (ts->current_mode != CY_MODE_OPERATIONAL)
			_cyttsp4_set_mode(ts, CY_OPERATE_MODE);
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
		ts->spike_touch_count = 0;
		/* charger armor off */
		ts->charmor_enabled = false;
...
static int _cyttsp4_boot_loader(struct cyttsp4 *ts, bool *upgraded)
{
	...
	*upgraded = false;
	if (ts->driver_state == CY_SLEEP_STATE) {
		...
		retval = 0;
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
	} else if (ts->sh_hw_revision == 0x00000006) {
		if ((ts->platform_data->fw->ver[CY_BL_VERS_SIZE - 1] & 0xF0) == 0xA0 ||
			(ts->platform_data->fw->ver[CY_BL_VERS_SIZE - 1] & 0xF0) == 0xB0 ||
			(ts->platform_data->fw->ver[CY_BL_VERS_SIZE - 1] & 0xF0) == 0x90) {
			dev_err(ts->dev, "%s: Invalid firmware version\n", __func__);
			retval = 0;
		}
#endif
	} else {
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = cyttsp4_get_charger_armor_status(ts, &on);

	return snprintf(buf, CY_MAX_PRBUF_SIZE,
			"Charger Armor: %s (%s)\n",
			(retval < 0) ? "Unknown" :
			on ? "ON" : "OFF",
			ts->charmor_enabled ? "enabled" : "disabled");
}
...
static void cyttsp4_firmware_cont(const struct firmware *fw, void *context)
{
...
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
	if (ts->sh_hw_revision == 0x00000006) {
		if (header_size == CY_BL_VERS_SIZE &&
			((fw->data[header_size] & 0xF0) == 0xA0 ||
				(fw->data[header_size] & 0xF0) == 0xB0 ||
				(fw->data[header_size] & 0xF0) == 0x90)) {
			dev_err(ts->dev, "%s: Invalid firmware version\n", __func__);
			goto cyttsp4_firmware_cont_release_exit;
		}
	}
	...
	retval = _shtpsif_calibration_idac(ts);
	if (retval < 0)
...
#endif /* CY_USE_FORCE_LOAD || CONFIG_TOUCHSCREEN_DEBUG || SHTPSIF_COMMAND */
	...
	u8 id, u8 *data, u8 *size)
{
	...
	retval = _cyttsp4_write_block_data(ts, ts->si_ofs.cmd_ofs + 1,
		sizeof(cmd_dat) - 1, &(cmd_dat[1]),
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		dev_err(ts->dev, "%s: Failed to write input parameters r=%d\n",
			__func__, retval);
		goto _cyttsp4_get_parameter_exit;
	}
	
	retval = _cyttsp4_put_cmd_wait(ts, ts->si_ofs.cmd_ofs,
		1, &(cmd_dat[0]), CY_HALF_SEC_TMO_MS,
		_cyttsp4_chk_cmd_rdy, NULL,
		ts->platform_data->addr[CY_TCH_ADDR_OFS], true);
	if (retval < 0) {
		dev_err(ts->dev,
...
static int cyttsp4_get_charger_armor_status(struct cyttsp4 *ts, bool *enabled)
{
	...
	if (ts->current_mode == CY_MODE_OPERATIONAL &&
		...
		retval = _cyttsp4_get_parameter(ts, CY_CHARGER_STATUS,
						data, &size);
		if (retval < 0 || size != 1) {
			dev_err(ts->dev,
				"%s: Failed to get charger armor status\n",
				__func__);
...
#ifdef DELAY_CHARGER_ARMOR_OFF
#define DELAY_CHARMOR_MS 15000
static int _cyttsp4_charmor_delay_start(struct cyttsp4 *ts, unsigned long delay_ms)
{
	cancel_delayed_work(&ts->charmor_delayed_work);
	schedule_delayed_work(&ts->charmor_delayed_work, msecs_to_jiffies(delay_ms));
	return 0;
}

static void cyttsp4_delay_recover_charger_armor(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct cyttsp4 *ts = container_of(dw, struct cyttsp4, charmor_delayed_work);
	int retval = -EFAULT;
	const u8 size = 1;
	u8 data;
	int tries = 0;
	...
	mutex_lock(&ts->data_lock);
_cyttsp4_recover_charger_armor_retry:
	if (ts->current_mode == CY_MODE_OPERATIONAL &&
		ts->driver_state == CY_ACTIVE_STATE) {
		if (ts->charmor_enabled != ts->charmor_on) {
			data = ts->charmor_enabled ? 0x01 : 0x00;
			retval = _cyttsp4_set_parameter(ts, CY_CHARGER_STATUS, &data, size);
			if (retval < 0) {
				dev_err(ts->dev, "%s: Failed to set parameter\n", __func__);
				tries++;
				if (tries < 3)
					goto _cyttsp4_recover_charger_armor_retry;
			} else {
				ts->charmor_on = ts->charmor_enabled;
			}
		} else {
			retval = 0;
		}
...
{
#ifdef DELAY_CHARGER_ARMOR_OFF
	unsigned long delay = ts->charmor_enabled ? 0 : DELAY_CHARMOR_MS;
	_cyttsp4_charmor_delay_start(ts, delay);
	return 0;
#else
	int retval = -EFAULT;
	const u8 size = 1;
...
{
	mutex_lock(&ts->data_lock);
	if (on != ts->charmor_enabled) {
		ts->charmor_enabled = on;
		_cyttsp4_recover_charger_armor(ts);
	}
	mutex_unlock(&ts->data_lock);
	return 0;
...
static int cyttsp4_set_wired_charger_armor(struct cyttsp4 *ts, bool on)
{
	ts->is_charging = on;
#if !defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
	return cyttsp4_set_charger_armor(ts, on);
#else
	return 0;
#endif
}
...
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
	if (on) {
		return cyttsp4_set_charger_armor(ts, false);
	} else {
		ts->spike_touch_count = 0;
		return 0;
	}
...
static bool _cyttsp4_read_fwup_flag(void)
{
	sharp_smem_common_type *sharp_smem;
	sharp_smem = sh_smem_get_common_address();
	return sharp_smem->shtps_fwup_flag != 0 ? true : false;
}

static void _cyttsp4_set_fwup_flag_off(void)
{
	sharp_smem_common_type *sharp_smem;
	sharp_smem = sh_smem_get_common_address();
	sharp_smem->shtps_fwup_flag = 0;
	return;
}

#ifdef CY_USE_TMA400
static int _cyttsp4_startup_tma400(struct cyttsp4 *ts, int reset_type)
{
	int tries;
	int retval = 0;
	u8 ic_crc[2];
...
_cyttsp4_startup_tma400_restart:
	...
	ts->current_mode = CY_MODE_BOOTLOADER;
	retval = _cyttsp4_reset_selectable(ts, reset_type);
	if (retval < 0) {
		dev_err(ts->dev,
			"%s: Fail reset device r=%d\n", __func__, retval);
...
#ifdef SHTPSIF_COMMAND
	if (_shtpsif_firmware_version(ts, &version) < 0) {
		...
		if (
			...
			((u8 *)&version)[2] == 0x00 &&
			((u8 *)&version)[1] == 0x00 &&
			((((u8 *)&version)[0] >= 0x04 && ((u8 *)&version)[0] <= 0x09) ||
				(((u8 *)&version)[0] >= 0xA0 && ((u8 *)&version)[0] <= 0xA3))
			) {
			ts->div_resolution = 2;
		} else {
			ts->div_resolution = 1;
...
static int _cyttsp4_startup(struct cyttsp4 *ts)
{
	return _cyttsp4_startup_tma400(ts, CY_USE_HW_RESET);
}
#endif /* --CY_USE_TMA400 */
...
static irqreturn_t cyttsp4_irq(int irq, void *handle)
{
	...
	dev_vdbg(ts->dev,
		"%s: GOT IRQ ps=%d\n", __func__, ts->driver_state);
	if (!(ts->driver_state == CY_CMD_STATE && atomic_read(&ts->waiting_int)))
		mutex_lock(&ts->data_lock);
...
static int cyttsp4_open(struct input_dev *dev)
{
	...
	if (!ts->powered) {
		...
		if (retval == 0) {
			ts->is_calibrating = true;
			if (_shtpsif_calibration_idac(ts) < 0)
				dev_err(ts->dev, "%s: fail calibration\n", __func__);
#ifdef RESET_AFTER_CALIBRATION
			_cyttsp4_startup(ts);
#endif	/* RESET_AFTER_CALIBRATION */
	...
	struct device *dev, int irq, char *name)
{
...
#ifdef SHTPSIF_COMMAND
	g_tpsif_ts = ts;
	init_waitqueue_head(&shtpsif_wq);
	shtpsif_event = 0;
	hrtimer_init(&shtpsif_polling_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	shtpsif_polling_timer.function = _shtpsif_poll_timer_handler;
	INIT_WORK(&shtpsif_polling_work, _shtpsif_poll_scan);
#endif	/* SHTPSIF_COMMAND */
	...
	sharp_smem = sh_smem_get_common_address();
	ts->sh_hw_revision = sharp_smem->sh_hw_revision;
#ifdef TMA443_COMPATIBILITY
	if (ts->sh_hw_revision == 0x00000000 /* SH_HW_REV_ES0 */ ||
		ts->sh_hw_revision == 0x00000005 /* SH_HW_REV_ES1 */) {
	...
	memset(ts->prv_touch, 0, sizeof(ts->prv_touch));
	for (i = 0; i < CY_NUM_TCH_ID; i++) {
		ts->prv_touch[i].abs[CY_TCH_T] = i;
		ts->prv_touch[i].abs[CY_TCH_E] = CY_EV_LIFTOFF;
	}

#ifdef ENABLE_JITTER_FILTER
	ts->drag_td_thresh = DRAG_TD_THRESH_DEFAULT;
	ts->drag_tu_thresh = DRAG_TU_THRESH_DEFAULT;
	for (i = 0; i < CY_NUM_TCH_ID; i++) {
		ts->drag_thresh[i] = ts->drag_td_thresh;
	}
	ts->p_timeout_ms = P_TIMEOUT_DEFAULT;
	ts->p_hist_num = P_HIST_NUM_DEFAULT;
	ts->p_thresh = P_THRESH_DEFAULT;
	memset(ts->p_timeout, 0, sizeof(ts->p_timeout));
	memset(ts->p_hist, 0, sizeof(ts->p_hist));
	memset(ts->p_hist_next, 0, sizeof(ts->p_hist_next));
	memset(ts->pause_touch, 0, sizeof(ts->pause_touch));
	memset(ts->on_pause, 0, sizeof(ts->on_pause));
	memset(ts->drag_timeout, 0, sizeof(ts->drag_timeout));
	memset(ts->prev_drag_dir, 0, sizeof(ts->prev_drag_dir));
	memset(ts->drag_dir_count, 0, sizeof(ts->drag_dir_count));
#endif	/* ENABLE_JITTER_FILTER */
...
#endif
#ifdef DELAY_CHARGER_ARMOR_OFF
	INIT_DELAYED_WORK(&ts->charmor_delayed_work, cyttsp4_delay_recover_charger_armor);
#endif	/* DELAY_CHARGER_ARMOR_OFF */
	atomic_set(&ts->waiting_int, 0);
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_001)
	ts->div_resolution = 2;
...
#endif
	ts->is_calibrating = false;
	wake_lock_init(&ts->wake_lock, WAKE_LOCK_SUSPEND, "shtps_wake_lock");
...
/* power */
#define SHTPSIF_HW_POWER_ON	1
#define SHTPSIF_HW_POWER_OFF	0

#define SHTPSIF_POLL_INTERVAL 100 /* [ms] */

static dev_t shtpsif_dev;
static struct cdev shtpsif_cdev;
static struct class *shtpsif_class;

static struct cyttsp4_scan_data shtpsif_scan_data[2];
static struct cyttsp4_idac_data shtpsif_idac_data;
static int shtpsif_scan_type = TPSIF_SCAN_TYPE_MUTUAL_RAW;
static int shtpsif_polling_loop = 0;
...
static void _shtpsif_adjust_point(struct cyttsp4 *ts, int *x, int *y)
{
	...
	for (i = 0; i < AREA_COUNT; i++) {
		if (shtpsif_area_rect[i].p.x <= *x && shtpsif_area_rect[i].s.x > *x &&
			shtpsif_area_rect[i].p.y <= *y && shtpsif_area_rect[i].s.y > *y)
			break;
	}
	...
	if (i != AREA_COUNT) {
		...
		l_xpq = (((shtpsif_area_diff[i].q.x * DOUBLE_ACCURACY) -
				(shtpsif_area_diff[i].p.x * DOUBLE_ACCURACY)) /
			(shtpsif_area_rect[i].q.x - shtpsif_area_rect[i].p.x)) *
			(*x - shtpsif_area_rect[i].p.x) +
			(shtpsif_area_diff[i].p.x * DOUBLE_ACCURACY);
		l_xrs = (((shtpsif_area_diff[i].s.x * DOUBLE_ACCURACY) -
				(shtpsif_area_diff[i].r.x * DOUBLE_ACCURACY)) /
			(shtpsif_area_rect[i].s.x - shtpsif_area_rect[i].r.x)) *
			(*x - shtpsif_area_rect[i].r.x) +
			(shtpsif_area_diff[i].r.x * DOUBLE_ACCURACY);
		l_x   = ((l_xrs - l_xpq) / (shtpsif_area_rect[i].r.y - shtpsif_area_rect[i].p.y)) *
			(*y - shtpsif_area_rect[i].p.y) + l_xpq;
		l_ypr = (((shtpsif_area_diff[i].r.y * DOUBLE_ACCURACY) -
				(shtpsif_area_diff[i].p.y * DOUBLE_ACCURACY)) /
			(shtpsif_area_rect[i].r.y - shtpsif_area_rect[i].p.y)) *
			(*y - shtpsif_area_rect[i].p.y) +
			(shtpsif_area_diff[i].p.y * DOUBLE_ACCURACY);
		l_yqs = (((shtpsif_area_diff[i].s.y * DOUBLE_ACCURACY) -
				(shtpsif_area_diff[i].q.y * DOUBLE_ACCURACY)) /
			(shtpsif_area_rect[i].s.y - shtpsif_area_rect[i].q.y)) *
			(*y - shtpsif_area_rect[i].q.y) +
			(shtpsif_area_diff[i].q.y * DOUBLE_ACCURACY);
		l_y   = ((l_yqs - l_ypr) / (shtpsif_area_rect[i].q.x - shtpsif_area_rect[i].p.x)) *
			(*x - shtpsif_area_rect[i].p.x) + l_ypr;
...
static void _shtpsif_qsort(shtpsif_qsort_t *table, int top, int end)
{
	int i, j;
	int center;
	shtpsif_qsort_t swap;
	...
	while (1) {
		...
		if (i >= j)
			break;
		memcpy(&swap, &table[i], sizeof(shtpsif_qsort_t));
		memcpy(&table[i], &table[j], sizeof(shtpsif_qsort_t));
		memcpy(&table[j], &swap, sizeof(shtpsif_qsort_t));
		i++;
		j--;
	}
	if (top < i - 1)
		_shtpsif_qsort(table, top, i - 1);
	if (j + 1 < end)
		_shtpsif_qsort(table, j + 1, end);
}

static void _shtpsif_round_value(int *value)
{
	shtpsif_qsort_t table[6];
	int i;
	...
	_shtpsif_qsort(table, 0, 5);
	value[table[0].num] = value[table[1].num];
	value[table[5].num] = value[table[4].num];
}

static int shtpsif_set_adjust_param(struct cyttsp4 *ts, u16 *param)
{
	int i;
	shtpsif_point_t sd[ADJUST_POINT];
	int diff[2][6];
	int retval = 0;
...
#ifdef CALIBRATION_ON_ADJUSTMENT
		mutex_lock(&ts->data_lock);
		dev_info(ts->dev, "%s: start calibration\n", __func__);
	...
	for (i = 0; i < ADJUST_POINT; i++) {
		if (param[i * 2 + 0] > shtpsif_base_point[i].x + POS_LIMIT ||
			param[i * 2 + 0] < shtpsif_base_point[i].x - POS_LIMIT)
			return -EINVAL;
		if (param[i * 2 + 1] > shtpsif_base_point[i].y + POS_LIMIT ||
			param[i * 2 + 1] < shtpsif_base_point[i].y - POS_LIMIT)
			return -EINVAL;
	}

	/* save parameters */
	SET_POINT(shtpsif_adjust_param[0], param[ 0], param[ 1]);
	SET_POINT(shtpsif_adjust_param[1], param[ 2], param[ 3]);
	SET_POINT(shtpsif_adjust_param[2], param[ 4], param[ 5]);
	SET_POINT(shtpsif_adjust_param[3], param[ 6], param[ 7]);
	SET_POINT(shtpsif_adjust_param[4], param[ 8], param[ 9]);
	SET_POINT(shtpsif_adjust_param[5], param[10], param[11]);

#if 0	/* changing the calculation method of diff */
	/* calculate diff value */
	for (i = 0; i < ADJUST_POINT; i++) {
		sd[i].x = (shtpsif_adjust_param[i].x - shtpsif_base_point[i].x) * 3 / 4;
		sd[i].y = (shtpsif_adjust_param[i].y - shtpsif_base_point[i].y) * 3 / 4;
	}
#else
	for (i = 0; i < ADJUST_POINT; i++) {
		diff[0][i] = (shtpsif_adjust_param[i].x - shtpsif_base_point[i].x);
		diff[1][i] = (shtpsif_adjust_param[i].y - shtpsif_base_point[i].y);
	}

	/* truncate the maximum and minimum */
	_shtpsif_round_value(diff[0]); /* X */
	_shtpsif_round_value(diff[1]); /* Y */
	...
	/*                     |-------p-------| |-------q-------| |-------r-------| |-------s-------|*/
	SET_AREA(shtpsif_area_diff[ 0], 0      , 0      , sd[0].x, 0      , 0      , sd[0].y, sd[0].x, sd[0].y);
	SET_AREA(shtpsif_area_diff[ 1], sd[0].x, 0      , sd[1].x, 0      , sd[0].x, sd[0].y, sd[1].x, sd[1].y);
	SET_AREA(shtpsif_area_diff[ 2], sd[1].x, 0      , 0      , 0      , sd[1].x, sd[1].y, 0      , sd[1].y);
	SET_AREA(shtpsif_area_diff[ 3], 0      , sd[0].y, sd[0].x, sd[0].y, 0      , sd[2].y, sd[2].x, sd[2].y);
	SET_AREA(shtpsif_area_diff[ 4], sd[0].x, sd[0].y, sd[1].x, sd[1].y, sd[2].x, sd[2].y, sd[3].x, sd[3].y);
	SET_AREA(shtpsif_area_diff[ 5], sd[1].x, sd[1].y, 0      , sd[1].y, sd[3].x, sd[3].y, 0      , sd[3].y);
	SET_AREA(shtpsif_area_diff[ 6], 0      , sd[2].y, sd[2].x, sd[2].y, 0      , sd[4].y, sd[4].x, sd[4].y);
	SET_AREA(shtpsif_area_diff[ 7], sd[2].x, sd[2].y, sd[3].x, sd[3].y, sd[4].x, sd[4].y, sd[5].x, sd[5].y);
	SET_AREA(shtpsif_area_diff[ 8], sd[3].x, sd[3].y, 0      , sd[3].y, sd[5].x, sd[5].y, 0      , sd[5].y);
	SET_AREA(shtpsif_area_diff[ 9], 0      , sd[4].y, sd[4].x, sd[4].y, 0      , 0      , sd[4].x, 0      );
	SET_AREA(shtpsif_area_diff[10], sd[4].x, sd[4].y, sd[5].x, sd[5].y, sd[4].x, 0      , sd[5].x, 0      );
	SET_AREA(shtpsif_area_diff[11], sd[5].x, sd[5].y, 0      , sd[5].y, sd[5].x, 0      , 0      , 0      );
...
static int shtpsif_hw_power(struct cyttsp4 *ts, int on)
{
	int retval = 0;
...
static int shtpsif_sw_reset(struct cyttsp4 *ts)
{
	...
	mutex_lock(&ts->data_lock);
	retval = _cyttsp4_reset_selectable(ts, CY_USE_SW_RESET);
	mutex_unlock(&ts->data_lock);
...
static int shtpsif_sw_reset_startup(struct cyttsp4 *ts)
{
	...
	mutex_lock(&ts->data_lock);
	retval = _cyttsp4_startup_tma400(ts, CY_USE_SW_RESET);
	mutex_unlock(&ts->data_lock);
...
static int shtpsif_change_mode(struct cyttsp4 *ts, int mode)
{
	int retval = -EINVAL;
...
shtpsif_change_mode_exit:

	mutex_unlock(&ts->data_lock);
	return retval;
}

static int _shtpsif_read_scan_result(struct cyttsp4 *ts, int type, struct cyttsp4_scan_data *data)
{
	int retval = 0;
	...
	if (retval < 0) {
		dev_err(ts->dev, "%s: fail read cmd status"
			" r=%d\n", __func__, retval);
		goto _shtpsif_read_scan_result_error;
	}
	...
	goto _shtpsif_read_scan_result_exit;

_shtpsif_read_scan_result_error:

_shtpsif_read_scan_result_exit:

	return retval;
}

static int _shtpsif_retrieve_panel_scan(struct cyttsp4 *ts, int type)
{
	int retval = 0;
	uint8_t *buf = NULL;
	size_t buf_size;
	...
	if (buf == NULL) {
		...
		retval = -ENOMEM;
		goto _shtpsif_retrieve_panel_scan_exit;
	}
...
_shtpsif_retrieve_panel_scan_exit:
	kfree(buf);
	return retval;
}

static int _shtpsif_execute_panel_scan(struct cyttsp4 *ts)
{
	int retval = 0;
	uint8_t *buf = NULL;
	size_t buf_size;
	...
	if (buf == NULL) {
		dev_err(ts->dev, "%s: Failed to allocate buffer\n", __func__);
		retval = -ENOMEM;
		goto _shtpsif_execute_panel_scan_exit;
	}
...
_shtpsif_execute_panel_scan_exit:
	kfree(buf);
	return retval;
}

static int shtpsif_scan_rawdata(struct cyttsp4 *ts, int type, struct cyttsp4_scan_data *data)
{
	int retval = 0;
	...
	if (ts->current_mode != CY_MODE_CONFIG) {
		...
		if (retval < 0) {
			dev_err(ts->dev, "%s: Failed to switch to config mode"
				" for sysinfo regs\n", __func__);
			goto shtpsif_scan_rawdata_exit;
		}
	}

	/* Execute panel scan */
	retval = _shtpsif_execute_panel_scan(ts);
	if (retval < 0) {
		dev_err(ts->dev, "%s: Fail execute panel scan r=%d\n", __func__, retval);
		shtpsif_polling_loop = 0;
		goto shtpsif_scan_rawdata_op_mode;
	}

	/* Retrieve panel scan */
	retval = _shtpsif_retrieve_panel_scan(ts, type);
	if (retval < 0) {
	...
	/* Read scan result */
	retval = _shtpsif_read_scan_result(ts, type, data);
	if (retval < 0) {
...
shtpsif_scan_rawdata_op_mode:

shtpsif_scan_rawdata_exit:
...
static int _shtpsif_retrieve_data_structure(struct cyttsp4 *ts, int type)
{
	int retval = 0;
	uint8_t *buf = NULL;
	size_t buf_size;
	...
	if (buf == NULL) {
		dev_err(ts->dev, "%s: Failed to allocate buffer\n", __func__);
		retval = -ENOMEM;
		goto _shtpsif_retrieve_data_structure_exit;
	}
...
_shtpsif_retrieve_data_structure_exit:
	kfree(buf);
	return retval;
}

static int _shtpsif_read_idac_data(struct cyttsp4 *ts, int type, struct cyttsp4_idac_data *data)
{
	int retval = 0;
	uint8_t response[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	...
	if (retval < 0) {
		dev_err(ts->dev, "%s: fail read cmd status"
			" r=%d\n", __func__, retval);
		goto _shtpsif_read_idac_data_exit;
	}
...
_shtpsif_read_idac_data_exit:

	return retval;
}

static int shtpsif_scan_idac(struct cyttsp4 *ts, int type, struct cyttsp4_idac_data *data)
{
	int retval = 0;
	...
	if (ts->current_mode != CY_MODE_CONFIG) {
		...
		if (retval < 0) {
			dev_err(ts->dev, "%s: Failed to switch to config mode"
				" for sysinfo regs\n", __func__);
			goto shtpsif_scan_idac_exit;
		}
	}

	/* Retrieve data structure */
	retval = _shtpsif_retrieve_data_structure(ts, type);
	if (retval < 0) {
	...
	/* Read IDAC data */
	retval = _shtpsif_read_idac_data(ts, type, data);
	if (retval < 0) {
...
static int _shtpsif_calibration(struct cyttsp4 *ts, u8 mode)
{
	int retval = 0;
	uint8_t *buf = NULL;
	size_t buf_size;
	...
	if (buf == NULL) {
		dev_err(ts->dev, "%s: Failed to allocate buffer\n", __func__);
		retval = -ENOMEM;
		goto _shtpsif_calibration_exit;
	}
...
static int _shtpsif_calibration_idac(struct cyttsp4 *ts)
{
	...
	if (ts->driver_state != CY_ACTIVE_STATE) {
		...
		dev_err(ts->dev, "%s: Cannot execute calibration"
			" in current state\n", __func__);
		goto _shtpsif_calibration_idac_exit;
	}
	...
	retval = _shtpsif_calibration(ts, TPSIF_CALIB_MODE_MUTUAL_FINE);
	if (retval < 0) {
		dev_err(ts->dev, "%s: Fail mutal calibration r=%d\n", __func__, retval);
		goto _shtpsif_calibration_idac_op_mode;
	}

	retval = _shtpsif_calibration(ts, TPSIF_CALIB_MODE_SELF);
	if (retval < 0) {
		dev_err(ts->dev, "%s: Fail self calibration r=%d\n", __func__, retval);
		goto _shtpsif_calibration_idac_op_mode;
	}

_shtpsif_calibration_idac_op_mode:
...
_shtpsif_calibration_idac_exit:
...
static int _shtpsif_firmware_version(struct cyttsp4 *ts, u32 *version)
{
	...
	if (ts->sysinfo_ptr.cydata == NULL || ts->sysinfo_ptr.pcfg == NULL) {
		retval = -EINVAL;
		goto _shtpsif_firmware_version_exit;
	}
...
_shtpsif_firmware_version_exit:

	return retval;
}

static int _shtpsif_firmware_update(struct cyttsp4 *ts, u8 *data, size_t size)
{
	int retval = 0;
	u8 header_size = 0;
	...
	if ((data == NULL) || (size == 0)) {
		...
		goto _shtpsif_firmware_update_exit;
	}
	...
	if (ts->sh_hw_revision == 0x00000006) {
		if (header_size == CY_BL_VERS_SIZE &&
			((data[header_size] & 0xF0) == 0xA0 ||
				(data[header_size] & 0xF0) == 0xB0 ||
				(data[header_size] & 0xF0) == 0x90)) {
			retval = -EINVAL;
			dev_err(ts->dev, "%s: Invalid firmware version\n", __func__);
			goto _shtpsif_firmware_update_exit;
		}
...
_shtpsif_firmware_update_exit:
	mutex_unlock(&ts->data_lock);
	return retval;
}

static int shtpsif_start_firmware_update(struct cyttsp4 *ts, u8 *data, size_t size)
{
	int retval = -EFAULT;
	enum cyttsp4_driver_state tmp_state;
	...
	if (ts->waiting_for_fw) {
		dev_err(ts->dev, "%s: Driver is already waiting for firmware\n",
			__func__);
		retval = -EALREADY;
		goto shtpsif_start_firmware_update_exit;
	}
	...
	ts->waiting_for_fw = true;
	retval = _shtpsif_firmware_update(ts, data, size);
	ts->waiting_for_fw = false;
	if (retval < 0) {
...
shtpsif_start_firmware_update_exit:
	return retval;
}
...
static int _shtpsif_poll_start(struct cyttsp4 *ts)
{
	...
	hrtimer_cancel(&shtpsif_polling_timer);
	hrtimer_start(&shtpsif_polling_timer, ktime_set(0, SHTPSIF_POLL_INTERVAL * 1000 * 1000), HRTIMER_MODE_REL);

	shtpsif_polling_loop = 1;
...
static void _shtpsif_poll_stop(struct cyttsp4 *ts)
{
	shtpsif_polling_loop = 0;
	shtpsif_event = 0;
	hrtimer_try_to_cancel(&shtpsif_polling_timer);
}
...
static void _shtpsif_poll_scan(struct work_struct *work)
{
	...
	if (shtpsif_scan_type == TPSIF_SCAN_TYPE_MUTUAL_RAW_AND_BASE) {
		shtpsif_scan_rawdata(ts, TPSIF_SCAN_TYPE_MUTUAL_RAW, &shtpsif_scan_data[0]);
		shtpsif_scan_rawdata(ts, TPSIF_SCAN_TYPE_MUTUAL_BASE, &shtpsif_scan_data[1]);
	} else if (shtpsif_scan_type == TPSIF_SCAN_TYPE_SELF_RAW_AND_BASE) {
		shtpsif_scan_rawdata(ts, TPSIF_SCAN_TYPE_SELF_RAW, &shtpsif_scan_data[0]);
		shtpsif_scan_rawdata(ts, TPSIF_SCAN_TYPE_SELF_BASE, &shtpsif_scan_data[1]);
	} else if (shtpsif_scan_type == TPSIF_SCAN_TYPE_IDAC_MUTUAL) {
		shtpsif_scan_idac(ts, 0x00, &shtpsif_idac_data);
	} else if (shtpsif_scan_type == TPSIF_SCAN_TYPE_IDAC_SELF) {
		shtpsif_scan_idac(ts, 0x01, &shtpsif_idac_data);
	} else
		shtpsif_scan_rawdata(ts, shtpsif_scan_type, &shtpsif_scan_data[0]);

	shtpsif_event = 1;
	wake_up_interruptible(&shtpsif_wq);

	if (shtpsif_polling_loop)
		_shtpsif_poll_start(ts);
}

static enum hrtimer_restart _shtpsif_poll_timer_handler(struct hrtimer *timer)
{
	schedule_work(&shtpsif_polling_work);

	return HRTIMER_NORESTART;
}

static int shtpsif_command(struct cyttsp4 *ts, unsigned int cmd, unsigned long arg)
{
	int retval = 0;
	...
	switch (cmd) {
	case TPSIF_POWER_ON:
		retval = shtpsif_hw_power(ts, SHTPSIF_HW_POWER_ON);
		break;
	case TPSIF_POWER_OFF:
		retval = shtpsif_hw_power(ts, SHTPSIF_HW_POWER_OFF);
		break;
	case TPSIF_HWRESET:
		retval = shtpsif_hw_reset(ts);
		break;
	case TPSIF_SWRESET:
		retval = shtpsif_sw_reset(ts);
		break;
	case TPSIF_SLEEP_ON:
#if defined(CONFIG_HAS_EARLYSUSPEND)
		cyttsp4_early_suspend(&ts->early_suspend);
	...
	case TPSIF_FW_VERSION:
		retval = _shtpsif_firmware_version(ts, &version);
		if (retval < 0)
	...
	case TPSIF_FW_UPDATE:
		...
		retval = shtpsif_start_firmware_update(ts, fw, TPSIF_FW_IMG_SIZE);
		kfree(fw);
		break;
	...
	case TPSIF_SWITCH_MODE:
		...
		_shtpsif_poll_stop(ts);
		retval = shtpsif_change_mode(ts, mode);
		break;
	case TPSIF_START_SCAN:
		...
		if (scan_type < 0 || scan_type >= TPSIF_SCAN_TYPE_NUM)
			return -EFAULT;
		_shtpsif_poll_stop(ts);
		shtpsif_scan_type = scan_type;
		retval = _shtpsif_poll_start(ts);
		break;
	case TPSIF_HWRESET_STARTUP:
		retval = shtpsif_hw_reset_startup(ts);
		break;
	case TPSIF_SWRESET_STARTUP:
		retval = shtpsif_sw_reset_startup(ts);
		break;
	case TPSIF_CALIBRATION_PARAM:
		if (argp == NULL) {
			retval = shtpsif_set_adjust_param(ts, NULL);
		} else {
			if (copy_from_user(param, (u16 __user *)argp, sizeof(param))) {
				return -EFAULT;
			}
			retval = shtpsif_set_adjust_param(ts, param);
		}
		break;
	case TPSIF_SET_CHARGER_ARMOR:
		if (copy_from_user(&mode, (int __user *)argp, sizeof(mode)))
			return -EFAULT;
		retval = cyttsp4_set_wired_charger_armor(ts, mode ? true : false);
		break;
	case TPSIF_SET_WIRELESS_CHARGER_ARMOR:
		if (copy_from_user(&mode, (int __user *)argp, sizeof(mode)))
			return -EFAULT;
		retval = cyttsp4_set_wireless_charger_armor(ts, mode ? true : false);
		break;
...
static int shtpsif_open(struct inode *inode, struct file *filp)
{
	return 0;
}

static int shtpsif_release(struct inode *inode, struct file *filp)
{
	return 0;
}

static long shtpsif_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct cyttsp4 *ts = g_tpsif_ts;

	return (long) shtpsif_command(ts, cmd, arg);
}

static ssize_t shtpsif_read(struct file *filp, char *buf, size_t count, loff_t *pos)
{
	struct cyttsp4_scan_data scan_data[2];
	struct cyttsp4_idac_data idac_data;
	ssize_t size;

	wait_event_interruptible(shtpsif_wq, shtpsif_event == 1);

	if (shtpsif_scan_type == TPSIF_SCAN_TYPE_IDAC_MUTUAL ||
		shtpsif_scan_type == TPSIF_SCAN_TYPE_IDAC_SELF) {
		memcpy(&idac_data, &shtpsif_idac_data, sizeof(idac_data));

		if (copy_to_user((u8 __user *)buf, &idac_data, sizeof(idac_data)))
			return -EFAULT;

		size = sizeof(idac_data);
	} else if (shtpsif_scan_type == TPSIF_SCAN_TYPE_MUTUAL_RAW_AND_BASE ||
		shtpsif_scan_type == TPSIF_SCAN_TYPE_SELF_RAW_AND_BASE) {
		memcpy(&scan_data, &shtpsif_scan_data, sizeof(scan_data));
		...
		memcpy(&scan_data[0], &shtpsif_scan_data[0], sizeof(scan_data[0]));
...
static unsigned int shtpsif_poll(struct file *filp, poll_table *wait)
{
	int retval;

	retval = wait_event_interruptible_timeout(shtpsif_wq,
						shtpsif_event == 1,
						msecs_to_jiffies(SHTPSIF_POLL_INTERVAL));
...
static const struct file_operations shtpsif_fops =
{
	.owner		= THIS_MODULE,
	.open		= shtpsif_open,
...
int __init shtpsif_init(void)
{
	...
	ret = alloc_chrdev_region(&shtpsif_dev, 0, 1, TPSIF_DEV_NAME);
	if (!ret) {
		major = MAJOR(shtpsif_dev);
		minor = MINOR(shtpsif_dev);
	} else
		goto shtpsif_err_exit;

	cdev_init(&shtpsif_cdev, &shtpsif_fops);

	shtpsif_cdev.owner = THIS_MODULE;
	shtpsif_cdev.ops = &shtpsif_fops;

	ret = cdev_add(&shtpsif_cdev, shtpsif_dev, 1);
	if (ret)
		goto shtpsif_err_add;

	shtpsif_class = class_create(THIS_MODULE, TPSIF_DEV_NAME);
	if (IS_ERR(shtpsif_class))
		goto shtpsif_err_add;

	device_create(shtpsif_class, NULL,
		shtpsif_dev, &shtpsif_cdev, TPSIF_DEV_NAME);
...
void __exit shtpsif_exit(void)
{
	device_destroy(shtpsif_class, shtpsif_dev);
...
void msm_tps_set_chargerarmor(int nMode)
{
	...
	retval = cyttsp4_set_charger_armor(ts, nMode ? true : false);
	if (retval < 0) {
		dev_err(ts->dev,
			"%s: Failed to set charger armor\n", __func__);
		return;
...
EXPORT_SYMBOL(msm_tps_set_chargerarmor);
