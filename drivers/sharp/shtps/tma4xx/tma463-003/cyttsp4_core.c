	...
	for (xy = 0; xy <= 1; xy++) {
		...
		if (ts->on_pause[xy][t] == true) {
			...
			if (touch->abs[CY_TCH_E] == CY_EV_TOUCHDOWN) {
				ts->pause_touch[xy][t] = touch->abs[abs];
			...
			if (signal != CY_IGNORE_VALUE) {
				...
				if (touch.abs[CY_TCH_E] == CY_EV_LIFTOFF) {
					...
					dev_dbg(ts->dev,
						...
						touch.abs[CY_TCH_E]);
#ifdef ENABLE_JITTER_FILTER
					ts->on_pause[0][t] = ts->on_pause[1][t] = false;
					ts->prev_drag_dir[0][t] = ts->prev_drag_dir[1][t] = DRAG_DIR_NONE;
					ts->drag_dir_count[0][t] = ts->drag_dir_count[1][t] = 0;
#endif	/* ENABLE_JITTER_FILTER */
#if defined(CONFIG_SHTPS_TMA4XX_TMA463_003)
					/* detect spike touch */
...
#ifdef ENABLE_JITTER_FILTER
			ts->on_pause[0][i] = ts->on_pause[1][i] = false;
			ts->prev_drag_dir[0][i] = ts->prev_drag_dir[1][i] = DRAG_DIR_NONE;
			ts->drag_dir_count[0][i] = ts->drag_dir_count[1][i] = 0;
#endif	/* --ENABLE_JITTER_FILTER */
