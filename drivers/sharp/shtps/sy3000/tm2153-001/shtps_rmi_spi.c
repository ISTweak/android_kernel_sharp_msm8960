/* drivers/sharp/shtps/sy3000/tm2153-001/shtps_rmi_spi.c
 *
...
//#elif defined( CONFIG_SHTPS_SY3000_TM2153_004 )
//	#define SHTPS_VKEY_INVALID_AREA_ENABLE
//#endif /* #if defined( CONFIG_SHTPS_SY3000_TM2153_001 ) */

#undef	SHTPS_GPIO_SPI_ENABLE
#define	SHTPS_BTLOADER_VER_ENABLE
#undef	SHTPS_SPICLOCK_CONTROL_ENABLE

/* Firmware version address modification  */
#define SHTPS_CUST_CONFIGID_ENABLE

/* -----------------------------------------------------------------------------------
 */
#if defined( SHTPS_BOOT_FWUPDATE_ENABLE )
	#if defined( CONFIG_SHTPS_SY3000_TM2153_001 )
		#include "shtps_fw_tm2153-001.h"
	#elif defined( CONFIG_SHTPS_SY3000_TM2153_004 )
		#include "shtps_fw_tm2153-004.h"
	#elif defined( CONFIG_SHTPS_SY3000_TM1981_001 )
		#include "shtps_fw_tm1981-001.h"
	#else
...
#if defined( SHTPS_LOG_BASE_ADDRESS )
	#define SHTPS_LOG_DEBUG_2(p)	p
#else
	#define SHTPS_LOG_DEBUG_2(p)
#endif /* defined( SHTPS_LOG_BASE_ADDRESS ) */
 ...
 */
	...
	static int SHTPS_EVENT_POLL_INTERVAL = 			25;
	static int SHTPS_POLL_MIN_INTERVAL =			30000;
	static int SHTPS_POLL_STANDARD_VAL =			30000;
	#else
	...
	#if defined( SHTPS_NOTIFY_INTERVAL_GUARANTEED )
	#define SHTPS_EVENT_POLL_INTERVAL				25
	#define SHTPS_POLL_MIN_INTERVAL					30000
	#define SHTPS_POLL_STANDARD_VAL					30000
	#else
...
struct shtps_rmi_spi {
	...
	u8							low_power_mode_req;
	u16							bt_ver;
#if defined( SHTPS_SPICLOCK_CONTROL_ENABLE )
	int							poll_state;
#endif /* #if defined( SHTPS_SPICLOCK_CONTROL_ENABLE ) */
};
...
#define	SHTPS_SPI_MOSI	93
#define	SHTPS_SPI_MISO	94
#define	SHTPS_SPI_CS	95
#define	SHTPS_SPI_CLK	96
...
#if 0
	result = gpio_request(SHTPS_SPI_CS, "spi_cs2");
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_cs2 gpio_request() error : %d\n", result);
	}
#endif
	result = gpio_tlmm_config(GPIO_CFG(SHTPS_SPI_CS, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_6MA), GPIO_CFG_ENABLE);
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_cs gpio_tlmm_config() error : %d\n", result);
	}

#if 0
	result = gpio_request(SHTPS_SPI_CLK, "spi_clk");
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_clk gpio_request() error : %d\n", result);
	}
#endif
	result = gpio_tlmm_config(GPIO_CFG(SHTPS_SPI_CLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_6MA), GPIO_CFG_ENABLE);
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_clk gpio_tlmm_config() error : %d\n", result);
	}

#if 0
	result = gpio_request(SHTPS_SPI_MOSI, "spi_mosi");
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_mosi gpio_request() error : %d\n", result);
	}
#endif
	result = gpio_tlmm_config(GPIO_CFG(SHTPS_SPI_MOSI, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_6MA), GPIO_CFG_ENABLE);
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_mosi gpio_tlmm_config() error : %d\n", result);
	}

#if 0
	result = gpio_request(SHTPS_SPI_MISO, "spi_miso");
	if(result < 0){
		printk(KERN_DEBUG "[shtps]spi_miso gpio_request() error : %d\n", result);
	}
#endif
	result = gpio_tlmm_config(GPIO_CFG(SHTPS_SPI_MISO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_6MA), GPIO_CFG_ENABLE);
	if(result < 0){
...
static void shtps_rmi_sendbyte(u8 data)
{
	int i;
	for(i = 0;i < 8;i++){
		gpio_set_value(SHTPS_SPI_CLK, 0);
		gpio_set_value(SHTPS_SPI_MOSI, (data >> (7 - i)) & 0x01);
		gpio_set_value(SHTPS_SPI_CLK, 1);
	}
}
...
static u8 shtps_rmi_readbyte(void)
{
	...
	for(i = 0;i < 8;i++){
		gpio_set_value(SHTPS_SPI_CLK, 0);
		data |= (gpio_get_value(SHTPS_SPI_MISO) & 0x01) << (7 - i);
		gpio_set_value(SHTPS_SPI_CLK, 1);
	}
...
static int shtps_rmi_write_multi(struct shtps_rmi_spi *ts, u16 addr, u8* data, int size)
{
	...
	gpio_set_value(SHTPS_SPI_CLK, 1);
	gpio_set_value(SHTPS_SPI_MOSI, 0);

	// CS Low
	gpio_set_value(SHTPS_SPI_CS, 0);
...
{
	SHTPS_LOG_DEBUG( printk(KERN_DEBUG "[shtps] shtps_rmi_write(gpio)\n"); )
	printk(KERN_DEBUG "[shtps]shtps_rmi_write() 0x%04X|0x%02X", addr, data);
...
static int shtps_rmi_read(struct shtps_rmi_spi *ts, u16 addr, u8 *buf, u32 size)
{
	...
	SHTPS_LOG_DEBUG( printk(KERN_DEBUG "[shtps] shtps_rmi_read(gpio)\n"); )
	printk(KERN_DEBUG "[shtps]shtps_rmi_read() 0x%04X\n", addr);
	gpio_set_value(SHTPS_SPI_CLK, 1);
	gpio_set_value(SHTPS_SPI_MOSI, 0);
	...
	for(i = 0;i < size;i++){
		...
		printk(KERN_DEBUG "[shtps]shtps_rmi_readb() data[%d] = 0x%02X\n", i, buf[i]);
	}
	...
	for(page = 0;page < 2;page++){
		...
		for(i = 0xE9;i > 0x0a;i-=sizeof(pdt)){
			...
			case 0x01:
				...
				SHTPS_LOG_DEBUG_2(
					printk(KERN_DEBUG "[shtps]F01 Base Address\n");
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
					printk(KERN_DEBUG "[shtps]Query Base   : 0x%04x\n", ts->map.fn01.queryBase);
					printk(KERN_DEBUG "[shtps]Ctrl Base    : 0x%04x\n", ts->map.fn01.ctrlBase);
					printk(KERN_DEBUG "[shtps]Data Base    : 0x%04x\n", ts->map.fn01.dataBase);
					printk(KERN_DEBUG "[shtps]Command Base : 0x%04x\n", ts->map.fn01.commandBase);
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
				);
			...
			case 0x05:
				...
				SHTPS_LOG_DEBUG_2(
					printk(KERN_DEBUG "[shtps]F05 Base Address\n");
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
					printk(KERN_DEBUG "[shtps]Query Base   : 0x%04x\n", ts->map.fn05.queryBase);
					printk(KERN_DEBUG "[shtps]Ctrl Base    : 0x%04x\n", ts->map.fn05.ctrlBase);
					printk(KERN_DEBUG "[shtps]Data Base    : 0x%04x\n", ts->map.fn05.dataBase);
					printk(KERN_DEBUG "[shtps]Command Base : 0x%04x\n", ts->map.fn05.commandBase);
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
				);
			...
			case 0x11:
				...
				SHTPS_LOG_DEBUG_2(
					printk(KERN_DEBUG "[shtps]F11 Base Address\n");
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
					printk(KERN_DEBUG "[shtps]Query Base   : 0x%04x\n", ts->map.fn11.queryBase);
					printk(KERN_DEBUG "[shtps]Ctrl Base    : 0x%04x\n", ts->map.fn11.ctrlBase);
					printk(KERN_DEBUG "[shtps]Data Base    : 0x%04x\n", ts->map.fn11.dataBase);
					printk(KERN_DEBUG "[shtps]Command Base : 0x%04x\n", ts->map.fn11.commandBase);
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
				);
			...
			case 0x34:
				...
				SHTPS_LOG_DEBUG_2(
					printk(KERN_DEBUG "[shtps]F34 Base Address\n");
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
					printk(KERN_DEBUG "[shtps]Query Base   : 0x%04x\n", ts->map.fn34.queryBase);
					printk(KERN_DEBUG "[shtps]Data Base    : 0x%04x\n", ts->map.fn34.dataBase);
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
				);
			...
			case 0x54:
				...
				SHTPS_LOG_DEBUG_2(
					printk(KERN_DEBUG "[shtps]F54 Base Address\n");
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
					printk(KERN_DEBUG "[shtps]Query Base   : 0x%04x\n", ts->map.fn54.queryBase);
					printk(KERN_DEBUG "[shtps]Ctrl Base    : 0x%04x\n", ts->map.fn54.ctrlBase);
					printk(KERN_DEBUG "[shtps]Data Base    : 0x%04x\n", ts->map.fn54.dataBase);
					printk(KERN_DEBUG "[shtps]Command Base : 0x%04x\n", ts->map.fn54.commandBase);
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
				);
			...
			case 0x19:
				...
				SHTPS_LOG_DEBUG_2(
					printk(KERN_DEBUG "[shtps]F19 Base Address\n");
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
					printk(KERN_DEBUG "[shtps]Query Base   : 0x%04x\n", ts->map.fn19.queryBase);
					printk(KERN_DEBUG "[shtps]Ctrl Base    : 0x%04x\n", ts->map.fn19.ctrlBase);
					printk(KERN_DEBUG "[shtps]Data Base    : 0x%04x\n", ts->map.fn19.dataBase);
					printk(KERN_DEBUG "[shtps]Command Base : 0x%04x\n", ts->map.fn19.commandBase);
					printk(KERN_DEBUG "[shtps]-------------------------------------------------\n");
				);
...
#if defined( SHTPS_CUST_CONFIGID_ENABLE )
static u16 shtps_data_atoi(u8* ver, u16 len)
{
	int count;
	u16 result = 0x00;
	...
	for(count=0;count<len;count++)
	{
		switch(ver[count])
		{
			...
			case '8':
			case '9':
				result |= (ver[count] - 0x30) << ((len - 1 - count) * len);
				break;
			case 'A':
			case 'B':
			case 'C':
			...
			case 'E':
			case 'F':
				result |= (ver[count] - 0x37) << ((len - 1 - count) * len);
				break;
			default:
				SHTPS_LOG_ERROR( printk(KERN_DEBUG "[shtps]%s ERROR(%c)\n", __func__, ver[count]); );
				return 0;
		}
	}
...
#endif	/* defined( SHTPS_CUST_CONFIGID_ENABLE ) */
...
static u16 shtps_fwver(struct shtps_rmi_spi *ts)
{
	...
	do{
		shtps_rmi_read(ts, 0x58, ver, 4);
		if(ver[0] != 0x00 || ver[1] != 0x00 || ver[2] != 0x00 || ver[3] != 0x00){
			break;
		}
	}while(retry-- > 0);
	
	ver_bin = shtps_data_atoi(ver, 4);
	
	printk(KERN_DEBUG "[shtps]%s(0x%04x)\n", __func__, ver_bin);
...
static int shtps_init_param(struct shtps_rmi_spi *ts)
{
	...
	if(ts->map.fn11.enable){
		rc = shtps_rmi_write(ts, ts->map.fn11.ctrlBase, 0x0B);
		SPI_ERR_CHECK(rc, err_exit);
...
static void shtps_calc_notify(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info, u8 *event)
{
	...
	*event = 0xff;
	...
	SHTPS_LOG_DEBUG( printk(KERN_DEBUG "----------------------------------------\n"); );
	for(i = 0;i < fingerMax;i++){
		...
		SHTPS_LOG_DEBUG( printk(KERN_DEBUG "[shtps]%d|%d|%d|%d|%d|%d\n", info->finger_num,
																		 info->fingers[i].x,
																		 info->fingers[i].y,
																		 info->fingers[i].wx,
																		 info->fingers[i].wy,
	...
	if(ts->diag.tm_mode == SHTPS_FWTESTMODE_V01){
		...
		if(mode == SHTPS_TMMODE_FRAMELINE){
			...
			for(i = 0;i < trans_num;i++){
				...
				for(j = 0;j < receive_num;j++){
					if( (tmp[j] & 0x80) == 0 ){
						ts->diag.tm_data[(j * trans_num * 2) + (i * 2)]     = tmp[j];
						ts->diag.tm_data[(j * trans_num * 2) + (i * 2) + 1] = 0x00;
					}else{
						ts->diag.tm_data[(j * trans_num * 2) + (i * 2)]     = tmp[j];
						ts->diag.tm_data[(j * trans_num * 2) + (i * 2) + 1] = 0xFF;
					}
				}
			}
			mutex_unlock(&shtps_ctrl_lock);
...
#if 0	/* test */
	irq_set_irq_type(ts->irq_mgr.irq, IRQ_TYPE_EDGE_FALLING);
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	default:
			...
			(ts->poll_info.stop_count < ts->poll_info.stop_margin))
		{
			...
			shtps_polling_stop(ts);
			ts->poll_state = 0;
			if(ts->finger_state[0] == 0 && ts->finger_state[1] == 0){
				sh_spi_tps_standby(ts->spi);
			}
		}
#else
...
static int shtps_fwupdate_hwid_enable(void)
{
	...
	if(hw_type == 0x0000){
		if( (hw_revision == 0x0000) || (hw_revision == 0x0002) ){
			ret = 0;
		}else{
			ret = 1;
		}
...
#if defined( SHTPS_BTLOADER_VER_ENABLE )
	shtps_rmi_read(ts, 0x00A8, ver, 2);
	if( (ver[0] == 0) && (ver[1] == 0) ){
		shtps_rmi_read(ts, 0x00A3, ver, 2);
	}
...
static int shtps_ioctl_set_pollinginterval(struct shtps_rmi_spi *ts, unsigned long arg)
{
	...
	if((int)arg < 0xFFFF){
		#if defined( SHTPS_NOTIFY_INTERVAL_GUARANTEED )
		SHTPS_POLL_STANDARD_VAL = (int)(arg * 1000);
		SHTPS_POLL_MIN_INTERVAL = (int)(arg * 1000);
		SHTPS_EVENT_POLL_INTERVAL = (int)(arg - 5);
		#else
...
#endif /* #if defined( SHTPS_DEBUG_VARIABLE_DEFINES ) */
	#if defined( SHTPS_NOTIFY_INTERVAL_GUARANTEED )
	return (SHTPS_POLL_STANDARD_VAL / 1000);
	#else
...
static int shtps_ioctl_set_low_power_mode(struct shtps_rmi_spi *ts, unsigned long arg)
{
	...
	if( (int)arg == 0 ){
		...
		ts->low_power_mode_req = 0;
	}else{
		shtps_rmi_write(ts, ts->map.fn01.ctrlBase, val & ~0x04);
		ts->low_power_mode_req = 1;
	}
...
static int shtps_ioctl_set_thinout_evmode(struct shtps_rmi_spi *ts, unsigned long arg)
{
	return 0;
}
...
static long shtpsif_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TPSDEV_SET_THINOUT_EVMODE:		rc = shtps_ioctl_set_thinout_evmode(ts, arg);	break;
	default:
		_log_msg_sync( LOGMSG_ID__DEVICE_IOCTL_FAIL, "");
		rc = -ENOIOCTLCMD;
		break;
...
#if defined( SHTPS_BOOT_FWUPDATE_ENABLE )
	if(shtps_touchpanel_enable(ts) && shtps_fwupdate_hwid_enable()){
		u8 buf;
...
static int __devinit shtps_rmi_probe(struct spi_device *spi)
{
	...
	if(shtps_irq_resuest(ts)){
		...
		if(result < 0){
			SHTPS_LOG_DEBUG( printk(KERN_DEBUG "[shtps] spi_setup fail\n"); );
		}
	#endif /* #if defined( SHTPS_GPIO_SPI_ENABLE ) */
	...
	if(input_dev->mt == NULL){
		SHTPS_LOG_DEBUG( printk(KERN_DEBUG "[shtps]input_mt_init_slots() ERROR mt == NULL!\n"); );
