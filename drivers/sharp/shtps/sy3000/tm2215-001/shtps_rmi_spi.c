/* drivers/sharp/shtps/sy3000/tm2215-001/shtps_rmi_spi.c
 *
...
#define SHTPS_THINOUT_EVENT_MODE_ENABLE
 ...
 */
	...
	#include "shtps_fw_tm2187-001.h"

#elif	defined( CONFIG_SHTPS_SY3000_TM2215_001 )
	#include "shtps_fw_tm2215-001.h"
...
#define SHTPS_POS_SCALE_Y(ts)	(((CONFIG_SHTPS_SY3000_PANEL_SIZE_Y) * 10000) / ts->map.fn11.ctrl.maxYPosition)
	...
	#if defined( SHTPS_DEBUG_VARIABLE_DEFINES )
		static int SHTPS_CHARGER_ARMOR_STRENGTH_DEFAULT =	0x03;
		static int SHTPS_CHARGER_ARMOR_STRENGTH =			0x10;
	...
	#else
		#define SHTPS_CHARGER_ARMOR_STRENGTH_DEFAULT		0x03
		#define SHTPS_CHARGER_ARMOR_STRENGTH				0x10
	#endif /* #if defined( SHTPS_DEBUG_VARIABLE_DEFINES ) */
...
struct shtps_rmi_spi {
	...
	#if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE )
		u8							thinout_event_mode;
		u8							drop_event;
		struct shtps_touch_info		drop_info;
	#endif /* #if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE ) */
...
{
#if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE )
	if(*event == 0xFF || type == 0xFF || type == SHTPS_EVENT_TU){
		*event = type;
	}else{
		if(*event == SHTPS_EVENT_DRAG){
			*event = type;
		}
	}
#else
...
#if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE )
	if(ts->thinout_event_mode){
		if(*event == SHTPS_EVENT_DRAG){
			if(!ts->drop_event){
				SHTPS_LOG_DBG_PRINT("[shtps][test] set drop_event flag\n");
				*event = 0xFF;
				ts->drop_event = 1;
				memcpy(&ts->drop_info, info, sizeof(ts->drop_info));
				
				for(i = 0;i < fingerMax;i++){
					if(info->fingers[i].state == 0x01){
						SHTPS_LOG_EVENT( printk(KERN_DEBUG "[shtps]Drop event[%d] touch=100(%d), x=%d(%d), y=%d(%d) w=%d(%d,%d)\n",
								i,
								info->fingers[i].z,
								info->fingers[i].x * SHTPS_POS_SCALE_X(ts) / 10000, info->fingers[i].x,
								info->fingers[i].y * SHTPS_POS_SCALE_Y(ts) / 10000, info->fingers[i].y,
								shtps_get_fingerwidth(ts, i, info), info->fingers[i].wx, info->fingers[i].wy); );
					}
				}
			}else{
				SHTPS_LOG_DBG_PRINT("[shtps][test] clear drop_event flag\n");
				ts->drop_event = 0;
			}
		}else if(*event == SHTPS_EVENT_TU){
			if(ts->drop_event){
				SHTPS_LOG_DBG_PRINT("[shtps][test] notify last drop event !!\n");
				shtps_event_report(ts, &ts->drop_info, SHTPS_EVENT_DRAG);
				ts->drop_event = 0;
			}
		}
	}
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	#if defined(SHTPS_MAXFINGER_5_ENABLE) && !defined(SHTPS_SPI_BLOCKREAD_ENABLE)
...
#if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE )
	if(arg){
		ts->thinout_event_mode = 1;
		ts->drop_event         = 0;
	}else{
		ts->thinout_event_mode = 0;
		ts->drop_event         = 0;
	}
#endif /* #if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE ) */
	return 0;
...
static int __devinit shtps_rmi_probe(struct spi_device *spi)
{
...
#if defined( SHTPS_ASYNC_OPEN_ENABLE )
	ts->workqueue_p = alloc_workqueue("TPS_WORK", WQ_HIGHPRI | WQ_CPU_INTENSIVE, 1);
	...
	ts->thinout_event_mode = 0;
	ts->drop_event = 0;
	memset(&ts->drop_info, 0, sizeof(ts->drop_info));
#endif /* #if defined( SHTPS_THINOUT_EVENT_MODE_ENABLE ) */
