...
#define SHCHG_DRV_IOCTL_CMD_PULL_USSE_PACKET _IOR(SHCHG_IOC_MAGIC, 0, shchg_usse_packet_t)
#define SHCHG_DRV_IOCTL_CMD_DONE_USSE_PACKET _IOW(SHCHG_IOC_MAGIC, 1, shchg_usse_packet_t)
#define SHCHG_DRV_IOCTL_CMD_INVALID _IO(SHCHG_IOC_MAGIC, 2)
#define SHCHG_DRV_IOCTL_CMD_GET_PARAMETER_LEVEL _IOWR(SHCHG_IOC_MAGIC, 3, shchg_prm_level_info_t)
#define SHCHG_DRV_IOCTL_CMD_WRITE_PMIC_REGISTER _IOW(SHCHG_IOC_MAGIC, 4, shchg_pm_reg_info_t)
#define SHCHG_DRV_IOCTL_CMD_READ_PMIC_REGISTER _IOWR(SHCHG_IOC_MAGIC, 5, shchg_pm_reg_info_t)
#define SHCHG_DRV_IOCTL_CMD_SET_TIMER _IOW(SHCHG_IOC_MAGIC, 6, shchg_poll_timer_info_t)
#define SHCHG_DRV_IOCTL_CMD_CLR_TIMER _IOW(SHCHG_IOC_MAGIC, 7, shchg_poll_timer_info_t)
#define SHCHG_DRV_IOCTL_CMD_SET_CHARGER_VMAXSEL _IOW(SHCHG_IOC_MAGIC, 8, int)
#define SHCHG_DRV_IOCTL_CMD_GET_CHARGER_VMAXSEL _IOR(SHCHG_IOC_MAGIC, 9, int)
#define SHCHG_DRV_IOCTL_CMD_SET_CHARGER_IMAXSEL _IOW(SHCHG_IOC_MAGIC, 10, int)
#define SHCHG_DRV_IOCTL_CMD_GET_CHARGER_IMAXSEL _IOR(SHCHG_IOC_MAGIC, 11, int)
#define SHCHG_DRV_IOCTL_CMD_SET_CHARGER_CIN_LIMIT _IOW(SHCHG_IOC_MAGIC, 12, int)
#define SHCHG_DRV_IOCTL_CMD_GET_CHARGER_CIN_LIMIT _IOR(SHCHG_IOC_MAGIC, 13, int)
#define SHCHG_DRV_IOCTL_CMD_SET_CHARGER_TRANSISTOR_SWITCH _IOW(SHCHG_IOC_MAGIC, 14, shchg_pm_sw_t)
#define SHCHG_DRV_IOCTL_CMD_SET_CRADLE_CB_REGIST _IO(SHCHG_IOC_MAGIC, 15)
#define SHCHG_DRV_IOCTL_CMD_GET_CRADLE_STATUS _IOR(SHCHG_IOC_MAGIC, 16, shchg_cradle_status_t)
#define SHCHG_DRV_IOCTL_CMD_SET_VBSW_REG _IOW(SHCHG_IOC_MAGIC, 17, int)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_USB_CHARGER_CONNECTED _IOW(SHCHG_IOC_MAGIC, 18, shchg_device_t)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_USB_CHARGER_DISCONNECTED _IO(SHCHG_IOC_MAGIC, 19)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_CRADLE_CONNECTED _IO(SHCHG_IOC_MAGIC, 20)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_CRADLE_DISCONNECTED _IO(SHCHG_IOC_MAGIC, 21)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_USB_CHARGER_I_IS_AVAILABLE _IOW(SHCHG_IOC_MAGIC, 22, int)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_USB_CHARGER_I_IS_NOT_AVAILABLE _IO(SHCHG_IOC_MAGIC, 23)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_BATTERY_DISCONNECTED _IO(SHCHG_IOC_MAGIC, 24)
#define SHCHG_DRV_IOCTL_CMD_NOTIFY_CHARGING_STATE_MACHINE_EXPIRE _IO(SHCHG_IOC_MAGIC, 25)
#define SHCHG_DRV_IOCTL_CMD_CHECK_CHARGER_DISCONNECTED _IO(SHCHG_IOC_MAGIC, 26)
#define SHCHG_DRV_IOCTL_CMD_INITIALIZE _IO(SHCHG_IOC_MAGIC, 27)
#define SHCHG_DRV_IOCTL_CMD_GET_KERNEL_TIME _IOR(SHCHG_IOC_MAGIC, 28, struct timeval)
#define SHCHG_DRV_IOCTL_CMD_SET_CHARGER_VINMIN _IOW(SHCHG_IOC_MAGIC, 29, int)
#define SHCHG_DRV_IOCTL_CMD_GET_CHARGER_VINMIN _IOR(SHCHG_IOC_MAGIC, 30, int)
...
static shchg_device_t shchg_cur_boot_time_cable_status = SHCHG_DEVICE_INVALID;
...
#define NO_IO_ACCESS
#ifndef NO_IO_ACCESS
static shchg_result_t shchg_io_write_pmic_reg( unsigned char mod,
																							 unsigned char reg,
																							 unsigned char* buf_p,
																							 unsigned char len );

static shchg_result_t shchg_io_read_pmic_reg( unsigned char mod,
																							unsigned char reg,
																							unsigned char* buf_p,
																							unsigned char len );
#else
#define shchg_io_write_pmic_reg(...)	SHCHG_RESULT_SUCCESS
#define shchg_io_read_pmic_reg(...)	SHCHG_RESULT_SUCCESS
#endif
...
static int shchg_drv_ioctl_cmd_write_pmic_register( struct file* fi_p, unsigned long arg );

static int shchg_drv_ioctl_cmd_read_pmic_register( struct file* fi_p, unsigned long arg );
...
static struct platform_driver shchg_platform_driver = 
{
	...
	.driver   = { .name = "shchg", .owner = THIS_MODULE, },
};
...
shchg_result_t shchg_api_notify_usb_charger_disconnected( void )
{
	...
	if(shchg_task_is_initialized == false)
	{
		shchg_cur_boot_time_cable_status = SHCHG_DEVICE_INVALID;
...
static shchg_result_t shchg_seq_notify_usb_charger_connected( shchg_device_t dev )
{
	...
	if(pm8921_charger_charge_dis(false) != 0)
	{
		return SHCHG_RESULT_REJECTED;
	}
...
static shchg_result_t shchg_seq_notify_usb_charger_disconnected( void )
{
	...
	if ((shchg_charger_connect & ~SHCHG_CHARGER_CONNECT_USB_AC) == 0)
	{
		if(pm8921_charger_charge_dis(true) != 0)
		{
			return SHCHG_RESULT_REJECTED;
		}
	}
...
static shchg_result_t shchg_seq_notify_cradle_disconnected( void )
{
	...
	if ((shchg_charger_connect & ~SHCHG_CHARGER_CONNECT_CRADLE) == 0)
	{
		if(pm8921_charger_charge_dis(true) != 0)
		{
			return SHCHG_RESULT_REJECTED;
...
static shchg_result_t shchg_seq_initialize( void )
{
	...
	if(shchg_cur_boot_time_cable_status != SHCHG_DEVICE_INVALID)
	{
...
static int shchg_seq_get_parameter_level( shchg_get_prm_t prm )
{
	...
	switch(prm)
	{
		case SHCHG_GET_PRM_BATTERY_VOLT:
			...
			{
				shbatt_api_get_fuelgauge_voltage(&vbatt);
		...
		case SHCHG_GET_PRM_BATTERY_I:
			...
			if(shbatt_api_get_fuelgauge_current(&ibatt) == SHBATT_RESULT_SUCCESS)
			{
				level_val = ibatt;
			}
			else
		...
		case SHCHG_GET_PRM_CHG_THERM_DEGC:
			...
			/*adc.channel = SHBATT_ADC_CHANNEL_CHG_TEMP;*/	/*chg therm can't used.*/
			adc.channel = SHBATT_ADC_CHANNEL_XO_THERM;
		...
		case SHCHG_GET_PRM_CAMERA_THERM_DEGC:
			...
			adc.channel = SHBATT_ADC_CHANNEL_CAM_TEMP;
		...
		case SHCHG_GET_PRM_CHG_THERM_DEGC_BUF:
			...
			/*adc.channel = SHBATT_ADC_CHANNEL_CHG_TEMP;*/	/*chg_temp can't used.*/
			adc.channel = SHBATT_ADC_CHANNEL_XO_THERM;
																							 ...
																							 unsigned char len )
{
	...
	tx_p = (unsigned char*)kzalloc(len + 1,GFP_KERNEL);

	if((buf_p != NULL) && (tx_p != NULL))
	{
		memcpy(&tx_p[1],buf_p,len);

		for(retry = 0; retry <= 10; retry++)
		{
			if(twl_i2c_write(mod,tx_p,reg,len) >= 0)
			{
																							...
																							unsigned char len )
{
	...
	if(buf_p != NULL)
	{
		for(retry = 0; retry <= 10; retry++)
		{
			if(twl_i2c_read(mod,buf_p,reg,len) >= 0)
			{
				result = SHCHG_RESULT_SUCCESS;
				break;
			}
...
static shchg_result_t shchg_pm_set_charger_vmaxsel( int mv )
{
	...
	if (rc) {
		...
		SHCHG_ERROR("error pm8921_charger_vddmax_set = %d, rc = %d\n", val, rc);
	}
...
static shchg_result_t shchg_pm_set_charger_imaxsel( int ma )
{
	...
	do
	{
		...
		{
			result = SHBATT_RESULT_FAIL;
			SHCHG_ERROR("error pm8921_charger_ibatmax_set = %d, rc = %d\n", val, rc);
			break;
		}
...
static shchg_result_t shchg_pm_set_charger_cin_limit( int cl )
{
	...
	do
	{
		...
		rc = pm8921_charger_iusbmax_set( cl );
		if (rc)
		{
			result = SHBATT_RESULT_FAIL;
			SHCHG_ERROR("error pm8921_charger_iusbmax_set = %d, rc = %d\n", cl, rc);
			break;
		}
...
static shchg_result_t shchg_pm_set_charger_vinmin( int vol )
{
	...
	do
	{
		rc = pm8921_charger_vinmin_set(vol);
		if(rc != 0)
		{
			result = SHBATT_RESULT_FAIL;
			SHCHG_ERROR("error pm8921_charger_vinmin_set vol=%d, rc=%d\n", vol, rc);
			break;
		}
...
static shchg_result_t shchg_pm_get_charger_vinmin( int* vol )
{
	...
	do
	{
		rc = pm8921_charger_vinmin_get(vol);
		if(rc != 0)
		{
			result = SHBATT_RESULT_FAIL;
...
static long shchg_drv_ioctl( struct file* fi_p, unsigned int cmd, unsigned long arg )
{
	...
	else if(cmd == SHCHG_DRV_IOCTL_CMD_WRITE_PMIC_REGISTER)
	{
		ret = shchg_drv_ioctl_cmd_write_pmic_register(fi_p,arg);
	}
	else if(cmd == SHCHG_DRV_IOCTL_CMD_READ_PMIC_REGISTER)
	{
		ret = shchg_drv_ioctl_cmd_read_pmic_register(fi_p,arg);
	}
...
static int shchg_drv_ioctl_cmd_write_pmic_register( struct file* fi_p, unsigned long arg )
{
	shchg_pm_reg_info_t pri;

	unsigned char mod, reg, len;
	...
	if(copy_from_user(&pri,(shchg_pm_reg_info_t*)arg,sizeof(shchg_pm_reg_info_t)) != 0)
	{
		SHCHG_ERROR("%s : copy_from_user failed.\n",__FUNCTION__);
		ret = -EPERM;
	}
	...
	{
		mod = (unsigned char)pri.mod;
		reg = (unsigned char)pri.reg;
		len = (unsigned char)pri.len;

		buf_p = pri.buf;

		if(shchg_io_write_pmic_reg(mod,reg,buf_p,len) != SHCHG_RESULT_SUCCESS)
		{
			SHCHG_ERROR("%s : shchg_io_write_pmic_reg failed.\n",__FUNCTION__);
			ret = -EPERM;
		}
	}
...
static int shchg_drv_ioctl_cmd_read_pmic_register( struct file* fi_p, unsigned long arg )
{
	...
	else
	{
		...
		if(shchg_io_read_pmic_reg(mod,reg,buf_p,len) != SHCHG_RESULT_SUCCESS)
		{
			SHCHG_ERROR("%s : shchg_io_read_pmic_reg failed.\n",__FUNCTION__);
			ret = -EPERM;
		}
		else
		{
			if(copy_to_user((shchg_pm_reg_info_t*)arg,&pri,sizeof(shchg_pm_reg_info_t)) != 0)
			{
				SHCHG_ERROR("%s : copy_to_user failed.\n",__FUNCTION__);
				ret = -EPERM;
			}
...
static int shchg_drv_ioctl_cmd_set_vbsw_reg( struct file* fi_p, unsigned long arg )
{
	...
	{
#ifdef __SUPPORT_CRADLE__
		ret = (shswic_write_vbsw_reg(vbsw) == SHSWIC_SUCCESS) ? 0 : -EPERM;
#else  /* __SUPPORT_CRADLE__ */
		ret = 0;
#endif /* __SUPPORT_CRADLE__ */
	}
