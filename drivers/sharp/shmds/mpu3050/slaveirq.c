...
/* shmds add -> */
#ifdef SHMDS_DETECT
extern atomic_t shmds_detect_mode_flg;
#endif /* SHMDS_DETECT */
/* shmds add <- */
...
/* shmds add -> */
static void slaveirq_timerhandler(u_long pdata)
{
	char gpio_data = 0;
	struct slaveirq_dev_data *data = (struct slaveirq_dev_data *)pdata;
	static int mycount;
...
/* shmds add -> */
	...
	if (atomic_read(&shmds_detect_mode_flg) == SHMDS_DETECT_ECONOMIZE) {
		...
		mod_timer(&timer_acc,jiffies + msecs_to_jiffies(2000));
	}
/* shmds add <- */
	else if (data->irq == MSM_GPIO_TO_INT(GPIO_ACCEL_INT)) {
#else
	if (data->irq == MSM_GPIO_TO_INT(GPIO_ACCEL_INT)) {
#endif /* SHMDS_DETECT */
		gpio_data = gpio_get_value(GPIO_ACCEL_INT);
		if (gpio_data == 1) {
			data->data.interruptcount++;
...
static irqreturn_t slaveirq_handler(int irq, void *dev_id)
{
	...
	wake_up_interruptible(&data->slaveirq_wait);
/* shmds add -> */
	if (irq == MSM_GPIO_TO_INT(GPIO_ACCEL_INT)) {
		  ...
		  struct ext_slave_platform_data *pdata, char *name)
{
	...
	data->timeout = 0;
/* shmds add -> */
	if (pdata->irq == MSM_GPIO_TO_INT(GPIO_ACCEL_INT)) {
		timer_acc.function = slaveirq_timerhandler;
		timer_acc.data = (u_long) data;
		init_timer(&timer_acc);
		INIT_WORK(&(data->work), slaveirq_work_acc);
	}
/* <- shmds add */
