...
/* shmds add*/
#ifdef SHMDS_DETECT
#ifdef SHMDS_ADB_FLAG
static int shmds_count = 0;
#endif /* SHMDS_ADB_FLAG */
extern int shmds_ths_value;
extern int shmds_duration_value;
extern int shmds_buf_size_temp;
extern int shmds_md_thresh;
extern int shmds_md_thresh2;
#endif /* SHMDS_DETECT */
/* shmds add*/
...
/* shmds add ->*/
	...
	shmds_count ++;
#endif /* defined(SHMDS_DETECT) && defined(SHMDS_ADB_FLAG) */
/* shmds add <-*/
...
/* shmds mod -> */
#if defined(SHMDS_DETECT) && defined(SHMDS_ADB_FLAG)
	case MPU_MODIFY_BUF_SIZE:
		if (copy_from_user(&shmds_buf_size_temp, (unsigned char __user *) arg, sizeof(shmds_buf_size_temp))) {
			printk("modify shmds_buf_size error!\n");
			return -1;
		}
		if (shmds_buf_size_temp <= 0) {
			shmds_buf_size_temp = 10;
		}
		break;
	case MPU_MODIFY_THRESH:
		if (copy_from_user(&shmds_md_thresh, (unsigned char __user *) arg, sizeof(int))) {
			printk("modify shmds_md_thresh error!\n");
			return -1;
		}
		break;
	case MPU_MODIFY_THS_VALUE:
		if (copy_from_user(&shmds_ths_value,(unsigned char __user *) arg, sizeof(int))) {
			printk("modify shmds_ths_value error!\n");
			return -1;
		}
		break;
	case MPU_MODIFY_DURATION_VALUE:
		if (copy_from_user(&shmds_duration_value,(unsigned char __user *) arg, sizeof(int))) {
			printk("modify shmds_duration_value error!\n");
			return -1;
		}
		break;
	case MPU_MODIFY_THRESH2:
		if (copy_from_user(&shmds_md_thresh2, (unsigned char __user *) arg, sizeof(int))) {
			printk("modify shmds_md_thresh2 error!\n");
			return -1;
		}
		break;
#endif /* defined(SHMDS_DETECT) && defined(SHMDS_ADB_FLAG) */
	case MPU_READ_REGISTER:
		{
			struct mpu_read_write param;
			unsigned char data[2];
			retval = copy_from_user(&param, (unsigned char __user *) arg, sizeof(param));
			if (retval) {
				break;
			}
			retval = copy_from_user(data,param.data,sizeof(data));
			if (retval) {
				break;
			}
			param.length = 1;
			if (mldl_cfg->mpu_chip_info->addr == param.address) {
				retval = inv_i2c_read(client->adapter, mldl_cfg->mpu_chip_info->addr, data[0], param.length, &(data[1]));
			} else if (pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address == param.address) {
				retval = inv_i2c_read(slave_adapter[EXT_SLAVE_TYPE_COMPASS], pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address, data[0], param.length, &(data[1]));
			} else if (pdata_slave[EXT_SLAVE_TYPE_ACCEL]->address == param.address) {
				retval = inv_i2c_read(slave_adapter[EXT_SLAVE_TYPE_ACCEL], pdata_slave[EXT_SLAVE_TYPE_ACCEL]->address, data[0], param.length, &(data[1]));
			}
			if (retval) {
				break;
			}
			retval = copy_to_user(param.data,data,sizeof(data));
			if (retval) {
				break;
			}
			retval = copy_to_user((unsigned char __user *) arg, &param, sizeof(param));
		}
		break;
	case MPU_WRITE_REGISTER:
		{
			struct mpu_read_write param;
			unsigned char data[2];
			retval = copy_from_user(&param, (unsigned char __user *) arg, sizeof(param));
			...
			if (mldl_cfg->mpu_chip_info->addr == param.address) {
				retval = inv_i2c_write_register(client->adapter, mldl_cfg->mpu_chip_info->addr, data[0], data[1]);
			} else if (pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address == param.address) {
				retval = inv_i2c_write_register(slave_adapter[EXT_SLAVE_TYPE_COMPASS], pdata_slave[EXT_SLAVE_TYPE_COMPASS]->address, data[0], data[1]);
			} else if (pdata_slave[EXT_SLAVE_TYPE_ACCEL]->address == param.address) {
				retval = inv_i2c_write_register(slave_adapter[EXT_SLAVE_TYPE_ACCEL], pdata_slave[EXT_SLAVE_TYPE_ACCEL]->address, data[0], data[1]);
			}
		}
		break;
	case MPU_GET_PORTSTATE:
		{
			char gpio_state[4];
			gpio_state[0] = gpio_get_value(GPIO_ACCEL_INT);
			gpio_state[1] = gpio_get_value(GPIO_GYRO_INT);
//			gpio_state[2] = gpio_get_value(GPIO_COMPASS_DRDY);
//			gpio_state[3] = gpio_get_value(GPIO_COMPASS_INT);
			retval = copy_to_user((unsigned char __user *) arg, gpio_state, sizeof(gpio_state));
		}
		break;
	case MPU_GET_POLARITY:
		{
			signed char orientation[3][9];
			memcpy(&(orientation[0][0]), mldl_cfg->pdata->orientation, sizeof(orientation[0]));
			memcpy(&(orientation[1][0]), pdata_slave[EXT_SLAVE_TYPE_ACCEL]->orientation, sizeof(orientation[1]));
			memcpy(&(orientation[2][0]), pdata_slave[EXT_SLAVE_TYPE_COMPASS]->orientation, sizeof(orientation[2]));
			retval = copy_to_user((unsigned char __user *) arg, orientation, sizeof(orientation));
		}
		break;
/* <- shmds mod */
	default:
...
/* shmds add -> */
	...
	signed char gyr_orient[9] = {-1,  0,  0, 
								  0, -1,  0, 
								  0,  0,  1 };
#else
	signed char gyr_orient[9] =	{-1,  0,  0,
...
/* shmds add -> */
	for (ii = 0; ii < ARRAY_SIZE(pdata->orientation); ii++){
		pdata->orientation[ii] = gyr_orient[ii];
	}
/* <- shmds add */
