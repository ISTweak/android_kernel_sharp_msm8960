...
static int shmds_buf_size_temp = 10;
...
static void SHMDS_ami603_work_func(struct work_struct *work)
{
	...
	if (((buf1 & 0x08) == 0x08) && ((buf2 & 0x80) == 0x80)) {
		AMI_i2c_recv_b(ami_i2c_client, AMI_REG_INTREL, &buf1);
		shmds_wakeup = 1;
...
/* shmds mod -> */
#ifdef SHMDS_DETECT
	if ((shmds_detect_switch_flg == 1) && ((ami_sen_stat.mode & AMI_BIT_PEDO_MODE) == 0)) {
		if (shmds_init_flg == 0) {
			int i, j;
			shmds_init_flg = 1;
...
/* shmds add -> */
#ifdef SHMDS_DETECT
		} else if (interval == AMI_INTERVAL_1000MS || interval == AMI_INTERVAL_500MS 
		        || interval == AMI_INTERVAL_100MS  || interval == AMI_INTERVAL_60MS) {
#else
		} else if (interval == AMI_INTERVAL_100MS
...
/* shmds add -> */
#ifdef SHMDS_DETECT
	else if (interval ==  AMI_INTERVAL_1000MS) {
		interval = AMI_INTERVAL_100MS;
	}
	else if (interval ==  AMI_INTERVAL_500MS) {
		interval = AMI_INTERVAL_60MS;
	}
	k_start_param.interval = interval;
#endif
...
/* shmds add -> */
	...
	case AMI_IOCTL_GET_SENSORRAWVALUE2:
		...
		if ((g_interval == AMI_INTERVAL_60MS) && (shmds_buf_size_temp != 100)) {
			shmds_buf_size_temp = 100;
			shmds_init_flg = 0;
		} else if ((g_interval == AMI_INTERVAL_100MS) && (shmds_buf_size_temp != 30)){
			shmds_buf_size_temp = 30;
			shmds_init_flg = 0;
		}
