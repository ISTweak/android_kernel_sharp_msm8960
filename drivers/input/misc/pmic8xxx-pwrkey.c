...
static irqreturn_t pwrkey_press_irq(int irq, void *_pwrkey)
{
	...
	printk("##PWR_KEY press\n");

	if (pwrkey->press == true) {
#ifdef	PWRKEY_CUST_SH
		int press;

		press = pm8xxx_read_irq_stat(pwrkey->dev->parent, pwrkey->key_press_irq);
		/* Is this TRUE press ? */
		if (press == 0) {
