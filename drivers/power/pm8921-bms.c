...
static s64 ccmilliampere_to_mas(s64 cc_ua)
{
	return div_s64(cc_ua * CC_READING_TICKS * SMPL_CLK_HZ_NUME,
			SMPL_CLK_HZ_DENO);
}
...
static int read_vbatt_avg(struct pm8921_bms_chip *chip, int *result, uint *adc_code)
{
	int rc;
	int16_t reading;
	int usb_chg;

	rc = pm_bms_read_output_data(chip, VBATT_AVG, &reading);
	if (rc)
	{
		pr_err("fail to read VBATT_AVG rc = %d\n", rc);
		return rc;
	}
	*adc_code = reading;
	...
	convert_vbatt_raw_to_uv(chip, usb_chg, reading, result);
	pr_debug("read = %04x vbatt = %d\n", reading, *result);
	return 0;
}
int pm8921_bms_get_last_good_ocv(uint *result, uint *adc_code)
{
	int rc;
	struct pm8921_soc_params raw;
	...
	if (the_chip)
	{
		rc = read_soc_params_raw(the_chip, &raw);
		if (!rc)
		{
			*result = raw.last_good_ocv_uv / 1000;
			*adc_code = raw.last_good_ocv_raw;
		}
		return rc;
	}
...
EXPORT_SYMBOL(pm8921_bms_get_last_good_ocv);

int pm8921_bms_get_vbatt_for_rbatt(uint *result, uint *adc_code)
{
	pr_debug("read_rbatt_params_raw() is not supported.\n");
	*result = 0;
	*adc_code = 0;
	return 0;
}
EXPORT_SYMBOL(pm8921_bms_get_vbatt_for_rbatt);

int pm8921_bms_get_vsense_for_rbatt(uint *result, uint *result_mA)
{
	pr_debug("read_rbatt_params_raw() is not supported.\n");
	*result = 0;
	*result_mA = 0;
...
EXPORT_SYMBOL(pm8921_bms_get_vsense_for_rbatt);

int pm8921_bms_get_ocv_for_rbatt(uint *result, uint *adc_code)
{
	pr_debug("read_rbatt_params_raw() is not supported.\n");
	*result = 0;
	*adc_code = 0;
...
EXPORT_SYMBOL(pm8921_bms_get_ocv_for_rbatt);

int pm8921_bms_get_vbatt_avg(int *result, uint *adc_code)
{
	int rc;

	if (the_chip)
	{
		rc = read_vbatt_avg(the_chip, result, adc_code);
		if (!rc)
		{
			*result = *result / 1000;
		}
...
EXPORT_SYMBOL(pm8921_bms_get_vbatt_avg);
...
int pm8921_bms_get_vsense_avg_read(int *result, int *result_mA)
{
	...
	if (the_chip)
	{
		...
		if (!rc)
		{
			*result_mA = pm8xxx_cc_scale_uv_to_ma(*result);
			pr_debug("vsense_for_milliamperes = %d\n", *result_mA);
		}
		return rc;
	}
...
EXPORT_SYMBOL(pm8921_bms_get_vsense_avg_read);
#endif /* CONFIG_BATTERY_SH */
...
int pm8921_bms_reset_cc(void)
{
	...
	if(!the_chip)
	{
		pr_err("pm8921_bms_reset_cc the_chip is null \n");
		return -EINVAL;
	}
	rc = reset_cc(the_chip);
	
	return rc;
}
EXPORT_SYMBOL(pm8921_bms_reset_cc);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
static void calculate_cc_mas(struct pm8921_bms_chip *chip, int cc, int64_t *cc_mas)
{
	...
	cc_voltage_uv = pm8xxx_cc_adjust_for_gain(cc_voltage_uv);
	pr_debug("cc_voltage_uv = %lld microvolts\n", cc_voltage_uv);
	cc_current_ma = pm8xxx_cc_scale_uv_to_ma(cc_voltage_uv);
	*cc_mas = ccmilliampere_to_mas(cc_current_ma);
	pr_debug("cc_mas = %lld micro_amp_second\n", *cc_mas);
}
#endif /* CONFIG_BATTERY_SH */
...
int pm8921_bms_get_cc(int offset, int *result, int64_t *result_mAs)
{
	...
	if (the_chip)
	{
		rc = read_cc(the_chip, result);
		if (!rc)
		{
			*result -= offset;
			calculate_cc_mas(the_chip, *result, result_mAs);
			pr_debug("cc = %d cc_mAs = %lldmAs\n", *result, *result_mAs);
		}
		return rc;
	}
	pr_err("called before initialization\n");
...
EXPORT_SYMBOL(pm8921_bms_get_cc);
#endif /* CONFIG_BATTERY_SH */
...
{
	u8 value = enable ? (EN_BMS_BIT | EN_PON_HS_BIT) : 0;
	if(!the_chip)
	{
		return -EINVAL;
	}
	return pm_bms_masked_write(the_chip, BMS_CONTROL, EN_BMS_BIT | EN_PON_HS_BIT, value);
}
EXPORT_SYMBOL(pm8921_bms_enable);
#endif /* CONFIG_BATTERY_SH */

#ifdef CONFIG_SHSYS_CUST
void pm_bms_masked_disable( void )
{
	pm_bms_masked_write(the_chip, BMS_CONTROL, EN_BMS_BIT , 0);
}
