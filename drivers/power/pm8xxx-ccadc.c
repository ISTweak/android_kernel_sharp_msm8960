...
static int cc_curmin = 0;
static int cc_curmax = 0;
static int cc_volmin = 0;
static int cc_volmax = 0;
static bool cc_calflg = false;

s64 pm8xxx_cc_scale_uv_to_ma(s64 uv)
{
	...
	if (cc_calflg == false)
	{
		if ((cc_curmin != cc_curmax) && (cc_volmin != cc_volmax))
		{
			calc_nume = cc_curmax - cc_curmin;
			calc_deno = cc_volmax - cc_volmin;
			calc_base = cc_volmin;
			calc_offset = cc_curmin;
			pr_debug("calib: nume = %d deno = %d base = %d offset = %d\n",
				calc_nume, calc_deno, calc_base, calc_offset);
			cc_calflg = true;
		}
	}
	...
	if (cc_calflg == true)
	{
		...
		result_mA += calc_offset;
	}
	else
	{
		result_mA = div_s64(uv, the_chip->r_sense);
	}

	return result_mA;
}
EXPORT_SYMBOL_GPL(pm8xxx_cc_scale_uv_to_ma);

void pm8xxx_cc_set_vsense_avg_calibration_data(int adjmin, int adjmax, int curmin, int curmax)
{
	cc_volmin = adjmin;
	cc_volmax = adjmax;
	cc_curmin = curmin;
...
EXPORT_SYMBOL_GPL(pm8xxx_cc_set_vsense_avg_calibration_data);

void pm8xxx_cc_refresh_vsense_avg_calibration_data(void)
{
	cc_calflg = false;
}
EXPORT_SYMBOL_GPL(pm8xxx_cc_refresh_vsense_avg_calibration_data);
#endif /* CONFIG_BATTERY_SH */
