...
#ifdef CONFIG_SHSYS_CUST
#ifdef __SUPPORT_CRADLE__
	struct work_struct		swic_detect_cb_regist_work;
#endif /* __SUPPORT_CRADLE__ */
#endif /* CONFIG_SHSYS_CUST */
	int64_t				batt_id_min;
	int64_t				batt_id_max;
...
#ifdef CONFIG_BATTERY_SH
	struct delayed_work		usbin_ov_check_work;
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
static unsigned int set_vbus_mA;
static unsigned long sh_boot_mode = 0;
static bool isOverVoltage = false;
static unsigned char shusb_softupdate_mode_flag = 0;
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_auto_enable(int enable)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_auto_enable(the_chip, enable);
}
EXPORT_SYMBOL_GPL(pm8921_charger_auto_enable);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
#define CHG_CHARGE_BAT	BIT(3)
static int pm_chg_batt_trans_switch(struct pm8921_chg_chip *chip, int enable)
{
	return pm_chg_masked_write(chip, CHG_CNTRL, CHG_CHARGE_BAT,
				enable ? CHG_CHARGE_BAT : 0);
}
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_vddmax_set(int voltage)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_vddmax_set(the_chip, voltage);
}
EXPORT_SYMBOL_GPL(pm8921_charger_vddmax_set);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_vinmin_set(int voltage)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(pm8921_charger_vinmin_set);

int pm8921_charger_vinmin_get(int *voltage)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	*voltage = pm_chg_vinmin_get(the_chip);
	
	return 0;
}
EXPORT_SYMBOL_GPL(pm8921_charger_vinmin_get);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_ibatmax_set(int chg_current)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_ibatmax_set(the_chip, chg_current);
}
EXPORT_SYMBOL_GPL(pm8921_charger_ibatmax_set);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_vtrkl_low_set(int millivolts)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_vweak_set(the_chip, millivolts);
}
EXPORT_SYMBOL_GPL(pm8921_charger_vtrkl_low_set);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_itrkl_set(int milliamps)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_itrkl_set(the_chip, milliamps);
}
EXPORT_SYMBOL_GPL(pm8921_charger_itrkl_set);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_charge_dis(int disable)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_usb_suspend_enable(the_chip, disable);
}
EXPORT_SYMBOL_GPL(pm8921_charger_charge_dis);

int pm8921_charger_batt_trans_switch(int enable)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	return pm_chg_batt_trans_switch(the_chip, enable);
}
EXPORT_SYMBOL_GPL(pm8921_charger_batt_trans_switch);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
#ifdef __SUPPORT_CRADLE__
shswic_result_t get_shswic_cradle_status(uint8_t* device)
{
	shswic_result_t swic_ret = SHSWIC_SUCCESS;

	if(shswic_is_implement() != SHSWIC_IMPLEMENT)
	{
		*device = SHSWIC_ID_CRADLE_UNKNOWN;
		return swic_ret;
	}

	swic_ret = shswic_get_cradle_status(device);
	if(swic_ret != SHSWIC_SUCCESS)
	{
		return swic_ret;
	}

	if(*device == SHSWIC_ID_CRADLE_UNKNOWN)
	{
		int usbin_valid;
		const int SHSWIC_GPIO_VCDET = 81;
		if(!the_chip)
		{
			pr_err("[%s]Error the_chip is NULL\n", __func__);
			return SHSWIC_FAILURE;
		}

		usbin_valid = pm_chg_get_rt_status(the_chip, USBIN_VALID_IRQ);

		if(usbin_valid && !gpio_get_value(SHSWIC_GPIO_VCDET))
		{
...
static int get_prop_batt_cable_status(struct pm8921_chg_chip *chip)
{
	int ret;
#ifdef __SUPPORT_CRADLE__
	shswic_result_t swic_ret;
	uint8_t device = SHSWIC_ID_NONE;
#endif /* __SUPPORT_CRADLE__ */

#ifdef __SUPPORT_CRADLE__
	swic_ret = get_shswic_cradle_status(&device);
	if(swic_ret != SHSWIC_SUCCESS)
	{
		pr_err("[%s]Error shswic_get_cradle_status() ret=[%d]\n", __func__, swic_ret);
		return POWER_SUPPLY_CABLE_STATUS_UNKNOWN;
	}

	if(device == SHSWIC_ID_CRADLE) /* CRADLE */
	{
		ret = POWER_SUPPLY_CABLE_STATUS_CRADLE;
	}
	else
#endif /* __SUPPORT_CRADLE__ */
	{
		if(set_vbus_mA == 1500) /* AC */
		{
			ret = POWER_SUPPLY_CABLE_STATUS_AC;
		}
		else if((set_vbus_mA == 100) || (set_vbus_mA == 500)) /* USB */
		{
			ret = POWER_SUPPLY_CABLE_STATUS_USB;
		}
		else /* USB not connect */
		{
			ret = POWER_SUPPLY_CABLE_STATUS_UNKNOWN;
		}
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL:
		...
		break;
#ifdef CONFIG_BATTERY_SH
	case POWER_SUPPLY_PROP_CABLE_STATUS:
		val->intval = get_prop_batt_cable_status(chip);
		break;
#endif /* CONFIG_BATTERY_SH */
	default:
		return -EINVAL;
...
#ifdef CONFIG_BATTERY_SH
#define SHBATT_BOOT_MODE_FUNC_TEST		0x40
#define SHBATT_BOOT_MODE_NORMAL			0xFFFF
#endif /* CONFIG_BATTERY_SH */
...
static void __pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	if(device != SHSWIC_ID_CRADLE)
#endif /* __SUPPORT_CRADLE__ */
	{

		printk(KERN_ERR "%s:%d,(%s,%d)",__FUNCTION__, mA,__FILE__, __LINE__);
		if(mA <= 2)
		{
			bool usb_suspend = true;
			int result = pm_chg_usb_suspend_enable(the_chip, usb_suspend);
			if (result != 0)
			{
				printk(KERN_ERR "usb suspend failed!!,(%s,%d)", __FILE__, __LINE__);
				return;
			}
		...
		else if(mA == 500)
		{
			...
			shchg_api_notify_usb_charger_i_is_available(500);
			return;
		}
		else if(mA == 1500)
		{
			...
			shchg_api_notify_usb_charger_connected(SHCHG_DEVICE_USB_CHARGER);
			shchg_api_notify_usb_charger_i_is_available(1500);
			return;
		}
		else
		{
...
#ifdef __SUPPORT_CRADLE__
/* cradle charging set */
static void __pm8921_charger_cradle_vbus_draw(uint8_t device)
{
	int rc;
	shswic_result_t swic_ret;
	...
	if(device == SHSWIC_ID_CRADLE)
	{
		if ( sh_boot_mode == SHBATT_BOOT_MODE_FUNC_TEST ) {
			rc = pm_chg_batt_trans_switch(the_chip, 1);
			if(rc)
			{
				pr_err("fail to reset bat_fet bit rc=%d\n", rc);
			}
			pr_debug("Func Test Mode and cradle\n");
		}

		/* VBUS OFF */
		if(shswic_is_implement() == SHSWIC_IMPLEMENT)
		{
			swic_ret = shswic_write_vbsw_reg(SHSWIC_VBSW_OFF);
			if(swic_ret != SHSWIC_SUCCESS)
			{
				pr_err("[%s]Error shswic_write_vbsw_reg() ret=[%d]\n", __func__, swic_ret);
			}
		}
		...
		if(rc)
		{
			pr_err("[%s]Error pm_chg_usb_suspend_enable() ret=[%d]\n", __func__, rc);
		}
		rc = pm_chg_iusbmax_set(the_chip, ARRAY_SIZE(usb_ma_table) - 1);
		if(rc)
		{
			pr_err("[%s]Error pm_chg_iusbmax_set() ret=[%d]\n", __func__, rc);
		}
	}
	else
	{
		__pm8921_charger_vbus_draw(set_vbus_mA);
		/* VBUS Auto */
		if(shswic_is_implement() == SHSWIC_IMPLEMENT)
		{
			swic_ret = shswic_write_vbsw_reg(SHSWIC_VBSW_AUTO);
			if(swic_ret != SHSWIC_SUCCESS)
			{
				pr_err("[%s]Error shswic_write_vbsw_reg() ret=[%d]\n", __func__, swic_ret);
			}
...
static void shchg_cradle_detect_cb(uint8_t device, void* user_data)
{
#if 0
	unsigned long flags;

	spin_lock_irqsave(&vbus_lock, flags);
	__pm8921_charger_cradle_vbus_draw(device);
...
static void swic_detect_cb_regist(struct work_struct *work)
{
	shswic_result_t swic_ret;
	swic_ret = shswic_detect_cb_regist(SHSWIC_CHG_DEVICE, SHSWIC_ID_CRADLE, (void*)shchg_cradle_detect_cb, NULL);
	if(swic_ret != SHSWIC_SUCCESS)
	{
		pr_err("[%s]Error shswic_detect_cb_regist() ret=[%d]\n", __func__, swic_ret);
		return;
	}
}
#endif /* __SUPPORT_CRADLE__ */

int pm8921_charger_iusbmax_set(int reg_val)
{
	int i;
	...
	for (i = ARRAY_SIZE(usb_ma_table) - 1; i >= 0; i--) {
		if (usb_ma_table[i].usb_ma <= reg_val)
			break;
	}
	if (i < 0)
		i = 0;
	return pm_chg_iusbmax_set(the_chip, i);
}
EXPORT_SYMBOL_GPL(pm8921_charger_iusbmax_set);
#endif /* CONFIG_BATTERY_SH */
...
#else  /* CONFIG_SHSYS_CUST */
	if (disable)
		pr_debug("current drawn from chg=0, battery provides current\n");		
#endif /* CONFIG_SHSYS_CUST */
...
#ifdef CONFIG_BATTERY_SH
int pm8921_charger_set_meas_freq(int update_time)
{
	if (!the_chip) {
		pr_err("called before init\n");
		return -EINVAL;
	}
	the_chip->update_time = update_time;
	return 0;
}
EXPORT_SYMBOL_GPL(pm8921_charger_set_meas_freq);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
#define USBIN_OV_CHECK_MS 2000
static void usbin_ov_check_worker(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct pm8921_chg_chip *chip = container_of(dwork,
			struct pm8921_chg_chip, usbin_ov_check_work);
...
static irqreturn_t usbin_valid_irq_handler(int irq, void *data)
{
	...
	if(isOverVoltage)
	{
		...
		usbin_valid = pm_chg_get_rt_status(chip, USBIN_VALID_IRQ);
		pr_err("[ovp]usbin_valid_irq_handler=%d\n",usbin_valid);
		if(usbin_valid)
		{
			cancel_delayed_work_sync(&the_chip->usbin_ov_check_work);
			isOverVoltage = false;
		}
	}
	else
	{
		pr_err("[ovp]usbin_valid_irq_handler\n");
...
static irqreturn_t usbin_ov_irq_handler(int irq, void *data)
{
	...
	if(!isOverVoltage)
	{
		...
		pm_chg_failed_clear(chip, 1);
		usbin_valid = pm_chg_get_rt_status(chip, USBIN_VALID_IRQ);
		usbin_uv = pm_chg_get_rt_status(chip, USBIN_UV_IRQ);
		if(!usbin_valid && !usbin_uv)
		{
			pr_err("[ovp]schedule_delayed_work usbin_valid=%d, usbin_uv=%d\n", usbin_valid, usbin_uv);
			isOverVoltage = true;
			schedule_delayed_work(&the_chip->usbin_ov_check_work,
				      round_jiffies_relative(msecs_to_jiffies
						(USBIN_OV_CHECK_MS)));
		}
	}
...
static void __devinit determine_initial_state(struct pm8921_chg_chip *chip)
{
...
#else  /* CONFIG_SHSYS_CUST */
	pm8921_chg_disable_irq(chip, BATT_REMOVED_IRQ);
#endif /* CONFIG_SHSYS_CUST */
...
#else  /* CONFIG_SHSYS_CUST */
	pm8921_chg_disable_irq(chip, CHGFAIL_IRQ);
	pm8921_chg_disable_irq(chip, FASTCHG_IRQ);
	pm8921_chg_disable_irq(chip, VBATDET_LOW_IRQ);
	pm8921_chg_disable_irq(chip, BAT_TEMP_OK_IRQ);
#endif /* CONFIG_SHSYS_CUST */
	...
	if(device == SHSWIC_ID_CRADLE)
	{
		__pm8921_charger_cradle_vbus_draw(SHSWIC_ID_CRADLE);
		if(usb_chg_current)
		{
			set_vbus_mA = usb_chg_current;
		}
	}
	else
#endif /* __SUPPORT_CRADLE__ */
...
#ifdef CONFIG_BATTERY_SH
	CHG_IRQ(WDOG_IRQ, IRQF_TRIGGER_RISING, wdog_irq_handler),
#endif /* CONFIG_BATTERY_SH */
};
...
static int __devinit pm8921_chg_hw_init(struct pm8921_chg_chip *chip)
{
	...
	if( p_smem_addr == NULL) {
		pr_err("sh_smem_get_common_address == NULL!!\n");
	}
	else {
		sh_boot_mode = p_smem_addr->sh_boot_mode;
		pr_debug("Func Test Mode = 0x%lx\n", sh_boot_mode);

		shusb_softupdate_mode_flag = p_smem_addr->shusb_softupdate_mode_flag;
		pr_debug("shusb_softupdate_mode_flag = %d\n", shusb_softupdate_mode_flag);
	}
	charging_disabled = 1;
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
	if(sh_boot_mode == SHBATT_BOOT_MODE_NORMAL && !p_smem_addr->shusb_softupdate_mode_flag)
	{
		/*initialize PMIC for charge.*/
		rc = pm_chg_usb_suspend_enable(chip, true);
		if(rc)
		{
			pr_err("Failed to usb suspend rc=%d\n", rc);
			return rc;
		}
		pm_chg_batt_trans_switch(chip, 1);
		if(rc)
		{
			pr_err("Failed to batfet on rc=%d\n", rc);
			return rc;
		}
...
#ifdef CONFIG_BATTERY_SH
	INIT_DELAYED_WORK(&chip->usbin_ov_check_work, 
						usbin_ov_check_worker);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
	pm8921_usb_ovp_set_threshold(PM_USB_OV_7V);
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_SHSYS_CUST
#ifdef __SUPPORT_CRADLE__
	INIT_WORK(&chip->swic_detect_cb_regist_work, swic_detect_cb_regist);
#endif /* __SUPPORT_CRADLE__ */
#endif /* CONFIG_SHSYS_CUST */

#ifdef CONFIG_SHSYS_CUST
#ifdef __SUPPORT_CRADLE__
	schedule_work(&chip->swic_detect_cb_regist_work);
#endif /* __SUPPORT_CRADLE__ */
#endif /* CONFIG_SHSYS_CUST */
...
#ifdef CONFIG_BATTERY_SH
#ifdef __SUPPORT_CRADLE__
	shswic_detect_cb_regist(SHSWIC_CHG_DEVICE, 0, (void*)NULL, NULL);
#endif /* __SUPPORT_CRADLE__ */
#endif /* CONFIG_BATTERY_SH */
	free_irqs(chip);
