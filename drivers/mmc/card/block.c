...
static u32 sh_stop_resp_err        = 0;
static u32 sh_card_status_resp_err = 0;
...
static int get_card_status(struct mmc_card *card, u32 *status, int retries)
{
	...
	if ((err == 0) && 
	    !strncmp(mmc_hostname(card->host),HOST_MMC_MMC,sizeof(HOST_MMC_MMC)))
		sh_card_status_resp_err |= cmd.resp[0];
#endif /* CONFIG_ERR_RETRY_EMMC_CUST_SH */
...
#ifdef MBR_EBR_PARTITION_CUST_SH
...
#endif /* MBR_EBR_PARTITION_CUST_SH */
		...
		if (reqs >= packed_num) {
			list_for_each_entry(prq, &mqrq->packed_list, queuelist) {
				if (mmc_check_protect_part(prq, &blocks, &protect_err) != 0) {
					...
					printk(KERN_WARNING "%s:%u: protect status(1) %d "
			...
			if (mmc_check_protect_part(req, &blocks, &protect_err) != 0) {
				...
				printk(KERN_WARNING "%s:%u: protect status(2) %d "
					"sector %u, nr %u, blocks %d\n",
					req->rq_disk->disk_name,
...
static int mmc_check_protect_blk_end_request(struct mmc_queue_req *mqrq, struct mmc_card *card, struct mmc_queue *mq, u8 reqs)
{
	...
	if (mmc_card_mmc(card)) {
		if (reqs >= packed_num) {
			...
			printk(KERN_WARNING "mmc_check_protect_blk_end_request (1)");
			return 1;	// Error 
		}
		else {
			protect_err = -EROFS;
			...
			spin_unlock_irq(&md->lock);
			printk(KERN_WARNING "mmc_check_protect_blk_end_request (2)");
			return 1;	// Error 
		}
	}
...
static int mmc_blk_set_retry_cnt(struct mmc_card *card, int param)
{
	...
	if (param == 0) {
		...
		printk(KERN_WARNING "%s: retry count(%d)\n",
...
#ifdef CONFIG_ERR_RETRY_EMMC_CUST_SH
	if (err)
		printk(KERN_ERR "%s: recovery %d error!!\n",
				mmc_hostname(host), err);
	else
		printk(KERN_INFO "%s: recovery success!!\n",
				mmc_hostname(host));
#endif /* CONFIG_ERR_RETRY_EMMC_CUST_SH */
...
#if defined( CONFIG_DISCARD_EMMC_CUST_SH )
	if( card->ext_csd.rev <= 5 ){
		err = 0;
		goto out;
	}
#endif /*CONFIG_DISCARD_MMC_CUST_SH*/
			     ...
			     struct mmc_async_req *areq)
{
	...
	{
		sh_stop_resp_err        = brq->stop.resp[0];
		sh_card_status_resp_err = 0;
	}
#endif /* CONFIG_ERR_RETRY_EMMC_CUST_SH */
...
#ifdef CONFIG_ERR_RETRY_EMMC_CUST_SH
	if ((((sh_stop_resp_err & RESP0_ERR_BITS_MASK) != 0) || 
		 ((sh_card_status_resp_err & RESP0_ERR_BITS_MASK) != 0 )) &&
		((brq->cmd.opcode == 24) || (brq->cmd.opcode == 25)) &&
	    !strncmp(mmc_hostname(card->host),HOST_MMC_MMC,sizeof(HOST_MMC_MMC)))
	{
		pr_err("%s: error found in resp[0] in stop[%08x] or status[%08x]\n",
				mmc_hostname(card->host), 
				sh_stop_resp_err, sh_card_status_resp_err );
		sh_stop_resp_err        = 0;
		sh_card_status_resp_err = 0;
...
#endif /* CONFIG_ACCESS_OPTIMIZE_EMMC_CUST_SH */
...
#if defined(CONFIG_MMC_CUST_SH) && defined(CONFIG_ANDROID_ENGINEERING)
	if (card != NULL) {
		if (sh_mmc_debug_mask && ((card)->type == (sh_mmc_debug_mask-1))) {
			printk( KERN_DEBUG"%d,%d,%d\n", 
				brq->cmd.opcode,
				((brq->cmd.arg)/((mmc_card_blockaddr(card)) ? 
					1 : brq->data.blksz)),
				brq->data.blocks);
		}
	}
...
#ifdef CONFIG_MMC_SD_CUST_SH
	if (mmc_card_sd(card)) {
		if(card->cid.manfid == 39 && card->cid.oemid == 20552){
			card->host->max_blk_count = 0x0040;
		}
	}
#endif /* CONFIG_MMC_SD_CUST_SH */
