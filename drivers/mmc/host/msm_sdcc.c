...
/* from core.c */
extern void mmc_gate_clock(struct mmc_host *host);
extern void mmc_ungate_clock(struct mmc_host *host);
...
unsigned int msmsdcc_gpio_flg = 0;
static unsigned int msmsdcc_slot_status(struct msmsdcc_host *host);
#endif /* CONFIG_MMC_SD_CUST_SH */

#ifdef CONFIG_MMC_SD_CUST_SH
#define DEV_ID_SDCC3	3
static struct platform_device *pdev_sd = NULL;
static bool force_enable_sd = false;
#endif /* CONFIG_MMC_SD_CUST_SH */

#ifdef CONFIG_MMC_CUST_SH
static int pm_qos_req_cnt_sdio = 0;
static int pm_qos_req_cnt_mmc = 0;
static int pm_qos_req_cnt_sd = 0;
static struct pm_qos_request pm_qos_req_dma_sdio;
static struct pm_qos_request pm_qos_req_dma_mmc;
static struct pm_qos_request pm_qos_req_dma_sd;
#endif /* CONFIG_MMC_CUST_SH */

#ifdef CONFIG_PM_EMMC_CUST_SH
static bool msmsdcc_runtime_suspend_clock_off = false;
#endif /* CONFIG_PM_EMMC_CUST_SH */
...
{
	if (!host->plat->enpwr_gpio)
		return;

	if( enable ) {
		if (!strcmp(mmc_hostname(host->mmc),HOST_MMC_SD)) {
		end = msm_timer_get_sclk_time(NULL);

		if (40 > ((end - start) / 1000000)) {
			msleep(40);
		}
		}
		gpio_set_value_cansleep(host->plat->enpwr_gpio, enable);
		msmsdcc_gpio_flg = 1;
	}
	else{
		msm_tlmm_set_hdrive(TLMM_PULL_SDC3_CMD, GPIO_CFG_NO_PULL);
		msm_tlmm_set_hdrive(TLMM_PULL_SDC3_DATA, GPIO_CFG_NO_PULL);

		gpio_set_value_cansleep(host->plat->enpwr_gpio, enable);
		msmsdcc_gpio_flg = 0;

		if (!strcmp(mmc_hostname(host->mmc),HOST_MMC_SD)) {
			start = msm_timer_get_sclk_time(NULL);
		}
		msleep(10);
	}
...
#ifdef CONFIG_MMC_SD_CUST_SH
	struct mmc_host *mmc_temp;
	struct msmsdcc_host *host_temp;

	if (force_enable_sd) {
		if (pdev_sd) {
			mmc_temp = mmc_get_drvdata(pdev_sd);
			host_temp = mmc_priv(mmc_temp);
			if ((host_temp->clk == host->clk) && (ios->power_mode == MMC_POWER_OFF)) {
				return;
			}
		}
	}
...
#ifdef CONFIG_MMC_CUST_SH
	if (strncmp(mmc_hostname(mmc), HOST_MMC_SDIO, sizeof(HOST_MMC_SDIO)) == 0)
		pm_qos_add_request(&pm_qos_req_dma_sdio,
				PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
	if (strncmp(mmc_hostname(mmc), HOST_MMC_MMC, sizeof(HOST_MMC_MMC)) == 0)
		pm_qos_add_request(&pm_qos_req_dma_mmc,
				PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
	if (strncmp(mmc_hostname(mmc), HOST_MMC_SD, sizeof(HOST_MMC_SD)) == 0)
		pm_qos_add_request(&pm_qos_req_dma_sd,
				PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
#endif /* CONFIG_MMC_CUST_SH */
...
#ifdef CONFIG_MMC_CUST_SH
		if (strncmp(mmc_hostname(mmc), HOST_MMC_SDIO, sizeof(HOST_MMC_SDIO)) == 0)
			pm_qos_remove_request(&pm_qos_req_dma_sdio);
		if (strncmp(mmc_hostname(mmc), HOST_MMC_MMC, sizeof(HOST_MMC_MMC)) == 0)
			pm_qos_remove_request(&pm_qos_req_dma_mmc);
		if (strncmp(mmc_hostname(mmc), HOST_MMC_SD, sizeof(HOST_MMC_SD)) == 0)
			pm_qos_remove_request(&pm_qos_req_dma_sd);
	}
#endif /* CONFIG_MMC_CUST_SH */
 clk_put:
	clk_put(host->clk);
...
#ifdef CONFIG_MMC_SD_CUST_SH
int msmsdcc_force_setup_sdc3_clk(bool enable)
{
	struct mmc_host *mmc_temp;
	struct msmsdcc_host *host_temp;
	...
	if (enable) {
		if (!atomic_read(&host_temp->clks_on)) {
			msmsdcc_setup_clocks(host_temp, true);
			atomic_set(&host_temp->clks_on, 1);
		}
		force_enable_sd = true;
	}
	else {
		if (atomic_read(&host_temp->clks_on)) {
			if(!msmsdcc_slot_status(host_temp)){
			msmsdcc_setup_clocks(host_temp, false);
			atomic_set(&host_temp->clks_on, 0);
			}
		}
		force_enable_sd = false;
	}
...
#ifdef CONFIG_MMC_CUST_SH
static void
msmsdcc_sh_pm_qos_update_latency(struct msmsdcc_host *host, int vote)
{
	int *count_tmp = NULL;
	struct pm_qos_request *pm_qos_req_dma_tmp = NULL;

	if (!host->cpu_dma_latency)
		return;

	if (strncmp(mmc_hostname(host->mmc), HOST_MMC_SDIO, sizeof(HOST_MMC_SDIO)) == 0) {
		count_tmp = &pm_qos_req_cnt_sdio;
		pm_qos_req_dma_tmp = &pm_qos_req_dma_sdio;
	}
	else if (strncmp(mmc_hostname(host->mmc), HOST_MMC_MMC, sizeof(HOST_MMC_MMC)) == 0) { 
        count_tmp = &pm_qos_req_cnt_mmc;
        pm_qos_req_dma_tmp = &pm_qos_req_dma_mmc;
    }
    else if (strncmp(mmc_hostname(host->mmc), HOST_MMC_SD, sizeof(HOST_MMC_SD)) == 0) { 
        count_tmp = &pm_qos_req_cnt_sd;
        pm_qos_req_dma_tmp = &pm_qos_req_dma_sd;
    }
    else{                                                                               
        pr_warn("%s: unexpected hostname\n",                                            
            mmc_hostname(host->mmc));                                                   
        return;                                                                         
    }
	...
	if (vote) {
		if (*count_tmp == 0)
			pm_qos_update_request(pm_qos_req_dma_tmp,
					host->cpu_dma_latency);
		(*count_tmp)++;
	} else {
		(*count_tmp)--;
	...
	if (*count_tmp < 0 || *count_tmp > 2)
		pr_warn("%s: program counter warning count = %d\n",
			mmc_hostname(host->mmc), *count_tmp);
}
#endif /* CONFIG_MMC_CUST_SH */
...
#ifdef CONFIG_MMC_CUST_SH
	msmsdcc_sh_pm_qos_update_latency(host, 1);
#endif /* CONFIG_MMC_CUST_SH */
...
#ifdef CONFIG_PM_EMMC_CUST_SH
			else if (mmc->card && (mmc->card->type == MMC_TYPE_MMC)) {
				if( mmc->ios.clock != 0 ){
					/* CLKGATE is not available */
					/* cancel any clock gating work scheduled by mmc_host_clk_release() */
					cancel_delayed_work_sync( &mmc->clk_gate_work );
					mmc_gate_clock( mmc );
					msmsdcc_runtime_suspend_clock_off = true;
				}
			}
#endif /* CONFIG_PM_EMMC_CUST_SH */
		}
...
out:
	...
	msmsdcc_msm_bus_cancel_work_and_set_vote(host, NULL);
#ifdef CONFIG_MMC_CUST_SH
	msmsdcc_sh_pm_qos_update_latency(host, 0);
#endif /* CONFIG_MMC_CUST_SH */
	return rc;
}
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	if (mmc) {
		if (mmc->card && mmc_card_sdio(mmc->card) &&
			...
			if( msmsdcc_runtime_suspend_clock_off ){
				mmc_ungate_clock( mmc );
				msmsdcc_runtime_suspend_clock_off = false;
			}
		}
#endif /* CONFIG_PM_EMMC_CUST_SH */
