...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (err && cmd->retries && !mmc_card_removed(host->card)) {
		...
		if (sh_mmc_debug_mask && card &&
			...
			printk( KERN_DEBUG" req done %u: %d: %08x %08x %08x %08x\n",
				cmd->opcode, err,
				cmd->resp[0], cmd->resp[1],
				cmd->resp[2], cmd->resp[3]);
		}
		...
		if (mrq->data) {
			...
			if (sh_mmc_debug_mask && card && 
	            ((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
	            ((sh_mmc_debug_mask&0xF0)==0x10) ) {
				printk( KERN_DEBUG"     %d bytes transferred: %d\n",
					mrq->data->bytes_xfered, mrq->data->error);
			}
#endif /* CONFIG_MMC_CUST_SH && CONFIG_ANDROID_ENGINEERING */
		}
		...
		if (mrq->stop) {
			...
			if (sh_mmc_debug_mask && card &&
	            ((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
	            ((sh_mmc_debug_mask&0xF0)==0x10) ) {
					printk( KERN_DEBUG" stop %u: %d: %08x %08x %08x %08x\n",
						mrq->stop->opcode,
						mrq->stop->error,
						mrq->stop->resp[0], mrq->stop->resp[1],
						mrq->stop->resp[2], mrq->stop->resp[3]);
...
mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (sh_mmc_debug_mask && card &&
        ((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
        ((sh_mmc_debug_mask&0xF0)==0x10) ) {
		printk( KERN_DEBUG" starting CMD%u arg %08x flags %08x\n",
			mrq->cmd->opcode,
			mrq->cmd->arg, mrq->cmd->flags);
	}
#endif /* CONFIG_MMC_CUST_SH && CONFIG_ANDROID_ENGINEERING */
	...
	if (mrq->stop) {
		...
		if (sh_mmc_debug_mask && card &&
	        ((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
	        ((sh_mmc_debug_mask&0xF0)==0x10) ) {
			printk( KERN_DEBUG"    stop %u arg %08x flags %08x\n",
				 mrq->stop->opcode,
				 mrq->stop->arg, mrq->stop->flags);
		}
#endif /* CONFIG_MMC_CUST_SH && CONFIG_ANDROID_ENGINEERING */
				  ...
				  struct mmc_request *mrq)
{
	...
	while (1) {
		...
		if (sh_mmc_debug_mask && card &&
	        ((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
	        ((sh_mmc_debug_mask&0xF0)==0x10) ) {
			printk( KERN_DEBUG" req failed %u: %d, retrying...%d\n",
