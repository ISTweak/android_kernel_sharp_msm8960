...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00138 */
struct mdp4_save_reg {
	 u32 save_reg_MDP_PIPE_SRC_SIZE;
	  u32 save_reg_MDP_PIPE_OP_MODE;
};
static struct mdp4_save_reg save_reg[OVERLAY_PIPE_MAX];
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00170 */
...
{
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00108 */
         if (ctrl->hw_version == 0) {
...
void mdp4_overlay_dmae_cfg(struct msm_fb_data_type *mfd, int atv)
{
	...
	if (atv) {
		...
		mdp_vid_quant_set();
#endif /* CONFIG_FB_MSM_EXT_INTERFACE_COMMON */
#else
		mdp_vid_quant_set();
#endif	/* CONFIG_SHLCDC_BOARD */
	}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00017 */
	data = inpdw(MDP_BASE + 0x90000);
	dma2_cfg_reg |= (data & 0x30000000);	/* PCC_EN PGC_EN */
#endif	/* CONFIG_SHLCDC_BOARD */
...
{
	uint32 off, bpp;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00105 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00092 */
	if ((pipe->src_w == 1 || pipe->src_h == 1) &&
	    (pipe->op_mode & MDP4_OP_SCALE_RGB_ENHANCED)) {
	  /*
	   * Bilinear method can not be applied when either pipe->src_w or pipe->src_h is 1.
	   * Therefore, use Pixel repeat/drop method instead of it in that case.
	   */
	  unsigned int w = pipe->src_w;
	  unsigned int h = pipe->src_h;
	  if (w == 1) {
	    w = 2;
	    pipe->phasex_step = 1;
	  }
	  ...
	  src_size = ((h << 16) | w);
	  pipe->op_mode &= ~(MDP4_OP_SCALE_RGB_BILINEAR | MDP4_OP_SCALE_ALPHA_BILINEAR);
	}
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00138 */
	 if((pipe->mixer_num == MDP4_MIXER2) && (pipe->pipe_num < OVERLAY_PIPE_MAX)){
		save_reg[pipe->pipe_num].save_reg_MDP_PIPE_SRC_SIZE = inpdw(rgb_base + 0x0000);
		save_reg[pipe->pipe_num].save_reg_MDP_PIPE_OP_MODE = inpdw(rgb_base + 0x0058);
	}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00027 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00007 */ /* CUST_ID_00029 */
	if((inpdw(vg_base + 0x0058)) & 0x20000){
		pipe->op_mode |= 0x20000;
	}else{
		pipe->op_mode &= ~0x20000;
	}
	if((inpdw(vg_base + 0x0058)) & 0x8000){
		pipe->op_mode |= 0x8000;
	}else{
		pipe->op_mode &= ~0x8000;
	}
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00008 */ /* CUST_ID_00029 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00138 */
	if((pipe->mixer_num == MDP4_MIXER2) && (pipe->pipe_num < OVERLAY_PIPE_MAX)){
		save_reg[pipe->pipe_num].save_reg_MDP_PIPE_SRC_SIZE = inpdw(vg_base + 0x0000);
		save_reg[pipe->pipe_num].save_reg_MDP_PIPE_OP_MODE = inpdw(vg_base + 0x0058);
	}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00138 */
	if((pipe->mixer_num == MDP4_MIXER2) && (pipe->pipe_num < OVERLAY_PIPE_MAX)){
		save_reg[pipe->pipe_num].save_reg_MDP_PIPE_SRC_SIZE = 0;
		save_reg[pipe->pipe_num].save_reg_MDP_PIPE_OP_MODE = 0;
	}
#endif /* CONFIG_SHLCDC_BOARD */
}
/*
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00171 */
	struct mdp4_iommu_pipe_info iom;
#endif /* CUST_ID_00171 */
	...
	for (i = MDP4_MIXER_STAGE_BASE; i < MDP4_MIXER_STAGE_MAX; i++) {
		...
		if ((pp->dst_x <= sp->dst_x) &&
			...
			if ((pp->dst_y <= sp->dst_y) &&
				((pp->dst_y + pp->dst_h) >=
					(sp->dst_y + sp->dst_h))) {
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00151 */
				if ((pp->pipe_type != OVERLAY_TYPE_VIDEO) ||
					(sp->pipe_type != OVERLAY_TYPE_VIDEO)) {
					kp = pp;
				}
#else
				kp = pp;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00138 */
	if((pipe->mixer_num == MDP4_MIXER2) && (pipe->pipe_num < OVERLAY_PIPE_MAX)){
		if(save_reg[pipe->pipe_num].save_reg_MDP_PIPE_SRC_SIZE != 0){
			src_size = save_reg[pipe->pipe_num].save_reg_MDP_PIPE_SRC_SIZE;
			outpdw(base + 0x0058, save_reg[pipe->pipe_num].save_reg_MDP_PIPE_OP_MODE);
		}
	}
#endif /* CONFIG_SHLCDC_BOARD */
...
void mdp4_mixer_blend_setup(int mixer)
{
	...
	for (i = MDP4_MIXER_STAGE0; i < MDP4_MIXER_STAGE_MAX; i++) {
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00099 */
		alpha_drop = 0; 
#endif /* CONFIG_SHLCDC_BOARD */
		blend->solidfill = 0;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00190 */
#ifdef CONFIG_MACH_LYNX_DL12
#define OVERLAY_BUS_SCALE_TABLE_BASE	7
#else
#define OVERLAY_BUS_SCALE_TABLE_BASE	6
#endif /* CONFIG_MACH_LYNX_DL12 */
#else
#define OVERLAY_BUS_SCALE_TABLE_BASE	6
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00095 */
	mdp_req_overlay_id_debug(req->id);
#endif /* CONFIG_SHLCDC_BOARD */
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	struct mdp4_overlay_pipe *pipe;
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00019 */
	struct dpp_ctrl dpp;
	int i;
#endif
	if (mfd == NULL)
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00007 */ /* CUST_ID_00008 */ /* CUST_ID_00009 */ /* CUST_ID_00029 */
	if (pipe->pipe_num <= OVERLAY_PIPE_VG2) {
		if ((mfd->panel_info.pdest == DISPLAY_1)
				&& (mdp4_overlay_format2type(pipe->src_format) == OVERLAY_TYPE_VIDEO)) {
			mdp4_vg_dither_disable(pipe->pipe_num);
			vg_dither_set_flg = 0;

			mdp4_vg_hist_lut_disable(pipe->pipe_num);
			vg_hist_lut_set_flg = 0;

			mdp4_vg_qseed_update_init();
			mdp4_vg_qseed_setup();
			vg_qseed_set_flg = 0;
...
int mdp4_overlay_vsync_get_status(void)
{
	
	int status = 0;
	
	if (ctrl->panel_mode & MDP4_PANEL_DSI_VIDEO){
		status = mdp4_dsi_video_vsync_get_status();
	} else if (ctrl->panel_mode & MDP4_PANEL_DSI_CMD){
		status = mdp4_dsi_cmd_vsync_get_status();
	}
...
void mdp4_overlay_vsync_commit(struct mdp4_overlay_pipe *pipe)
{
	...
	if (pipe->pipe_type == OVERLAY_TYPE_VIDEO)
		if (pipe->mixer_num == MDP4_MIXER1) {
			if (pipe->src_w > pipe->dst_w*3 || pipe->src_h > pipe->dst_h*3) {
					pipe->src_w = pipe->dst_w;
					pipe->src_h = pipe->dst_h;
			}
		}
...
int mdp4_overlay_play(struct fb_info *info, struct msmfb_overlay_data *req)
{
	...
	ulong start, addr;
	ulong len = 0;
#ifndef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00095 */
	mdp_req_overlay_play_debug(req);
#endif /* CONFIG_SHLCDC_BOARD */
	...
	if (pipe->pipe_num <= OVERLAY_PIPE_VG2) {
		if ((mfd->panel_info.pdest == DISPLAY_1)
				&& (mdp4_overlay_format2type(pipe->src_format) == OVERLAY_TYPE_VIDEO)) {
			if (!vg_dither_set_flg) {
				mdp4_vg_dither_setup(pipe->pipe_num);
				vg_dither_set_flg = 1;
			}
			if (!vg_hist_lut_set_flg) {
				mdp4_vg_hist_lut_setup(pipe->pipe_num);
				vg_hist_lut_set_flg = 1;
			}
			if (!vg_qseed_set_flg) {
				mdp4_vg_qseed_setup();
				vg_qseed_set_flg = 1;
			...
			if (!vg_csc_set_flg) {
				mdp4_vg_csc_setup(pipe->pipe_num);
				vg_csc_set_flg = 1;
			}
		}
	}
