...
#define SHMHL_DEBUG_LOG_SYS_FS_ENTRY
#else
/* #define DEBUG */
...
#ifdef CONFIG_SII8334_MHL_TX
struct work_struct sh_hdmi_state_work;
struct work_struct sh_hdmi_hpd_feature_off_work;
struct work_struct sh_hdmi_hpd_feature_on_work;
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
static void sh_hdmi_after_probe_proc(struct work_struct *poWork);
static void sh_hdmi_hpd_feature_off(struct work_struct *poWork);
static void sh_hdmi_hpd_feature_on(struct work_struct *poWork);
#endif /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
	/* For DDC I2C Start and Stop condition Timing */
...
/*	480p60	480i60	576p50	576i50	720p60	 720p50	1080p60	1080i60	1080p50
	...
	{0x00,	0x00,	0x00,	0x00,	0x00,	0x00,	0x00,	0x00,	0x00,
	 0x00,	0x00,	0x00,	0x00,	0x80, 0x00, 0x00}, /*02*/
#else /* Original */
...
#ifdef CONFIG_SII8334_MHL_TX
	if (changed || external_common_state->default_res_supported || (var->reserved[3]-1 == HDMI_VFRMT_720x480p60_16_9)) {
#else //CONFIG_SII8334_MHL_TX
...
#ifdef CONFIG_SII8334_MHL_TX
	queue_work(hdmi_work_queue, &sh_hdmi_state_work);
#else /* CONFIG_SII8334_MHL_TX */
	if (hdmi_prim_display) {
		rc = hdmi_msm_hpd_on();
		if (rc)
...
static int hdmi_msm_hpd_feature(int on)
{
	...
	if (on) {
#ifdef CONFIG_SII8334_MHL_TX
		queue_work(hdmi_work_queue, &sh_hdmi_hpd_feature_on_work);
#else /* CONFIG_SII8334_MHL_TX */
		rc = hdmi_msm_hpd_on();
#endif /* CONFIG_SII8334_MHL_TX */
	} else {
#ifdef CONFIG_SII8334_MHL_TX
		queue_work(hdmi_work_queue, &sh_hdmi_hpd_feature_off_work);
	}
#else /* CONFIG_SII8334_MHL_TX */
		if (external_common_state->hpd_state) {
			external_common_state->hpd_state = 0;
...
static int __init hdmi_msm_init(void)
{
	...
	*/
#ifdef CONFIG_SII8334_MHL_TX
	hdmi_work_queue = alloc_ordered_workqueue("hdmi_hdcp", 0);
#else /* CONFIG_SII8334_MHL_TX */
...
#ifdef CONFIG_SII8334_MHL_TX
	INIT_WORK(&sh_hdmi_state_work, sh_hdmi_after_probe_proc);
	INIT_WORK(&sh_hdmi_hpd_feature_off_work, sh_hdmi_hpd_feature_off);
	INIT_WORK(&sh_hdmi_hpd_feature_on_work, sh_hdmi_hpd_feature_on);
#endif /* CONFIG_SII8334_MHL_TX */
...
static void sh_hdmi_hpd_feature_off(struct work_struct *poWork)
{
	...
	if (hdmi_msm_state->panel_power_on) {
		...
		INIT_COMPLETION(hdmi_msm_state->hpd_event_processed);
		wait_for_completion_interruptible_timeout(
			&hdmi_msm_state->hpd_event_processed, HZ*6);
	}
	hdmi_msm_hpd_off();
