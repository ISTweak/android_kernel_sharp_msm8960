...
#endif
#include "mipi_dsi.h"
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00020 */ /* CUST_ID_00022 */ /* CUST_ID_00024 */ /* CUST_ID_00036 */ /* CUST_ID_00097 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */ /* CUST_ID_00022 */ /* CUST_ID_00058 */ /* CUST_ID_00084 */ /* CUST_ID_00101 */
extern int overlay_start;
extern int overlay_stop;
extern boolean lcdon_cmd_send;
extern boolean msm_fb_shutdown_in_progress;

struct dma_abl_data lut_abl_brightness[MDP_ABL_LUT_SIZE];
struct dma_abl_data lut_abl_gamma[MDP_ABL_LUT_SIZE];
struct dma_abl_data lut_abl_overlay[MDP_ABL_LUT_SIZE];
struct dma_abl_data lut_abl_adjust[MDP_ABL_LUT_SIZE];
struct dma_abl_data lut_abl_adjust2[MDP_ABL_LUT_SIZE];
struct mixed_dma_abl_data lut_mixed[MDP_ABL_LUT_SIZE];

struct dma_abl_data lut_abl_overlay_tmp[MDP_ABL_LUT_SIZE];
static int abl_overlay_enable;
static int abl_overlay_len;

static int mdp_dma_abl_lut_set(struct fb_cmap *cmap);
static int mdp_dma_abl_lut_update(struct fb_cmap *cmap);
static int mdp_dma_abl_lut_set_init(void);
static int mdp_dma_abl_lut_read_diagarea(void);
static int mdp_lut_update_overlay_lcdc(void);
#endif	/* CONFIG_SHLCDC_BOARD */

static int mdp_lut_i;
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */ /* CUST_ID_00022 */ /* CUST_ID_00037 */ /* CUST_ID_00038 */ /* CUST_ID_00058 */ /* CUST_ID_00083 */
static int mdp_dma_abl_lut_set(struct fb_cmap *cmap)
{
	int ret, len;
	...
	ret = mdp_dma_abl_lut_update(cmap);
	if (ret) {
		return -EFAULT;
	}

	mdp_dma_abl_lut_combine();

	ret = mdp_dma_abl_lut_hw_setup(len);
	if (ret) {
		return -EFAULT;
	}
...
static int mdp_dma_abl_lut_update(struct fb_cmap *cmap)
{
	...
	for (i = 0; i < cmap->len; i++) {
		...
		switch (cmap->classification) {
		case MDP_LUT_ABL_BRIGHTNESS:
			lut_abl_brightness[i].red = r;
			lut_abl_brightness[i].green = g;
			lut_abl_brightness[i].blue = b;
			break;

		case MDP_LUT_ABL_GAMMA:
			lut_abl_gamma[i].red = r;
			lut_abl_gamma[i].green = g;
			lut_abl_gamma[i].blue = b;
			break;

		case MDP_LUT_ABL_OVERLAY:
			lut_abl_overlay[i].red = r;
			lut_abl_overlay[i].green = g;
			lut_abl_overlay[i].blue = b;
			break;

		case MDP_LUT_ABL_ADJUST:
			lut_abl_adjust[i].red = r;
			lut_abl_adjust[i].green = g;
			lut_abl_adjust[i].blue = b;
			break;

		case MDP_LUT_ABL_ADJUST2:
			lut_abl_adjust2[i].red = r;
			lut_abl_adjust2[i].green = g;
			lut_abl_adjust2[i].blue = b;
			break;
...
int mdp_dma_abl_lut_combine(void)
{
	int i;

	for (i = 0; i < MDP_ABL_LUT_SIZE; i++) {
		lut_mixed[i].red = (unsigned char)lut_abl_adjust2[lut_abl_gamma[lut_abl_overlay[lut_abl_brightness[lut_abl_adjust[i].red].red].red].red].red;
		lut_mixed[i].green = (unsigned char)lut_abl_adjust2[lut_abl_gamma[lut_abl_overlay[lut_abl_brightness[lut_abl_adjust[i].green].green].green].green].green;
		lut_mixed[i].blue = (unsigned char)lut_abl_adjust2[lut_abl_gamma[lut_abl_overlay[lut_abl_brightness[lut_abl_adjust[i].blue].blue].blue].blue].blue;
	}

	return 0;
}

int mdp_dma_abl_lut_hw_setup(int len)
{
	int i;
...
int mdp_dma_abl_lut_hw_enable(void)
{
	uint32_t out;
	...
	MDP_OUTP(MDP_BASE + 0x90070, ((mdp_lut_i << 10) | out) | 0x07);
...
int mdp_dma_abl_lut_hw_enable_command_mode(void)
{
...
static int mdp_dma_abl_lut_set_init(void)
{
	...
	for (i = 0; i < MDP_ABL_LUT_SIZE; i++) {
		lut_abl_brightness[i].red = lut_abl_brightness[i].green = lut_abl_brightness[i].blue = (unsigned short)i;
		lut_abl_overlay[i].red = lut_abl_overlay[i].green = lut_abl_overlay[i].blue = (unsigned short)i;
		lut_abl_adjust[i].red = lut_abl_adjust[i].green = lut_abl_adjust[i].blue = (unsigned short)i;
		lut_abl_adjust2[i].red = lut_abl_adjust2[i].green = lut_abl_adjust2[i].blue = (unsigned short)i;
	}

	return 0;
}

int mdp_dma_abl_lut_set_dummy(void)
{
	int i;
...
int mdp_dma_abl_lut_invalid_brightness(void)
{
	int i;
...
int mdp_dma_abl_lut_store_overlay(struct fb_cmap *cmap)
{
	int i;
	u16 r, g, b;

	abl_overlay_enable = cmap->enable;
	abl_overlay_len = cmap->len;
	...
	for (i = 0; i < cmap->len; i++) {
		...
		lut_abl_overlay_tmp[i].red = r;
		lut_abl_overlay_tmp[i].green = g;
		lut_abl_overlay_tmp[i].blue = b;
	}

	return 0;
}

int mdp_dma_abl_lut_set_overlay(struct fb_info *info)
{
	memcpy(lut_abl_overlay, lut_abl_overlay_tmp, sizeof(lut_abl_overlay));

	mdp_dma_abl_lut_combine();

	mdp_lut_update_overlay_lcdc();
...
{
	int i;
	sharp_smem_common_type *p = NULL;

	p = sh_smem_get_common_address();
	if( p == NULL) {
		return -EFAULT;
	} else {
		memcpy(&shdisp_boot_ctx, (struct shdisp_boot_context *)p->shdisp_data_buf, sizeof(shdisp_boot_ctx));

		for (i = 0; i < MDP_ABL_LUT_SIZE; i++) {
			lut_abl_gamma[i].red   = (uint16_t)shdisp_boot_ctx.abl_lut_gamma.abl_color[i].red;
			lut_abl_gamma[i].green = (uint16_t)shdisp_boot_ctx.abl_lut_gamma.abl_color[i].green;
			lut_abl_gamma[i].blue  = (uint16_t)shdisp_boot_ctx.abl_lut_gamma.abl_color[i].blue;
		}
	}
...
{
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */
	int ret;
	uint32_t out;
	...
	if (!cmap->enable) {
		...
		out = inpdw(MDP_BASE + 0x90070) & ~(0x1 << 10);
		MDP_OUTP(MDP_BASE + 0x90070, out & ~0x07);
...
{
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00014 */ /* CUST_ID_00058 */
	int ret;
	uint32_t out;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00015 */ /* CUST_ID_00058 */
static int mdp_lut_update_overlay_lcdc(void)
{
	int ret;
	uint32_t out;

	if (!abl_overlay_enable) {
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	...
	ret = mdp_dma_abl_lut_hw_setup(abl_overlay_len);
	if (ret) {
		return -EFAULT;
	}
...
void mdp_dma_abl_lut_enable(void)
{
	uint32_t out;
...
static int mdp_lut_get_lcdc(struct fb_info *info, struct msmfb_dma_abl_lut_read_data *p)
{
	uint32 data;
	...
	case MDP_LUT_ABL_REG:
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
		data = inpdw(MDP_BASE + 0x90070);
		p->value = inpdw(MDP_BASE + 0x94800 + (0x400 * (((data >> 10) & 0x01))) + (p->step * 4));
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
		break;

	case MDP_LUT_ABL_BRIGHTNESS:
		p->value = ((lut_abl_brightness[p->step].green & 0xff) |
							((lut_abl_brightness[p->step].blue & 0xff) << 8) |
								((lut_abl_brightness[p->step].red & 0xff) << 16));
		break;

	case MDP_LUT_ABL_GAMMA:
		p->value = ((lut_abl_gamma[p->step].green & 0xff) |
							((lut_abl_gamma[p->step].blue & 0xff) << 8) |
								((lut_abl_gamma[p->step].red & 0xff) << 16));
		break;

	case MDP_LUT_ABL_OVERLAY:
		p->value = ((lut_abl_overlay[p->step].green & 0xff) |
							((lut_abl_overlay[p->step].blue & 0xff) << 8) |
								((lut_abl_overlay[p->step].red & 0xff) << 16));
		break;

	case MDP_LUT_ABL_ADJUST:
		p->value = ((lut_abl_adjust[p->step].green & 0xff) |
							((lut_abl_adjust[p->step].blue & 0xff) << 8) |
								((lut_abl_adjust[p->step].red & 0xff) << 16));
		break;

	case MDP_LUT_ABL_ADJUST2:
		p->value = ((lut_abl_adjust2[p->step].green & 0xff) |
							((lut_abl_adjust2[p->step].blue & 0xff) << 8) |
								((lut_abl_adjust2[p->step].red & 0xff) << 16));
		break;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00059 */
		mdp_lut_i = (mdp_lut_i + 1)%2;
#endif	/* CONFIG_SHLCDC_BOARD */
		mutex_unlock(&mdp_lut_push_sem);
	}
...
static int mdp_histogram_disable(struct mdp_hist_mgmt *mgmt)
{
	...
	mdp_intr_mask &= ~mgmt->intr;
	outp32(MDP_INTR_ENABLE, mdp_intr_mask);
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00104 */
#define MDP_HISTOGRAM_TIMEOUT_MS	1000
#else
...
{
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00096 */
	static int mdp_clk_cnt;
#endif /* CONFIG_SHLCDC_BOARD */
	...
	if (on) {
		...
		SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_MDP_CLK_ENABLE,mdp_clk_cnt);
#endif /* CONFIG_SHLCDC_BOARD */
	} else {
		if (mdp_clk_cnt) {
			mdp_clk_cnt--;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00097 */
			SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_MDP_CLK_DISABLE,mdp_clk_cnt);
#endif /* CONFIG_SHLCDC_BOARD */
		}
#ifdef	CONFIG_SHLCDC_BOARD	/* CUST_ID_00141 */
		else
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00096 */
int mdp_get_clk_cnt(void)
{
	return mdp_clk_cnt;
}

void mdp_clk_info_out(void)
{
	u32 wk[4];

	wk[0] = readl(MIPI_DSI_BASE + 0x0200);
	wk[1] = readl(MDP_INTR_STATUS);
	wk[2] = readl(MDP_INTR_ENABLE);
	wk[3] = readl(MDP_INTR_CLEAR);
	pr_err("[ mdp_info   ] pll=0x%lx, status=0x%lx, enable=0x%lx, clear=0x%lx \n",
			(unsigned long)wk[0],
			(unsigned long)wk[1],
			(unsigned long)wk[2],
			(unsigned long)wk[3]
	);
	pr_err("[ ---------- ] mdp_irq_mask=0x%lx, mdp_irq_enabled=%d \n", (unsigned long)mdp_irq_mask, mdp_irq_enabled);
}
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00020 */ /* CUST_ID_00024 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00142 */
	{
		if ((pdata) && (pdata->stop_display)) {
			mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
			pdata->stop_display(mfd);
			mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
		}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00084 */
	lcdon_cmd_send = FALSE;
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00110 */
	if (mipi_dsi_clk_cfg_check())
		mipi_dsi_clk_cfg_clear();
#endif /* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00020 */ /* CUST_ID_00041 */ /* CUST_ID_00084 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00083 */
			mdp4_dma_picture_adjust_setup_command_mode();
#endif /* CONFIG_SHLCDC_BOARD */
		} else if (mfd->panel.type == MIPI_VIDEO_PANEL) {
			mdp4_dsi_video_on(pdev);
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00142 */
			mdp4_dma_picture_adjust_setup();
#endif /* CONFIG_SHLCDC_BOARD */
		} else if (mfd->panel.type == HDMI_PANEL ||
				mfd->panel.type == LCDC_PANEL ||
				mfd->panel.type == LVDS_PANEL) {
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00036 */ /* CUST_ID_00142 */
		disp_status = shdisp_api_get_boot_disp_status();
		if (!disp_status) {
			mdp_pdata->cont_splash_enabled = 0x00;
#ifdef CONFIG_SHDISP_PANEL_FLUTE
			lcdon_cmd_send = FALSE;
#endif
		} else {
			mdp_pdata->cont_splash_enabled = 0x01;
#ifdef CONFIG_SHDISP_PANEL_FLUTE
			lcdon_cmd_send = TRUE;
#endif
		}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00080 */
	mdp4_iommu_attach();
#endif /* CONFIG_SHLCDC_BOARD */
	mutex_lock(&mdp_suspend_mutex);
	mdp_suspended = FALSE;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00022 */

	mdp_dma_abl_lut_set_init();
	mdp_dma_abl_lut_read_diagarea();


	mdp4_dma_argc_lut_read_diagarea();
#endif	/* CONFIG_SHLCDC_BOARD */
