...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00026 */ /* CUST_ID_00033 */ /* CUST_ID_00035 */ /* CUST_ID_00042 */
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00137 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00015 */ /* CUST_ID_00038 */ /* CUST_ID_00051 */ /* CUST_ID_00058 */ /* CUST_ID_00071 */ /* CUST_ID_00084 */ /* CUST_ID_00089 */ /* CUST_ID_00094 */ /* CUST_ID_00101 */ /* CUST_ID_00188 */
int overlay_start;
int overlay_stop;
...
boolean lcdon_cmd_send = FALSE;
static struct msmfb_dma_abl_lut_read_data dma_abl_read_data;
static int shutdown_mode = 0;
boolean msm_fb_shutdown_in_progress = FALSE;
#ifdef CONFIG_SHDISP_USE_CABC
static int panelsave_notify_enable = 0;
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00032 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00001 */
	if (value == 0)
		bl_lvl = 0;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00021 */
	complete(&mfd->msmfb_panelsave_notify);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00026 */ /* CUST_ID_00071 */ /* CUST_ID_00101 */ /* CUST_ID_00143 */
static void msm_fb_shutdown(struct platform_device *pdev)
{
	...
	msm_fb_shutdown_in_progress = TRUE;

    if ((pdata->panel_info.pdest == DISPLAY_1) && (mfd != NULL)){
        if((mfd->panel.type == MIPI_CMD_PANEL) || (mfd->panel.type == MIPI_VIDEO_PANEL)){
			shutdown_mode = 1;
...
int msm_fb_get_shutdown_mode(void)
{
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00101 */
	if (msm_fb_shutdown_in_progress) 
		return 0;
#endif
...
static int msm_fb_suspend_sub(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->suspend.panel_power_on && !mfd->panel_power_on) {
		SHLCDC_ERRLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_SUSPEND_ERR,0);
	}
#endif
...
#endif
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00026 */
...
static void msmfb_early_suspend(struct early_suspend *h)
{
	...
	struct msm_fb_panel_data *pdata = NULL;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00097 */
	struct msm_panel_info *pinfo = (struct msm_panel_info *)&mfd->panel_info;
#endif
...
#endif
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00042 */ /* CUST_ID_00051 */ /* CUST_ID_00101 */ /* CUST_ID_00097 */
	if (msm_fb_shutdown_in_progress) 
		return;
	SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_EARLY_SUSPEND,pinfo->pdest);
	lock_recovery();
	msm_fb_suspend_sub(mfd);
	unlock_recovery();
	fps_low_mode = 0;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00097 */
	SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_EARLY_SUSPEND_RET,pinfo->pdest);
#endif
}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00042 */ /* CUST_ID_00101 */ /* CUST_ID_00097 */
	if (msm_fb_shutdown_in_progress)
		return;
	SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_EARLY_RESUME,pinfo->pdest);
	lock_recovery();
	msm_fb_resume_sub(mfd);
	unlock_recovery();
	SHLCDC_EVENTLOG_REC(FB,LCD,MSM_FB_KERL,EEVENTID_FB_EARLY_RESUME_RET,pinfo->pdest);
#else  
	msm_fb_resume_sub(mfd);
#endif /* CONFIG_SHLCDC_BOARD */
}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00139 */
static int bl_updated_recovery;
#endif /* CONFIG_SHLCDC_BOARD */


#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00024 */ /* CUST_ID_00116 */ /* CUST_ID_00139 */
int msm_fb_suspend_sub_recovery(struct msm_fb_data_type *mfd)
{
	bl_updated_recovery = bl_updated;
	return msm_fb_suspend_sub(mfd);
}
int msm_fb_resume_sub_recovery(struct msm_fb_data_type *mfd)
{
#ifdef CONFIG_PM
	int ret;
...
void msm_fb_resume_sub_recovery_phase2(struct fb_info *info)
{
	struct msm_fb_data_type *mfd;
	struct msm_fb_panel_data *pdata = NULL;
	long long xx;
	...
	if (mfd->panel.type == MIPI_CMD_PANEL) {
		...
		mdp4_overlay_vsync_ctrl(info, 1);
		
		mdp4_dsi_cmd_overlay(mfd);

		if (mfd->panel_power_on && !lcdon_cmd_send) {
		...
		mdp4_overlay_vsync_ctrl(info, 0);
	}

	bl_updated = bl_updated_recovery;
	return;
}
#endif /* CONFIG_SHLCDC_BOARD */
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00023 */ /* CUST_ID_00061 */
	if ( (!mfd->panel_power_on || (!bl_updated && !panel_displayed)) && bkl_lvl ) {
#else	/* CONFIG_SHLCDC_BOARD */
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		mfd->bl_level = temp;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00061 */ /* CUST_ID_00094 */
		bl_level_old = temp;
		pdata->set_backlight(mfd);
		mfd->bl_level = bkl_lvl;
#ifdef CONFIG_SHDISP_USE_CABC
		if(panelsave_notify_enable && bkl_lvl ){
			pdata->cabc_ctrl(mfd->pdev);
			panelsave_notify_enable = 0;
		}
#endif	/* CONFIG_SHDISP_USE_CABC */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00048 */ /* CUST_ID_00134 */
			if (mfd->panel_info.type == MIPI_CMD_PANEL || mfd->panel_info.type == DTV_PANEL)
				mipi_sharp_delay_us(16000);
#else	/* CONFIG_SHLCDC_BOARD */
			msleep(16);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00021 */ /* CUST_ID_00094 */
				complete(&mfd->msmfb_panelsave_notify);
#ifdef CONFIG_SHDISP_USE_CABC
				if (mipi_sharp_is_recovery()) {
					pdata->cabc_ctrl(mfd->pdev);
				}else{
					panelsave_notify_enable = 1;
				}
#endif	/* CONFIG_SHDISP_USE_CABC */
#endif	/* CONFIG_SHLCDC_BOARD */
...
*/
	...
	default:
		if (mfd->panel_power_on) {
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00048 */ /* CUST_ID_00134 */ /* CUST_ID_00144 */
			...
			mfd->op_enable = TRUE;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00023 */ /* CUST_ID_00038 */
			panel_displayed = 0;
			overlay_state = 0;
#endif	/* CONFIG_SHLCDC_BOARD */
		}
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00038 */ /* CUST_ID_00052 */ /* CUST_ID_00058 */
	if (cmap->enable > 1)
		return -EINVAL;

	if ((cmap->classification < MDP_LUT_ABL_REG) || (cmap->classification >= MDP_LUT_ABL_MAX))
		return -EINVAL;

	if ((cmap->red == NULL) || (cmap->green == NULL) || (cmap->blue == NULL))
		return -EINVAL;

	if ((cmap->classification == MDP_LUT_ABL_BRIGHTNESS) && overlay_state)
		return 0;

	if (mfd->panel.type == MIPI_VIDEO_PANEL) {
		if ((cmap->classification == MDP_LUT_ABL_OVERLAY) && overlay_start) {
			mdp_dma_abl_lut_store_overlay(cmap);
			return 0;
		}
	}
#endif	/* CONFIG_SHLCDC_BOARD */
...
static int msm_fb_get_lut(struct msmfb_dma_abl_lut_read_data *p, struct fb_info *info)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	...
	if ((p->classification < MDP_LUT_ABL_REG) || (p->classification >= MDP_LUT_ABL_MAX))
		return -EINVAL;

	if ((p->step < 0) || (p->step >= MDP_ABL_LUT_SIZE))
		return -EINVAL;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00002 */ /* CUST_ID_00081 */
	var->height = panel_info->height_in_mm,	/* height of picture in mm */
	var->width  = panel_info->width_in_mm,	/* width of picture in mm */
#else
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00021 */
	init_completion(&mfd->msmfb_panelsave_notify);
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00065 */ /* CUST_ID_00086 */ /* CUST_ID_00086 */ /* CUST_ID_00093 */
	if (mfd->index != 0) {
		mfd->display_iova = 0;
		mfd->rotator_iova = 0;
	} else {
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00065 */ /* CUST_ID_00086 */
	if (mfd->index == 0) {
		fbram += fix->smem_len;
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00033 */
			      ...
			      struct fb_info *info)
{
	...
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00084 */ /* CUST_ID_00120 */
	struct msm_fb_panel_data *pdata;
#endif	/* CONFIG_SHLCDC_BOARD */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00168 */
	mdp_set_dma_pan_info(info, dirtyPtr,
			     ((var->activate &= ~FB_ACTIVATE_FORCE) == FB_ACTIVATE_VBL));
#else
	mdp_set_dma_pan_info(info, dirtyPtr,
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00084 */ /* CUST_ID_00120 */ /* CUST_ID_00173 */
	if (mfd->panel_info.type == MIPI_CMD_PANEL && mfd->panel_power_on 
			&& !lcdon_cmd_send && (info->node == 0)) {
		if(!mdp4_dsi_cmd_overlay_get_err_flg()){
			pdata = (struct msm_fb_panel_data *)mfd->pdev->
				dev.platform_data;
			if ((pdata) && (pdata->start_display)) {
				mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00137 */
...
#ifndef CONFIG_SHLCDC_BOARD /* CUST_ID_00172 */
	if (unset_bl_level && !bl_updated)
		schedule_delayed_work(&mfd->backlight_worker,
				backlight_duration);
#endif /* CONFIG_SHLCDC_BOARD */
...
static int msmfb_notify_update(struct fb_info *info, unsigned long *argp)
{
	...
	if (notify == NOTIFY_UPDATE_START) {
		INIT_COMPLETION(mfd->msmfb_update_notify);
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00140 */
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00021 */ /* CUST_ID_00038 */ /* CUST_ID_00051 */ /* CUST_ID_00058 */ /* CUST_ID_00089 */ /* CUST_ID_00090 */
static int msmfb_notify_panelsave(struct fb_info *info, unsigned long *argp)
{
	int ret, wait_ret;
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	struct msmfb_panelsave_status panelsave_status;

	ret = copy_from_user(&panelsave_status, argp, sizeof(panelsave_status));
	if (ret) {
		pr_err("%s:ioctl failed\n", __func__);
		return ret;
	}

	if (panelsave_status.direction > MSMFB_PANELSAVE_GETSTATE)
		return -EINVAL;

	if (panelsave_status.direction == MSMFB_PANELSAVE_ENABLE) {
		if (mfd->panel_power_on) {
			INIT_COMPLETION(mfd->msmfb_panelsave_notify);
			wait_ret = wait_for_completion_interruptible(&mfd->msmfb_panelsave_notify);
			if( wait_ret != 0 ){
				return wait_ret;
			}
		}
		panelsave_status.status = MSMFB_PANELSAVE_ENABLE;
	} else if (panelsave_status.direction == MSMFB_PANELSAVE_GETSTATE) {
		
		if ( mfd->panel_power_on ) {
			panelsave_status.status = MSMFB_PANELSAVE_DISABLE;
		} else {
			panelsave_status.status = MSMFB_PANELSAVE_ENABLE;
		}
	} else {
	...
	ret = copy_to_user(argp, &panelsave_status, sizeof(panelsave_status));
	if (ret) {
		pr_err("%s:ioctl failed\n", __func__);
		return ret;
	}
...
static int msmfb_notify_overlay(struct fb_info *info, unsigned long *argp)
{
	int ret, notify;
	...
	if (notify > MSMFB_OVERLAY_ENABLE)
		return -EINVAL;

	if (notify == MSMFB_OVERLAY_ENABLE) {
		overlay_state = 1;
		mdp_dma_abl_lut_invalid_brightness();
		overlay_start = 1;
		overlay_stop = 0;
	} else {
		overlay_state = 0;
...
static int msmfb_change_fps_low(struct fb_info *info, unsigned long *argp)
{
	int ret, notify;
	...
	if (notify > MSMFB_FPS_LOW_ENABLE)
		return -EINVAL;
...
static int msmfb_change_base_fps_low(struct fb_info *info, unsigned long *argp)
{
	int ret, notify;
...
static int msmfb_set_lpf(struct fb_info *info, unsigned long *argp)
{
	int ret;
	struct msmfb_low_power_function_reg reg;
	unsigned char param[2];

	ret = copy_from_user(&reg, argp, sizeof(struct msmfb_low_power_function_reg));
	if (ret) {
		pr_err("%s:ioctl failed\n", __func__);
		return ret;
	}
	...
	param[0] = reg.param1;
	param[1] = reg.param2;
	ret = mipi_sharp_api_set_lpf(param);
...
{
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00032 */
	return 0;
#else	/* CONFIG_SHLCDC_BOARD */
			...
			unsigned long arg)
{
...
#ifdef CONFIG_SHLCDC_BOARD /* CUST_ID_00006 */ /* CUST_ID_00007 */ /* CUST_ID_00008 */ /* CUST_ID_00010 */ /* CUST_ID_00011 */ /* CUST_ID_00012 */ /* CUST_ID_00013 */ /* CUST_ID_00015 */ /* CUST_ID_00021 */ /* CUST_ID_00038 */ /* CUST_ID_00044 */ /* CUST_ID_00051 */ /* CUST_ID_00058 */ /* CUST_ID_00085 */ /* CUST_ID_00089 */ /* CUST_ID_00090 */
	case MSMFB_SET_PIPE_HIST_LUT:
		ret = copy_from_user(&pipe_hist_lut_data, argp, sizeof(pipe_hist_lut_data));
		if (ret) {
			return ret;
		}

		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_vg_hist_lut_update(&pipe_hist_lut_data);
		up(&msm_fb_ioctl_ppp_sem);

		break;

	case MSMFB_SET_PIPE_QSEED:
		ret = copy_from_user(&pipe_qseed_data, argp, sizeof(pipe_qseed_data));
		if (ret)
			return ret;

		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_vg_qseed_update(&pipe_qseed_data);
		up(&msm_fb_ioctl_ppp_sem);

		break;

	case MSMFB_SET_DMA_CONVERT:
		ret = copy_from_user(&dma_convert_data, argp, sizeof(dma_convert_data));
		if (ret)
			return ret;

		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_dma_convert_update(&dma_convert_data);
		if (ret) {
			up(&msm_fb_ioctl_ppp_sem);
			return ret;
		}
	...
	case MSMFB_SET_DMA_PCC:
		ret = copy_from_user(&dma_pcc_data, argp, sizeof(dma_pcc_data));
		if (ret)
			return ret;

		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_dma_pcc_update(&dma_pcc_data);
		if (ret) {
			up(&msm_fb_ioctl_ppp_sem);
			return ret;
		}
	...
	case MSMFB_SET_DMA_ARGC_LUT:
		ret = copy_from_user(&dma_argc_data, argp, sizeof(dma_argc_data));
		if (ret)
			return ret;

		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_dma_argc_lut_update(&dma_argc_data);
		if (ret) {
			up(&msm_fb_ioctl_ppp_sem);
			return ret;
		}
		mdp4_dma_argc_lut_setup();
		up(&msm_fb_ioctl_ppp_sem);

		break;

	case MSMFB_GET_DMA_ARGC_LUT:
		ret = copy_from_user(&dma_argc_read_data, argp, sizeof(dma_argc_read_data));
		if (ret)
			return ret;

		down(&msm_fb_ioctl_ppp_sem);
		ret = mdp4_dma_argc_lut_get(&dma_argc_read_data);
		if (ret) {
			up(&msm_fb_ioctl_ppp_sem);
			return ret;
		}
		...
		ret = copy_to_user(argp, &dma_argc_read_data, sizeof(dma_argc_read_data));
		if (ret)
			return ret;
	...
	case MSMFB_GET_LUT:
		ret = copy_from_user(&dma_abl_read_data, argp, sizeof(dma_abl_read_data));
		if (ret)
			return ret;

		mutex_lock(&msm_fb_ioctl_lut_sem);
		ret = msm_fb_get_lut(&dma_abl_read_data, info);
		if (ret) {
			mutex_unlock(&msm_fb_ioctl_lut_sem);
			return ret;
		}
		...
		ret = copy_to_user(argp, &dma_abl_read_data, sizeof(dma_abl_read_data));
		if (ret)
			return ret;

		break;

	case MSMFB_PANELSAVE_STATUS_IND:
		ret = msmfb_notify_panelsave(info, argp);

		break;

	case MSMFB_OVERLAY_STATUS_IND:
		mutex_lock(&msm_fb_ioctl_lut_sem);
		ret = msmfb_notify_overlay(info, argp);
		mutex_unlock(&msm_fb_ioctl_lut_sem);

		break;

	case MSMFB_CHANGE_FPS_LOW:
		down(&msm_fb_ioctl_ppp_sem);
		ret = msmfb_change_fps_low(info, argp);
		up(&msm_fb_ioctl_ppp_sem);
	...
	case MSMFB_CHANGE_BASE_FPS_LOW:
		down(&msm_fb_ioctl_ppp_sem);
		ret = msmfb_change_base_fps_low(info, argp);
		up(&msm_fb_ioctl_ppp_sem);
