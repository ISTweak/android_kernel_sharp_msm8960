...
#ifdef CONFIG_BATTERY_SH
	[ADC_SCALE_VBATT] = {pm8xxx_adc_scale_vbatt},
	[ADC_SCALE_CAM_TEMP] = {pm8xxx_adc_scale_cam_temp},
#ifdef CONFIG_PM_MPP_12_USE_BACKLIGHT_THERM
	[ADC_SCALE_BACKLIGHT_TEMP] = {pm8xxx_adc_scale_backlight_temp},
...
static struct pm8xxx_mpp_config_data pm8xxx_adc_mpp_unconfig = {
	...
	.level		= PM8XXX_MPP_AOUT_LVL_1V25,
	.control	= PM8XXX_MPP_AOUT_CTRL_DISABLE,
...
#ifdef CONFIG_BATTERY_SH
/* MPP Configuration for custom settings */
static struct pm8xxx_mpp_config_data pm8xxx_adc_mpp_unconfig_custom_1 = {
	.type		= PM8XXX_MPP_TYPE_D_BI_DIR,
	.level		= PM8921_MPP_DIG_LEVEL_S4,
	.control	= PM8XXX_MPP_BI_PULLUP_30KOHM,
};
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
	|| (channel > ADC_MPP_2_ATEST_7 && channel < ADC_MPP_1_CHG_AMUX6)
	|| (channel > ADC_MPP_1_CHG_AMUX6 && channel < ADC_MPP_1_USB_AMUX6)
#ifdef CONFIG_PM_MPP_12_USE_BACKLIGHT_THERM
	|| (channel > ADC_MPP_1_USB_AMUX6 && channel < ADC_MPP_1_BACKLIGHT_AMUX6)
#endif /* CONFIG_PM_MPP_12_USE_BACKLIGHT_THERM */
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
uint32_t pm8xxx_adc_recalib_device(void)
{
	uint32_t result;
	mutex_lock(&pmic_adc->adc_lock);
	result = pm8xxx_adc_calib_device();
	mutex_unlock(&pmic_adc->adc_lock);
	return result;
}
EXPORT_SYMBOL_GPL(pm8xxx_adc_recalib_device);
#endif /* CONFIG_BATTERY_SH */
				...
				struct pm8xxx_adc_chan_result *result)
{
	...
	if (channel < PM8XXX_CHANNEL_MPP_SCALE1_IDX) {
		...
		adc_pmic->conv->amux_channel = channel %
			...
			channel < PM8XXX_CHANNEL_MPP_SCALE1_CHG_IDX) {
		mpp_scale = PREMUX_MPP_SCALE_1_DIV3;
		adc_pmic->conv->amux_channel = channel %
				PM8XXX_CHANNEL_MPP_SCALE3_IDX;
	} else if (channel >= PM8XXX_CHANNEL_MPP_SCALE1_CHG_IDX &&
			channel < PM8XXX_CHANNEL_MPP_SCALE1_USB_IDX) {
		mpp_scale = PREMUX_MPP_SCALE_1;
		adc_pmic->conv->amux_channel = channel %
				PM8XXX_CHANNEL_MPP_SCALE1_CHG_IDX;
	} else {
		mpp_scale = PREMUX_MPP_SCALE_1;
		adc_pmic->conv->amux_channel = channel %
				PM8XXX_CHANNEL_MPP_SCALE1_USB_IDX;
	}

#ifdef CONFIG_PM_MPP_12_USE_BACKLIGHT_THERM
	if(channel >= PM8XXX_CHANNEL_MPP_SCALE1_BACKLIGHT_IDX )
	{
		mpp_scale = PREMUX_MPP_SCALE_1;
		adc_pmic->conv->amux_channel = channel %
				PM8XXX_CHANNEL_MPP_SCALE1_BACKLIGHT_IDX;
	}
#endif /* CONFIG_PM_MPP_12_USE_BACKLIGHT_THERM */
#endif /* CONFIG_BATTERY_SH */
			...
			struct pm8xxx_adc_chan_result *result)
{
	...
	if (mpp_num == 11) {
		...
		if (rev == 0x00000005/*SH_HW_REV_ES1*/) {
			mpp_config_custom = 1;
			pr_debug("pm8xxx adc mpp %d config custom %d\n", mpp_num, mpp_config_custom);
		}
	}
#endif
#endif /* CONFIG_BATTERY_SH */
...
#ifdef CONFIG_BATTERY_SH
	if (mpp_config_custom == 1) {
		rc = pm8xxx_mpp_config(((mpp_num - 1) + adc_pmic->mpp_base),
						&pm8xxx_adc_mpp_unconfig_custom_1);
	} else {
		rc = pm8xxx_mpp_config(((mpp_num - 1) + adc_pmic->mpp_base),
						&pm8xxx_adc_mpp_unconfig);
	}
	if (rc < 0) {
		pr_err("pm8xxx adc mpp %d config error with %d\n", mpp_num, rc);
	}
#else  /* CONFIG_BATTERY_SH */
	rc = pm8xxx_mpp_config(((mpp_num - 1) + adc_pmic->mpp_base),
					&pm8xxx_adc_mpp_unconfig);
