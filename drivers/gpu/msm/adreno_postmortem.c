...
/* TimeOut Check Timer handler*/
static void kgsl_flush_workqueue_timer_handler( unsigned long data ){
       printk( KERN_ERR "kgsl flush_workqueue Timeout\n");
       BUG_ON(1);
}
...
int adreno_postmortem_dump(struct kgsl_device *device, int manual)
{
    ...
    struct timer_list kgsl_flush_workqueue_timer;    
    ...
    /* Add TimeOut Check Timer*/
    init_timer( &kgsl_flush_workqueue_timer );
    kgsl_flush_workqueue_timer.expires  = jiffies + 5*HZ;
    kgsl_flush_workqueue_timer.data     = ( unsigned long )jiffies;
    kgsl_flush_workqueue_timer.function = kgsl_flush_workqueue_timer_handler;
    add_timer( &kgsl_flush_workqueue_timer );

	flush_workqueue(device->work_queue);

    /* Delete TimeOut Check Timer */
    del_timer_sync( &kgsl_flush_workqueue_timer );
