...
static int cpufreq_add_dev(struct device *dev, struct subsys_interface *sif)
{
...
#ifdef CONFIG_SHSYS_CUST
	{
		if (sh_boot_get_bootmode() == SH_BOOT_H_C && sh_smem_get_100hflg())
			policy->governor = (&cpufreq_gov_ondemand);
		else
				...
				struct cpufreq_policy *policy)
{
	...
	if (cpufreq_driver->setpolicy) {
		...
		if (get_msm_thermal_enabled()) {
			if (sh_smem_get_battery_voltage() <= 3800) {
				freq_table = cpufreq_frequency_get_table(policy->cpu);
				data->max = freq_table[5].frequency;
			}
		}
		if (sh_smem_get_softupdate_flg() && sh_boot_get_bootmode() == SH_BOOT_F_T) {
			freq_table = cpufreq_frequency_get_table(policy->cpu);
			data->max = freq_table[0].frequency;
		}
#endif /* CONFIG_SHSYS_CUST */
