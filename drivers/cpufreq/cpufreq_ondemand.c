...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (policy->cur < this_dbs_info->freq_table[6].frequency) {
		policy->cur = this_dbs_info->freq_table[6].frequency;

		__cpufreq_driver_target(policy, this_dbs_info->freq_table[6].frequency,
					CPUFREQ_RELATION_L);
...
#ifdef CONFIG_SHSYS_CUST
	if (is_ignorable_event(type, code))
		return;
#endif /* CONFIG_SHSYS_CUST */
