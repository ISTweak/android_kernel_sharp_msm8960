...
#define TSIF_PKTS_IN_CHUNK_DEFAULT  TSIF_NUM_OF_EACH_TSBUFFER  /**< !! must be equal to TSIF_NUM_OF_EACH_TSBUFFER !! */
#define TSIF_PKTS_IN_CHUNK_DEFAULT_1SEG  TSIF_NUM_OF_EACH_TSBUFFER_1SEG  /**< !! must be equal to TSIF_NUM_OF_EACH_TSBUFFER_1SEG !! */
#define TSIF_CHUNKS_IN_BUF_DEFAULT   (4)
#define TSIF_CHUNKS_IN_BUF_DEFAULT_1SEG  (8)
#else
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )
#define TS_XFER_SIZE              (TSIF_PKTS_IN_CHUNK * TSIF_PKT_SIZE)

tsif_2nd_buffer_t tsif_2nd_buffer;

static void tsif_2nd_buffer_init(tsif_2nd_buffer_t *pbuffer, unsigned num_of_each_tsbuffer, unsigned tsbuffer_width);
#endif	// CONFIG_DTV_TUNERDEV_DL12
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )
unsigned char * tsif_pcktloc_from_num(tsif_2nd_buffer_t *p2nd_buffer, int num)
{
	if( (num >= p2nd_buffer->cnt_data_buffer_ptr * p2nd_buffer->num_of_each_tsbuffer) || (num < 0)){
		return NULL;
	}
	return &(p2nd_buffer->data_buffer_ptr[num * TSIF_PKT_SIZE]);
}
#endif
...
#ifndef CONFIG_DTV_TUNERDEV_DL12
...
{
#ifdef CONFIG_DTV_TUNERDEV_DL12
	int rc;
#else
...
#define TSIF_TS_PACKET_SIZE 192
#define TSIF_TS_PACKET_NUM 200

static unsigned char TsifBuf[TSIF_TS_PACKET_SIZE * TSIF_TS_PACKET_NUM];
static unsigned short TsifWtite;
static unsigned short TsifRead;
static unsigned char TsifLock;

static int tsif_start_hw_S( struct msm_tsif_device *tsif_device )
{
	...
	iowrite32(tsif_device->time_limit, tsif_device->base + TSIF_TIME_LIMIT_OFF);
	wmb();
	iowrite32(ctl | TSIF_STS_CTL_START, tsif_device->base + TSIF_STS_CTL_OFF);
	wmb();
	ctl = ioread32(tsif_device->base + TSIF_STS_CTL_OFF);

	return (ctl & TSIF_STS_CTL_START) ? 0 : -1;
}

static int tsif_stop_hw_S( struct msm_tsif_device *tsif_device )
{
...
static int tsif_read_packet( struct msm_tsif_device *tsif_device )
{
	unsigned char			*p_buf;
	unsigned long			tmp;
	unsigned short			i;
...
//	}
	...
	p_buf = &TsifBuf[ TsifWtite * TSIF_TS_PACKET_SIZE ];

	for ( i=0; i<TSIF_TS_PACKET_SIZE/sizeof(tmp); i++ ) {
		tmp = ioread32(tsif_device->base + TSIF_DATA_PORT_OFF);
		memcpy( p_buf + (i * 4), &tmp, sizeof(tmp) );
	}

	TsifWtite = (TsifWtite + 1) % TSIF_TS_PACKET_NUM;
...
static void tsif_init_buffer(void)
{
	memset( &TsifBuf[0], 0, sizeof(TsifBuf) );
...
int tsif_start_S(void *cookie)
{
	...
	tsif_start_hw_S(tsif_device);

	wake_lock(&tsif_device->wake_lock);

	return 0;
}
EXPORT_SYMBOL(tsif_start_S);

int tsif_stop_S(void *cookie)
{
	struct msm_tsif_device *tsif_device = cookie;

	tsif_stop_hw_S(tsif_device);
...
EXPORT_SYMBOL(tsif_stop_S);

int tsif_read_S(unsigned char *buf, unsigned short count, unsigned short *result)
{
	unsigned short i;
...
//	}
	...
	for ( i=0; i<count; i++ ) {
		if ( TsifWtite == TsifRead ) {
			break;
		}
		if (copy_to_user( buf + (i * TSIF_TS_PACKET_SIZE),
				&TsifBuf[TsifRead * TSIF_TS_PACKET_SIZE],
				TSIF_TS_PACKET_SIZE)) {
			break ;
		}
		(*result)++;
		TsifRead = (TsifRead + 1) % TSIF_TS_PACKET_NUM;
	}
...
EXPORT_SYMBOL(tsif_read_S);

#endif /* TSIF_NON_DMA_TRANSFER end=== */
				    ...
				    struct msm_dmov_errdata *err)
{
	...
	if (result & DMOV_RSLT_DONE) {
		...
		if(reschedule) {
			/* consistency check */
			if (tsif_device->ri != (p2nd_buffer->dev_rptr / TSIF_PKT_SIZE)) {
				printk("%s: inconsistent read pointers: ri %d rptr %d\n",__func__, tsif_device->ri, p2nd_buffer->dev_rptr);
				p2nd_buffer->dev_rptr = tsif_device->ri * TSIF_PKT_SIZE;
			}
			memcpy( &(p2nd_buffer->data_buffer_ptr[p2nd_buffer->wi * TSIF_PKT_SIZE]), tsif_device->data_buffer + p2nd_buffer->dev_rptr, TS_XFER_SIZE);

			p2nd_buffer->wi = (p2nd_buffer->wi + TSIF_PKTS_IN_CHUNK) % (p2nd_buffer->num_of_each_tsbuffer * p2nd_buffer->cnt_data_buffer_ptr);
			
			p2nd_buffer->dev_rptr = (p2nd_buffer->dev_rptr + TS_XFER_SIZE) % (TSIF_PKT_SIZE * TSIF_PKTS_IN_BUF);
			tsif_device->ri = p2nd_buffer->dev_rptr / TSIF_PKT_SIZE;		
			//printk("%s: wi=%d,dev_wi=%d,dev_ri=%d,rptr=%d,2nd_wi=%d\n",__func__,wi,tsif_device->wi,tsif_device->ri,p2nd_buffer->dev_rptr,p2nd_buffer->wi);
		
			//tsif_reclaim_packets(tsif_device, tsif_device->ri);
		}
#endif	// CONFIG_DTV_TUNERDEV_DL12
...
static void tsif_dma_exit(struct msm_tsif_device *tsif_device)
{
	...
	if (tsif_device->data_buffer) {
		...
		dev_info(&tsif_device->pdev->dev, "%s(): data_buffer isn't freed here.\n", __func__);
	}	
#else
	if (tsif_device->data_buffer) {
		tsif_device->blob_wrapper_databuf.data = NULL;
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )	   
	if(!tsif_device->data_buffer){
		tsif_device->data_buffer = dma_alloc_coherent(NULL, (TSIF_PKTS_IN_CHUNK_DEFAULT * TSIF_CHUNKS_IN_BUF_DEFAULT * TSIF_PKT_SIZE),
					&tsif_device->data_buffer_dma, GFP_KERNEL);
		if (!tsif_device->data_buffer)
			goto err;
		dev_info(&tsif_device->pdev->dev, "%s(): data_buffer: %p phys 0x%08x\n",
			 __func__, tsif_device->data_buffer, tsif_device->data_buffer_dma);
	}else{
		dev_info(&tsif_device->pdev->dev, "%s(): data_buffer is already allocated.\n", __func__);
	}
#else
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )
	// Allocate maximum buffer to be required
	tsif_device->data_buffer = dma_alloc_coherent(NULL, (TSIF_PKTS_IN_CHUNK_DEFAULT * TSIF_CHUNKS_IN_BUF_DEFAULT * TSIF_PKT_SIZE),
				&tsif_device->data_buffer_dma, GFP_KERNEL);
	if (!tsif_device->data_buffer){
		dev_err(&pdev->dev, "%s(): failed to allocate dma buffer\n", __func__);
	}else{
		dev_info(&pdev->dev, "%s(): data_buffer: %p phys 0x%08x\n",
			 __func__, tsif_device->data_buffer, tsif_device->data_buffer_dma);
	}
#endif	
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )
	if (tsif_device->data_buffer) {
		dma_free_coherent(NULL, (TSIF_PKTS_IN_CHUNK_DEFAULT * TSIF_CHUNKS_IN_BUF_DEFAULT * TSIF_PKT_SIZE),
				  tsif_device->data_buffer,
				  tsif_device->data_buffer_dma);
		tsif_device->data_buffer = NULL;
	}
...
static void tsif_2nd_buffer_init(tsif_2nd_buffer_t *pbuffer, unsigned num_of_each_tsbuffer, unsigned tsbuffer_width)
{
	pbuffer->num_of_each_tsbuffer = num_of_each_tsbuffer;
	pbuffer->cnt_data_buffer_ptr = tsbuffer_width;
	pbuffer->wi = 0;
	pbuffer->ri = 0;
	pbuffer->rptr = 0;
	pbuffer->dev_rptr = 0;
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )
int tsif_start(void *cookie, int index)
#else
...
#if defined( CONFIG_DTV_TUNERDEV_DL12 )
	unsigned  num_of_each_tsbuffer, tsbuffer_width;
	if (index==0) {				// for 1seg configuration
		tsif_device->pkts_per_chunk = TSIF_PKTS_IN_CHUNK_DEFAULT_1SEG;
		tsif_device->chunks_per_buf = TSIF_CHUNKS_IN_BUF_DEFAULT_1SEG;
		num_of_each_tsbuffer = TSIF_NUM_OF_EACH_TSBUFFER_1SEG;
		tsbuffer_width = TSIF_1SEG_TSBUFFER_WIDTH;
	}
	else {						// for Tmm configuration
		tsif_device->pkts_per_chunk = TSIF_PKTS_IN_CHUNK_DEFAULT;
		tsif_device->chunks_per_buf = TSIF_CHUNKS_IN_BUF_DEFAULT;
		num_of_each_tsbuffer = TSIF_NUM_OF_EACH_TSBUFFER;
		tsbuffer_width = TSIF_MM_TSBUFFER_WIDTH;
	}
	tsif_2nd_buffer_init(&tsif_2nd_buffer, num_of_each_tsbuffer, tsbuffer_width);
#endif	// CONFIG_DTV_TUNERDEV_DL12
