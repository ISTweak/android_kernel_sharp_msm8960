...
static DEFINE_SPINLOCK(charge_timer_lock);
	...
	#define USB_SWIC_INTERRUPTS_BYPASS  (SHSWIC_ID_USB_CABLE | SHSWIC_ID_AC_ADAPTER)
...
#if !defined(CONFIG_USB_MSM_OTG_EXT5V_EXCLUSIVE) && defined(CONFIG_USB_MSM_OTG_SH_CUST)
	static struct regulator *ext_5v;
#endif /* !defined(CONFIG_USB_MSM_OTG_EXT5V_EXCLUSIVE) && defined(CONFIG_USB_MSM_OTG_SH_CUST) */
...
#elif defined( CONFIG_USB_MSM_OTG_SH_CUST)
		if(ext_5v == NULL) {
...
static void msm_otg_swic_irq(u8 device, void *data)
{
	...
	dev_info(otg->phy->dev, "msm_otg_swic_irq device=%d \n", device);
...
static bool msm_otg_mhl_sm_work(struct work_struct *w)
{
	...
	if( shusb_read_data.shusb_mhl_status && (otg->phy->state == OTG_STATE_B_IDLE)){
		shusb_read_data.shusb_mhl_status = false;
#ifdef CONFIG_USB_DEBUG_SH_LOG
