...
#ifdef CONFIG_USB_ANDROID_SH_SERIALS
static unsigned int no_acm_hsic_ports;
#endif /* CONFIG_USB_ANDROID_SH_SERIALS */
...
#ifdef CONFIG_USB_ANDROID_SH_SERIALS
	if (no_acm_hsic_ports) {
		port_idx = ghsic_data_setup(no_acm_hsic_ports, USB_GADGET_SERIAL);
		if (port_idx < 0) {
			pr_err("%s: ghsic_data_setup failed: err:%d\n",
				__func__, port_idx);
			return port_idx;
		}
		...
		ret = ghsic_ctrl_setup(no_acm_hsic_ports, USB_GADGET_SERIAL);
		if (ret < 0) {
			pr_err("%s: ghsic_ctrl_setup failed: err:%d\n",
				__func__, ret);
			return ret;
		}
		return 0;
