...
__acquires(udc->lock)
{
	...
	if (udc->transceiver) {
		...
		otg_notify_chg_info(udc->transceiver, 0);
...
#ifdef CONFIG_USB_MSM_OTG_SH_CUST_SWIC
static irqreturn_t ci13xxx_msm_udc_irq(int irq, void *data)
{
	return udc_irq();
}

static int ci13xxx_irq_control(struct usb_gadget *_gadget, int is_active)
{
	struct ci13xxx *udc = container_of(_gadget, struct ci13xxx, gadget);
	int ret=0;
	...
	if(is_active){
		...
		if(udc->udc_select_irq){
			return ret;
		}

		ret = request_irq(udc->udc_driver->udc_irq_val , ci13xxx_msm_udc_irq, IRQF_SHARED, 
							udc->udc_driver->name, udc->udc_driver);
		if (ret < 0) {
			err("request_irq failed\n");
			return ret;
		}

		udc->udc_select_irq = udc->udc_driver->udc_irq_val;


	}else if(udc->udc_select_irq){


		free_irq(udc->udc_driver->udc_irq_val, udc->udc_driver);

		udc->udc_select_irq = 0;

	}else{
		err("irq_control active %d irq=%d sel_irq=%d\n", is_active , 
		udc->udc_driver->udc_irq_val, udc->udc_select_irq);
	}
...
#ifdef CONFIG_USB_MSM_OTG_SH_CUST_SWIC
	ci13xxx_irq_control(_gadget , is_active);
#endif /* CONFIG_USB_MSM_OTG_SH_CUST_SWIC */
...
static int ci13xxx_notify_chg_info(struct usb_gadget *_gadget, unsigned mA)
{
	struct ci13xxx *udc = container_of(_gadget, struct ci13xxx, gadget);

	if (udc->transceiver)
		return otg_notify_chg_info(udc->transceiver, mA);
	return -ENOTSUPP;
}
...
static int ci13xxx_notify_enumeration_started(struct usb_gadget *_gadget)
{
	struct ci13xxx *udc = container_of(_gadget, struct ci13xxx, gadget);

	if (udc->transceiver)
		return otg_notify_enumeration_started(udc->transceiver);
	return -ENOTSUPP;
}
...
static const struct usb_gadget_ops usb_gadget_ops = {
	...
	.notify_chg_info = ci13xxx_notify_chg_info,
#endif /* CONFIG_USB_ANDROID_SH_CUST */
#ifdef CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE
	.notify_enumeration_started = ci13xxx_notify_enumeration_started,
#endif /* CONFIG_USB_SH_CUST_NON_STANDARD_CHARGE */
};
