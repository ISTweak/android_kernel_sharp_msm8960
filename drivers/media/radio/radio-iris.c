		...
		struct v4l2_queryctrl *qc)
{
    ...
    printk( KERN_DEBUG"iris: %s called (id=%x).\n", __func__, qc->id );
#endif
...
#if defined( SH_FMTX_DEBUG )
    printk( KERN_DEBUG"iris: %s called (id=%x value=%d).\n", __func__, ctrl->id, ctrl->value );
#endif
	switch (ctrl->id) {
...
#if defined( SH_FMTX )
        if( 256 <= ctrl->value && ctrl->value <= 511 ){
            wr_txgain.data[FM_TX_PWR_GAIN_OFFSET] = (ctrl->value) - 256;
        }else{
#endif
...
#endif
#if defined( SH_FMTX_DEBUG )
                printk( KERN_DEBUG"iris: tx_power=%d.\n", (int)wr_txgain.data[FM_TX_PWR_GAIN_OFFSET] );
#endif
...
#if defined( SH_FMTX_DEBUG )
    printk( KERN_DEBUG"iris: %s called.\n", __func__ );
#endif
	if (tuner->index > 0) {
...
#if defined( SH_FMTX_DEBUG )
    printk( KERN_DEBUG"iris: %s called (range=%d %d).\n", __func__, tuner->rangelow, tuner->rangehigh );
#endif
	if (radio->mode == FM_RECV) {
...
#if defined( SH_FMTX_DEBUG )
        printk( KERN_DEBUG"iris: freq=%d\n", freq->frequency / TUNE_PARAM );
#endif
	} else
		return -EINVAL;
	return 0;
...
#if defined( SH_FMTX_DEBUG )
    printk( KERN_DEBUG"iris: %s called (freq=%d).\n", __func__, freq->frequency );
#endif
...
#if defined( SH_FMTX_DEBUG )
    int i;
    printk( KERN_DEBUG"iris: %s called(index=%d).\n", __func__, buffer->index );
#endif
...
#if defined( SH_FMTX_DEBUG )
    printk( KERN_DEBUG"iris: bytesused=%d.\n", buffer->bytesused );
    for( i = 0; i < buffer->bytesused; ++i ){
        printk( KERN_DEBUG"iris: %d\n", buf[i] );
    }
#endif
	return 0;
}
