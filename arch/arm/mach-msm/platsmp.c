...
static void sh_check_core1_on(unsigned int *val);
#endif
...
static void sh_check_core1_on(unsigned int *val)
{
	if (((sh_boot_get_bootmode() == SH_BOOT_H_C) && (sh_smem_get_100hflg())) ||
		(sh_boot_get_bootmode() == SH_BOOT_O_C) || (sh_boot_get_bootmode() == SH_BOOT_U_O_C) ||
		((sh_smem_get_softupdate_flg()) && (sh_boot_get_bootmode() == SH_BOOT_F_T)))
		*val = 1;
}
#endif
