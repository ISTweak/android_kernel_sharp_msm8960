...
#if defined(CONFIG_SHSYS_CUST) || defined(CONFIG_PERFLOCK_BOOT_LOCK)
...
void shsys_enter_state_perf_lock( void )
{
	...
	pm_qos_update_request(&shsys_suspend_sleep, msm_cpuidle_get_deep_idle_latency());
#endif
}
...
static void do_set_rate_fn(struct work_struct *work)
{
	...
	if (ret || !policy.governor)
		return;
	if (strncmp("ondemand", policy.governor->name, 8) == 0)
		freqs.new = perflock_override(&policy, acpuclk_get_rate(freqs.cpu));
	else if (strncmp("performance", policy.governor->name, 11) == 0)
		freqs.new = perflock_override(&policy, policy.max);
	else
		return;
#endif
...
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
static DEFINE_PER_CPU(struct work_struct, do_setrate_work);
#else
...
void perf_lock(struct perf_lock *lock)
{
	...
	for_each_online_cpu(cpu) {
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
		queue_work_on(cpu, perflock_setrate_workqueue, &per_cpu(do_setrate_work, cpu));
#else
...
#endif
{
	...
	policy_min = table[PERF_LOCK_384MHz].frequency;
#else
	if (!pdata)
		goto invalid_config;
...
#ifdef CONFIG_PERFLOCK_SUSPEND_LOCK
	perf_lock_init(&enter_state_perf_lock, PERF_LOCK_702MHz, "enter_state");
#endif
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
	for_each_possible_cpu(i) {
		INIT_WORK(&per_cpu(do_setrate_work, i), do_set_rate_fn);
	}
#endif
	return;
